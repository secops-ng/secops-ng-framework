# GDPR data flow — codebase-vuln-management

Per-workflow GDPR data-flow entry for the
`codebase-vuln-management` cookbook playbook
...
[`content/playbooks/codebase-vuln-management/`](../../playbooks/codebase-vuln-management/).
...
`schemas/evidence/codebase-vuln-management/disclosure-timeline-record.schema.json`
...
sibling `vuln-intake` workflow under
[`data-flow-vuln-intake.md`](./data-flow-vuln-intake.md)), this
...
- the regulator-notification chain owned by the sibling
  `vuln-intake` workflow when (and only when) the codebase finding
...
any, reach the operator through the sibling `vuln-intake` workflow
