# GDPR data flow — vuln-intake

Per-workflow GDPR data-flow entry for the `vuln-intake` cookbook
...
- The **incident commander** of the parallel incident-management
...
- The **regulator destination** the operator has configured for the
  ...
  incident clock. As with the incident-management workflow, the
...
- **Closed disclosures** age into the operator's CVD record-
  ...
  evidence-pack expiry when an incident-management chain ran in
