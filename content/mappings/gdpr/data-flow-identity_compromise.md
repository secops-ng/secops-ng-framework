# GDPR data flow — identity_compromise

Per-workflow GDPR data-flow entry for the `identity_compromise`
...
[`content/playbooks/identity_compromise/`](../../playbooks/identity_compromise/).
...
- **MFA factor inventories pre / post reset** and the
  **session-revocation count** are linked onto the incident case as
  ...
  incident-open + the operator's post_incident_review window;
  bounded by the operator's evidence-pack expiry on the
  incident_management playbook).
