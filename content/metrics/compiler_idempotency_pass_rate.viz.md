# Reference visualisation — `kpi.compiler_idempotency_pass_rate@v1`

This is the committed reference-visualisation artifact for the
compiler idempotency pass rate across repeated identical-input
compile runs KPI. It exists so the G-04 catalog definition-of-done
...
`ratio` aggregate for `kpi.compiler_idempotency_pass_rate@v1`. Each
...
title: "kpi.compiler_idempotency_pass_rate@v1 — repeated-input compiler outcomes (illustrative snapshot)"
---
xychart-beta horizontal
    title "1 = second compile produced identical bytes, 0 = drift"
...
The bands match the `thresholds` array on `compiler_idempotency_pass_rate.yaml`;
...
declared under `measurement.inputs` on `compiler_idempotency_pass_rate.yaml`.
