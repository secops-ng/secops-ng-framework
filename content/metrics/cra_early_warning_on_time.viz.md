# Reference visualisation — `kpi.cra_early_warning_on_time@v1`

This is the committed reference-visualisation artifact for the CRA
Article 14(1) 24-hour early-warning on-time-rate KPI. It exists so the
...
horizontal bar chart, one bar per CRA Article 14(1) notification
...
- **Drill-down x-axis:** `dispatch_margin_minutes` — minutes
  remaining at dispatch against the 24-hour early-warning clock. Positive
...
title: "kpi.cra_early_warning_on_time@v1 — dispatch margin per CRA Article 14(1) notification (P30D window)"
---
xychart-beta horizontal
    title "minutes of slack against the 24-hour early-warning clock at dispatch"
    x-axis "notification (dispatched in window)" ["case-N1", "case-N2", "case-N3", "case-N4", "case-N5"]
    y-axis "dispatch_margin_minutes" -240 --> 1500
    bar [-120, 45, 360, 720, 1380]
```
...
| case-N4 | 720                    | yes      | mid-window dispatch, healthy slack                     |
| case-N5 | 1380                    | yes      | early dispatch, large slack                            |
...
The bands match the `thresholds` array on `cra_early_warning_on_time.yaml`; the catalog
...
CRA Article 14(1) reporting pipeline. Each dispatched notification in
the evaluation window contributes one `dispatch_margin_minutes`
sample computed from the two inputs declared in `cra_early_warning_on_time.yaml`'s
`measurement.inputs`:
...
- `awareness_timestamp` — the operator-awareness timestamp recorded
  ...
  of the 24-hour early-warning clock.
- `notification_dispatch` — the regulator-notification chain step
  transition that dispatches the 24-hour early-warning notification to the
...
`clock` is the 24-hour early-warning clock defined by CRA Article 14(1).
