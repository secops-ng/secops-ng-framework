# Reference visualisation — `kpi.non_eu_saas_free_workflow_ratio@v1`

This is the committed reference-visualisation artifact for the
reference workflows executable end-to-end without any non-EU SaaS KPI. It exists so the G-04 catalog
...
`ratio` aggregate for `kpi.non_eu_saas_free_workflow_ratio@v1`. Each bar plots one observation
...
title: "kpi.non_eu_saas_free_workflow_ratio@v1 — reference workflows executable end-to-end without any non-EU SaaS (illustrative snapshot)"
---
xychart-beta horizontal
    title "1 = no non-EU SaaS touched, 0 = non-EU SaaS touched"
...
The bands match the `thresholds` array on `non_eu_saas_free_workflow_ratio.yaml`; the catalog
...
declared under `measurement.inputs` on `non_eu_saas_free_workflow_ratio.yaml`. Each observation
