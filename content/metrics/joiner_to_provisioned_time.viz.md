# Reference visualisation — `kri.joiner_to_provisioned_time@v1`

This is the committed reference-visualisation artifact for the
joiner-to-provisioned time KRI on the onboarding edge of the
...
p50 / p95 latency histogram of `joiner_provisioning_latency_minutes`
across access-evidence artifacts emitted within the evaluation
window for joiner lifecycle events, sliced by `identity_provider` on
a daily-tumbling cadence. The `p95` aggregate is the headline figure
operators read first; the per-day, per-provider histogram is the
supporting drill-down that names *which* identity sources are pulling
the provisioning tail.
...
- **y-axis:** `joiner_provisioning_latency_minutes` — minutes between
  `lifecycle_event.effective_at` and `artifact.captured_at` for each
  joiner access-evidence artifact whose confirm-grant-revoke step
  recorded the declared add-set as observed-present.
...
- **Headline annotation:** the `p95` aggregate across all in-scope
  joiner access-evidence artifacts in the window, annotated as the
...
config:
    xyChart:
        showTitle: true
title: "kri.joiner_to_provisioned_time@v1 — joiner provisioning latency (p50 / p95 by day, P30D window)"
---
xychart-beta
    title "minutes from lifecycle effective_at to access-evidence captured_at (joiner add-set observed-present)"
    x-axis "day in window" ["d-05", "d-04", "d-03", "d-02", "d-01"]
    y-axis "joiner_provisioning_latency_minutes" 0 --> 1800
    bar [42, 51, 38, 48, 55]
    line [180, 1620, 240, 130, 1480]
```
...
|------|-----------|------------|------------|------------------------------------------------------|
| d-05 | 42        | 180        | warn       | above 60-min warn floor, below breach                |
| d-04 | 51        | 1620       | breach     | above 1440-min breach floor — slow joiner tail       |
| d-03 | 38        | 240        | warn       | inside warn band                                     |
| d-02 | 48        | 130        | warn       | just above warn floor                                |
| d-01 | 55        | 1480       | breach     | above breach floor — provisioning-pipeline lag       |
...
`joiner_to_provisioned_time.yaml`; the catalog entry is the source of
truth, this file is the visualisation surface. The `target` (≤60 min)
is the floor the warn band sits above. Asset-tier-scoped variants
(for example a privileged-role-scoped joiner provisioning indicator)
tighten these targets and live as separate catalog entries.

## OCSF source-data shape

The chart's underlying observations are derived from the operator's
identity-lifecycle pipeline. Each joiner access-evidence artifact
emitted by the `onboarding_offboarding_tracker` playbook contributes
one `joiner_provisioning_latency_minutes` sample computed from the
inputs declared in `joiner_to_provisioned_time.yaml`'s
`measurement.inputs`:

- `lifecycle_event_effective_at` — declared `effective_at` field on
  the joiner lifecycle event ingested by
...
- `confirmed_capability_observation` — the confirm-grant-revoke step
  transition that records the joiner's declared add-set as
  observed-present on the operator's identity source, bound to
...
timestamps per joiner access-evidence artifact and computes a
