# Reference visualisation — `kpi.mttd_cloud_misconfig@v1`

This is the committed reference-visualisation artifact for the
cloud-misconfiguration-scoped mean-time-to-detect KPI. It exists so
...
*which* misconfiguration findings pulled the tail.

- **x-axis:** `detection_latency_minutes` — minutes between
  `earliest_telemetry_event_timestamp` and
  `first_detection_fire_timestamp` for each closed cloud-misconfiguration
...
- **Threshold overlays:** the unscoped baseline catalog entry
  ...
  access-control obligations typically tighten cloud-platform-specific
  values in their own scoped overrides.
- **Headline annotation:** the `p95` aggregate across closed
  cloud-misconfiguration incidents, annotated as the metric value.
...
title: "kpi.mttd_cloud_misconfig@v1 — detection latency per closed cloud-misconfiguration incident (P30D window)"
---
xychart-beta horizontal
    title "minutes from earliest telemetry to first detection fire (cloud-misconfiguration-rooted)"
    x-axis "incident (closed in window)" ["case-C1", "case-C2", "case-C3", "case-C4", "case-C5"]
    y-axis "detection_latency_minutes" 0 --> 360
    bar [320, 180, 95, 40, 15]
```
...
|---------|---------------------------|------------------------|--------------------------------------------------------|
| case-C1 | 320                       | breach                 | above 240-min breach floor — drift sat unflagged       |
| case-C2 | 180                       | warn                   | inside warn band                                       |
| case-C3 | 95                        | warn                   | just above warn floor                                  |
| case-C4 | 40                        | on-target              | under 60-min target floor                              |
| case-C5 | 15                        | on-target              | well under target — drift caught at deploy time        |

The headline `p95` figure here is `≈ 320 min` — that value is what
...
typically tighten cloud-misconfiguration-specific values in their own
scoped overrides given the NIS2 Art. 21(2)(e) access-control surface.
See `mttd.yaml` and `mttd.viz.md` for the inherited numeric bands.

## OCSF source-data shape

The chart's underlying observations are derived from the operator's
detection pipeline for cloud-misconfiguration-rooted incidents. Each
closed incident contributes one `detection_latency_minutes` sample
computed from the two inputs declared in
`mttd_cloud_misconfig.yaml`'s `measurement.inputs`:

- `earliest_telemetry_event` — earliest event in the causal chain
  (typically cloud-platform configuration-change, posture-finding, or
  identity-and-access telemetry for the cloud-misconfiguration
  containment path declared in `playbook_refs[]`). The catalog entry
  does not pin a single OCSF class at this scope — the
  cloud-misconfiguration playbook step
