# Reference visualisation — `kpi.reference_deployment_target_coverage@v1`

This is the committed reference-visualisation artifact for the
reference compile-target ring coverage (n8n / temporal / langgraph) per workflow KPI. It exists so the G-04 catalog
...
`ratio` aggregate for `kpi.reference_deployment_target_coverage@v1`. Each bar plots one observation
...
title: "kpi.reference_deployment_target_coverage@v1 — reference compile-target ring coverage (n8n / temporal / langgraph) per workflow (illustrative snapshot)"
---
xychart-beta horizontal
    title "1 = full ring covered, 0 = missing on any target"
...
The bands match the `thresholds` array on `reference_deployment_target_coverage.yaml`; the catalog
...
declared under `measurement.inputs` on `reference_deployment_target_coverage.yaml`. Each observation
