# Reference visualisation — `kpi.cra_notification_72h_on_time@v1`

This is the committed reference-visualisation artifact for the CRA
Article 14(2) 72-hour notification on-time-rate KPI. It exists so the
...
- **Drill-down x-axis:** `dispatch_margin_minutes` — minutes
  remaining at dispatch against the 72-hour notification clock. Positive
...
title: "kpi.cra_notification_72h_on_time@v1 — dispatch margin per CRA Article 14(2) notification (P30D window)"
---
xychart-beta horizontal
    title "minutes of slack against the 72-hour notification clock at dispatch"
...
The bands match the `thresholds` array on `cra_notification_72h_on_time.yaml`; the catalog
...
sample computed from the two inputs declared in `cra_notification_72h_on_time.yaml`'s
`measurement.inputs`:
...
- `awareness_timestamp` — the operator-awareness timestamp recorded
  ...
  of the 72-hour notification clock.
- `notification_dispatch` — the regulator-notification chain step
  transition that dispatches the 72-hour notification notification to the
...
`clock` is the 72-hour notification clock defined by CRA Article 14(2).
