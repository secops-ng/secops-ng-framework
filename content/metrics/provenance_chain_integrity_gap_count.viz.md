# Reference visualisation — `kri.provenance_chain_integrity_gap_count@v1`

This is the committed reference-visualisation artifact for the
provenance-chain integrity gap count across content-artifact lineage
...
`count` aggregate for `kri.provenance_chain_integrity_gap_count@v1`.
...
the `count` roll-up across the bars per the catalog
...
title: "kri.provenance_chain_integrity_gap_count@v1 — provenance-chain integrity gap count across content-artifact lineage (illustrative snapshot)"
---
xychart-beta horizontal
    title "count of compiled artifacts with broken lineage"
...
The bands match the `thresholds` array on `provenance_chain_integrity_gap_count.yaml`;
...
declared under `measurement.inputs` on `provenance_chain_integrity_gap_count.yaml`.
...
class is `telemetry.ocsf.detection_finding@v1`, matched on
`metadata.uid`.
