# Reference visualisation — `kpi.audit_log_completeness_ratio@v1`

This is the committed reference-visualisation artifact for the
audit-log completeness ratio across playbook step transitions KPI. It
exists so the G-04 catalog definition-of-done (a *committed* reference
...
`ratio` aggregate for `kpi.audit_log_completeness_ratio@v1`. Each bar
...
`higher_is_better` fixes the colour convention — higher bars are better.
...
title: "kpi.audit_log_completeness_ratio@v1 — audit-log completeness ratio across playbook step transitions (illustrative snapshot)"
---
xychart-beta horizontal
    title "1 = step transition carries matching audit record, 0 = missing"
...
The bands match the `thresholds` array on `audit_log_completeness_ratio.yaml`;
...
declared under `measurement.inputs` on `audit_log_completeness_ratio.yaml`.
