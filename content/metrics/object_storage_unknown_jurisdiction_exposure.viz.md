# Reference visualisation — `kri.object_storage_unknown_jurisdiction_exposure@v1`

This is the committed reference-visualisation artifact for the
residual-risk KRI paired with `kpi.sovereign_object_storage_binding_coverage@v1`. It exists so the G-04 catalog
...
it judged as failing. One bar per unknown-jurisdiction binding, sliced by compile target, with the count as the headline — mirroring the paired LM-endpoint exemplar so the two residency KRIs read identically.
...
title: "kri.object_storage_unknown_jurisdiction_exposure@v1 — residual exposure by item"
---
xychart-beta horizontal
    title "object-storage bindings whose jurisdiction the residency guard cannot "
...
The bands match the `thresholds` array on `object_storage_unknown_jurisdiction_exposure.yaml`; the catalog
...
declared under `measurement.inputs` on `object_storage_unknown_jurisdiction_exposure.yaml`. Each observation
...
Paired coverage KPI: `kpi.sovereign_object_storage_binding_coverage@v1` — its `residual_risk_refs` declares this
