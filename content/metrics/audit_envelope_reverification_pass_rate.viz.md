# Reference visualisation — `kpi.audit_envelope_reverification_pass_rate@v1`

This is the committed reference-visualisation artifact for the
audit-envelope re-verification pass rate on stored playbook runs KPI.
...
`ratio` aggregate for `kpi.audit_envelope_reverification_pass_rate@v1`.
...
title: "kpi.audit_envelope_reverification_pass_rate@v1 — audit-envelope re-verification pass rate on stored playbook runs (illustrative snapshot)"
---
xychart-beta horizontal
    title "1 = envelope re-verifies cleanly, 0 = deviation"
...
The bands match the `thresholds` array on `audit_envelope_reverification_pass_rate.yaml`;
...
declared under `measurement.inputs` on `audit_envelope_reverification_pass_rate.yaml`.
