# Reference visualisation — `kpi.sovereign_cloud_provider_diversity@v1`

This is the committed reference-visualisation artifact for the
sovereign cloud provider diversity across reference deployment surfaces KPI. It exists so the G-04 catalog
...
`ratio` aggregate for `kpi.sovereign_cloud_provider_diversity@v1`. Each bar plots one observation
...
title: "kpi.sovereign_cloud_provider_diversity@v1 — sovereign cloud provider diversity across reference deployment surfaces (illustrative snapshot)"
---
xychart-beta horizontal
    title "1 = provider referenced, 0 = provider unreferenced"
...
The bands match the `thresholds` array on `sovereign_cloud_provider_diversity.yaml`; the catalog
...
declared under `measurement.inputs` on `sovereign_cloud_provider_diversity.yaml`. Each observation
