# Reference visualisation — `kri.non_eu_critical_dependency_count@v1`

This is the committed reference-visualisation artifact for the
non-EU-originated critical dependency count in the framework supply chain KRI. It exists so the G-04 catalog
...
`count` aggregate for `kri.non_eu_critical_dependency_count@v1`. Each bar plots one observation
...
title: "kri.non_eu_critical_dependency_count@v1 — non-EU-originated critical dependency count in the framework supply chain (illustrative snapshot)"
---
xychart-beta horizontal
    title "count of distinct non-EU critical dependencies"
...
The bands match the `thresholds` array on `non_eu_critical_dependency_count.yaml`; the catalog
...
declared under `measurement.inputs` on `non_eu_critical_dependency_count.yaml`. Each observation
