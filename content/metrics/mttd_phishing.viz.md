# Reference visualisation — `kpi.mttd_phishing@v1`

This is the committed reference-visualisation artifact for the
phishing-scoped mean-time-to-detect KPI. It exists so the G-04
...
*which* phishing incidents pulled the tail.

- **x-axis:** `detection_latency_minutes` — minutes between
  `earliest_telemetry_event_timestamp` and
  `first_detection_fire_timestamp` for each closed phishing incident
  in the window.
...
- **Threshold overlays:** the unscoped baseline catalog entry
  ...
  does not redeclare numeric values — the reference rendering shows
  the inherited band positions so operators reading the chart see the
  same band geometry as the unscoped baseline.
- **Headline annotation:** the `p95` aggregate across closed
  phishing incidents, annotated as the metric value.
...
title: "kpi.mttd_phishing@v1 — detection latency per closed phishing incident (P30D window)"
---
xychart-beta horizontal
    title "minutes from earliest telemetry to first detection fire (phishing-rooted)"
    x-axis "incident (closed in window)" ["case-P1", "case-P2", "case-P3", "case-P4", "case-P5"]
    y-axis "detection_latency_minutes" 0 --> 300
    bar [275, 140, 70, 35, 12]
```
...
|---------|---------------------------|------------------------|--------------------------------------------------|
| case-P1 | 275                       | breach                 | above 240-min breach floor                       |
| case-P2 | 140                       | warn                   | above 60-min warn floor, below breach            |
| case-P3 | 70                        | warn                   | just above warn floor                            |
| case-P4 | 35                        | on-target              | under 60-min target floor                        |
| case-P5 | 12                        | on-target              | well under target — phishing triage caught early |

The headline `p95` figure here is `≈ 275 min` — that value is what
...
typically tighten phishing-specific values in their own scoped
overrides. See `mttd.yaml` and `mttd.viz.md` for the inherited
numeric bands.

## OCSF source-data shape

The chart's underlying observations are derived from the operator's
detection pipeline for phishing-rooted incidents. Each closed
phishing incident contributes one `detection_latency_minutes` sample
computed from the two inputs declared in `mttd_phishing.yaml`'s
`measurement.inputs`:

- `earliest_telemetry_event` — earliest event in the causal chain
  (typically email, identity, or endpoint telemetry for the
  phishing-triage path declared in `playbook_refs[]`). The catalog
  entry does not pin a single OCSF class at this scope — the
  phishing-triage playbook step
  (`playbook.phishing_triage@v1` /
  `action--c0a17a01-0000-4000-8000-000000000003`) is the playbook
