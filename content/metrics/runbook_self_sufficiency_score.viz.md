# Reference visualisation — `kpi.runbook_self_sufficiency_score@v1`

This is the committed reference-visualisation artifact for the runbook
self-sufficiency score across playbooks with a complete cookbook
walkthrough KPI. It exists so the G-04 catalog definition-of-done
...
`ratio` aggregate for `kpi.runbook_self_sufficiency_score@v1`. Each
...
title: "kpi.runbook_self_sufficiency_score@v1 — cookbook completeness (illustrative snapshot)"
---
xychart-beta horizontal
    title "1 = cookbook complete, 0 = missing sections"
...
The bands match the `thresholds` array on `runbook_self_sufficiency_score.yaml`;
...
declared under `measurement.inputs` on `runbook_self_sufficiency_score.yaml`.
