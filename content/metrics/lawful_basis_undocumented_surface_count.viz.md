# Reference visualisation — `kri.lawful_basis_undocumented_surface_count@v1`

This is the committed reference-visualisation artifact for the
residual-risk KRI paired with `kpi.gdpr_lawful_basis_section_coverage@v1`. It exists so the G-04 catalog
...
it judged as failing. One bar per undocumented processing playbook (all bars length 1), so the chart is a to-do list by name; the count is the headline. When the count is zero the chart is deliberately empty rather than omitted — an empty to-do list is the reading.
...
title: "kri.lawful_basis_undocumented_surface_count@v1 — residual exposure by item"
---
xychart-beta horizontal
    title "processing playbooks with no gdpr data-flow document at all — residual"
...
The bands match the `thresholds` array on `lawful_basis_undocumented_surface_count.yaml`; the catalog
...
declared under `measurement.inputs` on `lawful_basis_undocumented_surface_count.yaml`. Each observation
...
Paired coverage KPI: `kpi.gdpr_lawful_basis_section_coverage@v1` — its `residual_risk_refs` declares this
