# Reference visualisation — `kri.cloud_container_unclassifiable_scope_count@v1`

This is the committed reference-visualisation artifact for the
residual-risk KRI paired with `kpi.cloud_posture_coverage@v1`. It exists so the G-04 catalog
...
it judged as failing. One bar per unclassifiable container, grouped by the surface that observed it (inventory, billing), with the count aggregate as the headline. The grouping matters: a container visible to billing but absent from every inventory is a different remediation conversation than one inventoried but never scoped.
...
title: "kri.cloud_container_unclassifiable_scope_count@v1 — residual exposure by item"
---
xychart-beta horizontal
    title "cloud containers whose in-scope state the posture surface cannot class"
...
The bands match the `thresholds` array on `cloud_container_unclassifiable_scope_count.yaml`; the catalog
...
declared under `measurement.inputs` on `cloud_container_unclassifiable_scope_count.yaml`. Each observation
...
Paired coverage KPI: `kpi.cloud_posture_coverage@v1` — its `residual_risk_refs` declares this
