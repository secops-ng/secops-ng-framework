# Reference visualisation — `kri.declared_target_without_example_count@v1`

This is the committed reference-visualisation artifact for the
residual-risk KRI paired with `kpi.reference_deployment_target_coverage@v1`. It exists so the G-04 catalog
...
it judged as failing. One bar per (workflow, target) pair, grouped by target so a systematically unexercised target reads as a block rather than noise; the pair count is the headline.
...
title: "kri.declared_target_without_example_count@v1 — residual exposure by item"
---
xychart-beta horizontal
    title "compile targets declared by a playbook but never exercised by a worked"
...
The bands match the `thresholds` array on `declared_target_without_example_count.yaml`; the catalog
...
declared under `measurement.inputs` on `declared_target_without_example_count.yaml`. Each observation
...
Paired coverage KPI: `kpi.reference_deployment_target_coverage@v1` — its `residual_risk_refs` declares this
