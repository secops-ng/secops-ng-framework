# Reference visualisation — `kpi.dependency_free_ratio@v1`

This is the committed reference-visualisation artifact for the
dependency-free playbook ratio executable without an external SaaS
binding KPI. It exists so the G-04 catalog definition-of-done (a
...
`ratio` aggregate for `kpi.dependency_free_ratio@v1`. Each bar plots
one observation of the shape declared under `measurement.inputs` on
the catalog YAML; the headline aggregate is the `ratio` roll-up
...
title: "kpi.dependency_free_ratio@v1 — playbooks runnable without external SaaS (illustrative snapshot)"
---
xychart-beta horizontal
    title "1 = playbook runs without mandatory external SaaS, 0 = locked"
...
The bands match the `thresholds` array on `dependency_free_ratio.yaml`;
...
declared under `measurement.inputs` on `dependency_free_ratio.yaml`.
