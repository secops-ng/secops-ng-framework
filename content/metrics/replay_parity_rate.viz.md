# Reference visualisation — `kpi.replay_parity_rate@v1`

This is the committed reference-visualisation artifact for the
test-replay byte-parity rate across the three reference compile
targets KPI. It exists so the G-04 catalog definition-of-done (a
*committed* reference visualisation, not a narrated one) is closed;
downstream compile targets (n8n / Temporal / LangGraph) read the same
...
`ratio` aggregate for `kpi.replay_parity_rate@v1`. Each bar plots one
observation of the shape declared under `measurement.inputs` on the
catalog YAML; the headline aggregate is the `ratio` roll-up across
the bars per the catalog `measurement.aggregation`. Direction
`higher_is_better` fixes the colour convention — higher bars are
better.
...
title: "kpi.replay_parity_rate@v1 — cross-target replay byte-parity outcomes (illustrative snapshot)"
---
xychart-beta horizontal
    title "1 = cross-target replay bytes equal, 0 = target drift"
...
The bands match the `thresholds` array on `replay_parity_rate.yaml`;
...
declared under `measurement.inputs` on `replay_parity_rate.yaml`.
