# Reference visualisation — `kri.cross_border_transfer_exposure_count@v1`

This is the committed reference-visualisation artifact for the
cross-border transfer exposure count from reference deployment surfaces KRI. It exists so the G-04 catalog
...
`count` aggregate for `kri.cross_border_transfer_exposure_count@v1`. Each bar plots one observation
...
title: "kri.cross_border_transfer_exposure_count@v1 — cross-border transfer exposure count from reference deployment surfaces (illustrative snapshot)"
---
xychart-beta horizontal
    title "count of non-EU-traversing surfaces"
...
The bands match the `thresholds` array on `cross_border_transfer_exposure_count.yaml`; the catalog
...
declared under `measurement.inputs` on `cross_border_transfer_exposure_count.yaml`. Each observation
