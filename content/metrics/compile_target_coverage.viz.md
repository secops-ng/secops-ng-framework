# Reference visualisation — `kpi.compile_target_coverage@v1`

This is the committed reference-visualisation artifact for the
reference-compile-target coverage across playbooks with the full
three-target ring KPI. It exists so the G-04 catalog
...
`ratio` aggregate for `kpi.compile_target_coverage@v1`. Each bar
...
title: "kpi.compile_target_coverage@v1 — three-target ring presence (illustrative snapshot)"
---
xychart-beta horizontal
    title "1 = all three compile-target examples present, 0 = partial ring"
...
The bands match the `thresholds` array on `compile_target_coverage.yaml`;
...
declared under `measurement.inputs` on `compile_target_coverage.yaml`.
