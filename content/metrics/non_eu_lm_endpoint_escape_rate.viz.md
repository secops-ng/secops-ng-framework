# Reference visualisation — `kri.non_eu_lm_endpoint_escape_rate@v1`

This is the committed reference-visualisation artifact for the
non-EU LM endpoint escape rate (merged-to-main commits) KRI. It exists so the G-04 catalog
...
`ratio` aggregate for `kri.non_eu_lm_endpoint_escape_rate@v1`. Each bar plots one observation
...
title: "kri.non_eu_lm_endpoint_escape_rate@v1 — non-EU LM endpoint escape rate (merged-to-main commits) (illustrative snapshot)"
---
xychart-beta horizontal
    title "1 = at least one NON_EU endpoint on merge, 0 = clean"
...
The bands match the `thresholds` array on `non_eu_lm_endpoint_escape_rate.yaml`; the catalog
...
declared under `measurement.inputs` on `non_eu_lm_endpoint_escape_rate.yaml`. Each observation
