# Reference visualisation — `kpi.eu_regulatory_reference_coverage@v1`

This is the committed reference-visualisation artifact for the
EU-regulatory external reference coverage across the metrics catalogue KPI. It exists so the G-04 catalog
...
`ratio` aggregate for `kpi.eu_regulatory_reference_coverage@v1`. Each bar plots one observation
...
title: "kpi.eu_regulatory_reference_coverage@v1 — EU-regulatory external reference coverage across the metrics catalogue (illustrative snapshot)"
---
xychart-beta horizontal
    title "1 = anchored, 0 = unanchored"
...
The bands match the `thresholds` array on `eu_regulatory_reference_coverage.yaml`; the catalog
...
declared under `measurement.inputs` on `eu_regulatory_reference_coverage.yaml`. Each observation
