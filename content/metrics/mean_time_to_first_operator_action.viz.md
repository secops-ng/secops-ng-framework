# Reference visualisation — `kpi.mean_time_to_first_operator_action@v1`

This is the committed reference-visualisation artifact for the mean
time to first operator action from workflow trigger to first
human-actionable step (MTTFA) KPI. It exists so the G-04 catalog
...
`median` aggregate for `kpi.mean_time_to_first_operator_action@v1`.
Each bar plots one observation of the shape declared under
`measurement.inputs` on the catalog YAML; the headline aggregate is
the `median` roll-up across the bars per the catalog
`measurement.aggregation`. Direction `lower_is_better` fixes the
...
title: "kpi.mean_time_to_first_operator_action@v1 — trigger to first human-actionable step (illustrative snapshot)"
---
xychart-beta horizontal
    title "seconds from workflow trigger to first human-actionable step"
    x-axis "observation" ["sample_a", "sample_b", "sample_c", "sample_d", "sample_e", "sample_f"]
    y-axis "outcome" 0 --> 900
    bar [120, 240, 180, 420, 300, 150]
```
...
The bands match the `thresholds` array on `mean_time_to_first_operator_action.yaml`;
...
declared under `measurement.inputs` on `mean_time_to_first_operator_action.yaml`.
...
class is `telemetry.ocsf.incident_finding@v1`, matched on
`metadata.uid`.
