# Reference visualisation — `kpi.cra_final_report_on_time@v1`

This is the committed reference-visualisation artifact for the CRA
Article 14(2) 14-day final-report on-time-rate KPI. It exists so the
...
horizontal bar chart, one bar per CRA Article 14(2) notification
...
- **Drill-down x-axis:** `dispatch_margin_minutes` — minutes
  remaining at dispatch against the 14-day final-report clock. Positive
...
title: "kpi.cra_final_report_on_time@v1 — dispatch margin per CRA Article 14(2) notification (P30D window)"
---
xychart-beta horizontal
    title "minutes of slack against the 14-day final-report clock at dispatch"
    x-axis "notification (dispatched in window)" ["case-N1", "case-N2", "case-N3", "case-N4", "case-N5"]
    y-axis "dispatch_margin_minutes" -240 --> 21000
    bar [-120, 45, 360, 7200, 19800]
```
...
| case-N4 | 7200                    | yes      | mid-window dispatch, healthy slack                     |
| case-N5 | 19800                    | yes      | early dispatch, large slack                            |
...
The bands match the `thresholds` array on `cra_final_report_on_time.yaml`; the catalog
...
CRA Article 14(2) reporting pipeline. Each dispatched notification in
the evaluation window contributes one `dispatch_margin_minutes`
sample computed from the two inputs declared in `cra_final_report_on_time.yaml`'s
`measurement.inputs`:
...
- `awareness_timestamp` — the operator-awareness timestamp recorded
  ...
  of the 14-day final-report clock.
- `notification_dispatch` — the regulator-notification chain step
  transition that dispatches the 14-day final-report notification to the
...
`clock` is the 14-day final-report clock defined by CRA Article 14(2).
