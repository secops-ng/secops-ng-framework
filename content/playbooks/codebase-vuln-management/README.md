...
| `track-timeline`   | Emit one disclosure-timeline record per finding (see [`disclosure-timeline-record.schema.json`](../../evidence/codebase-vuln-management/disclosure-timeline-record.schema.json)) so the downstream metric streams pick it up. |
...
[`content/mappings/gdpr/data-flow-codebase-vuln-management.md`](../../mappings/gdpr/data-flow-codebase-vuln-management.md).
...
- **EXTEND-tests-goldens** — per-target byte-parity goldens under
  `tests/examples/{n8n,temporal,langgraph}/codebase-vuln-management/`
  plus a worked example under
  `examples/{n8n,temporal,langgraph}/codebase-vuln-management/`.
