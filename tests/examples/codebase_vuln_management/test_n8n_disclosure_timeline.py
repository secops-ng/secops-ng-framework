...
The committed
``examples/n8n/codebase-vuln-management/evidence/disclosure-timeline-record.json``
...
If the change is intentional, regenerate the example::
    ...
    PYTHONPATH=. python examples/n8n/codebase-vuln-management/regenerate.py
...
def test_example_snapshot_matches_n8n_adapter(tmp_path: Path) -> None:
    ...
    assert written.read_bytes() == SNAPSHOT.read_bytes(), (
        "examples/n8n/codebase-vuln-management/evidence/"
        "disclosure-timeline-record.json drifted from the n8n adapter. "
        "If intentional, regenerate via "
        "`PYTHONPATH=. python examples/n8n/codebase-vuln-management/"
