"""Byte-parity golden tests for the backup_recovery worked examples (G-03).

Pins the three reference-compiler outputs against the bytes that ship
under ``examples/{n8n,temporal,langgraph}/backup_recovery/``.
...
``examples/<target>/backup_recovery/README.md`` and commit the
...
N8N_EXAMPLE_DIR = (
    REPO_ROOT / "examples" / "n8n" / "backup_recovery"
)
TEMPORAL_EXAMPLE_DIR = (
    REPO_ROOT / "examples" / "temporal" / "backup_recovery"
)
LANGGRAPH_EXAMPLE_DIR = (
    REPO_ROOT / "examples" / "langgraph" / "backup_recovery"
)
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "backup_recovery n8n example drifted. Regenerate via "
...
def test_temporal_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "backup_recovery Temporal example drifted. Regenerate via "
        "`PYTHONPATH=. python -m tools.compile "
...
def test_langgraph_graph_spec_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "backup_recovery LangGraph example drifted. Regenerate via "
