"""Drift guard for the ``examples/n8n/phishing_triage/`` worked example.

Mirrors the vuln_intake and threat_intel_ingest n8n example tests:
parses the canonical CACAO playbook, emits the n8n workflow JSON, and
pins the result byte-for-byte against the committed
``examples/n8n/phishing_triage/workflow.n8n.json``. Adds a node-id ↔
CACAO action-id parity check so the one-to-one mirroring contract
documented in ``examples/n8n/phishing_triage/README.md`` is enforced
...
WORKED_EXAMPLE = REPO_ROOT / "examples" / "n8n" / "phishing_triage" / "workflow.n8n.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        ...
        "`./examples/n8n/phishing_triage/regenerate.sh` and commit the new bytes."
    )
...
def test_co_located_cacao_mirror_matches_canonical() -> None:
    ...
    assert mirror.exists(), (
        "examples/n8n/phishing_triage/playbook.cacao.json missing — "
        "run ./examples/n8n/phishing_triage/regenerate.sh"
    )
    assert mirror.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/phishing_triage/playbook.cacao.json drifted from the "
        "canonical CACAO source. Re-run "
        "./examples/n8n/phishing_triage/regenerate.sh and commit."
    )
