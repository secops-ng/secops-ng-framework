"""Golden tests for the identity_compromise worked example.

Pins the three reference-compiler outputs against the bytes that ship
under ``examples/{n8n,temporal,langgraph}/identity_compromise/``.
...
commands documented in each ``examples/<target>/identity_compromise/README.md``
...
N8N_GOLDEN = (
    REPO_ROOT / "examples" / "n8n" / "identity_compromise" / "workflow.n8n.json"
)
TEMPORAL_GOLDEN = (
    REPO_ROOT
    / "examples"
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "identity_compromise n8n example drifted. Regenerate via "
...
def test_temporal_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "identity_compromise Temporal example drifted. Regenerate via "
...
def test_langgraph_graph_spec_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "identity_compromise LangGraph example drifted. Regenerate via "
