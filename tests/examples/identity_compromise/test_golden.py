"""Golden tests for the identity-compromise worked example.

Pins the three reference-compiler outputs against the bytes that ship
under ``examples/{n8n,temporal,langgraph}/identity-compromise/``.
...
commands documented in each ``examples/<target>/identity-compromise/README.md``
...
N8N_GOLDEN = (
    REPO_ROOT / "examples" / "n8n" / "identity-compromise" / "workflow.n8n.json"
)
TEMPORAL_GOLDEN = (
    REPO_ROOT
    / "examples"
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "identity-compromise n8n example drifted. Regenerate via "
...
def test_temporal_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "identity-compromise Temporal example drifted. Regenerate via "
...
def test_langgraph_graph_spec_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "identity-compromise LangGraph example drifted. Regenerate via "
