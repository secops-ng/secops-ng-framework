"""F-SV-04 CORE (langgraph) — byte-parity replay golden.

Pins the committed sovereignty worked example for the langgraph target
under ``examples/langgraph/infra_posture_management/evidence/sovereignty/``
against a fresh re-emission driven through the langgraph adapter.
...
2. **Byte-parity with the committed example.** If the shared emitter or
   the langgraph adapter intentionally changes serialisation, regenerate
   via ``PYTHONPATH=. python examples/langgraph/infra_posture_management/evidence/sovereignty/regenerate.py``
...
EXAMPLE_DIR = (
    REPO_ROOT / "examples" / "langgraph" / "infra_posture_management"
...
def test_replay_matches_committed_snapshot(tmp_path: Path) -> None:
    update = emit_sovereignty_artifact_node(
        {"sovereignty_context": CTX, "evidence_output_dir": tmp_path}
    )
