"""F-G03-PARITY EXTEND — drift guard for examples/langgraph/infra_posture_management/.
...
infra_posture_management CACAO playbook and pins the committed
...
drift guards for the infra_posture_management playbook.

This closes the LangGraph end of the cross-target parity ring (G-03)
for the ``infra_posture_management`` playbook.
...
EXAMPLE_DIR = REPO_ROOT / "examples" / "langgraph" / "infra_posture_management"
...
def test_graph_spec_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/langgraph/infra_posture_management/graph_spec.json drift. "
        "Regenerate via "
        "`./examples/langgraph/infra_posture_management/regenerate.sh` "
...
def test_state_bindings_matches_state_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/langgraph/infra_posture_management/state_bindings.py drift. "
        "Regenerate via "
        "`./examples/langgraph/infra_posture_management/regenerate.sh` "
        "and commit the result."
    )
