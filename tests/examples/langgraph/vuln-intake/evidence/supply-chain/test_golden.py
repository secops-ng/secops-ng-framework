...
under ``examples/langgraph/vuln-intake/evidence/supply-chain/`` against
...
``examples/langgraph/vuln-intake/evidence/supply-chain/regenerate.py``.
...
EXAMPLE_DIR = REPO / "examples" / "langgraph" / "vuln-intake" / "evidence" / "supply-chain"
GOLDEN = EXAMPLE_DIR / "dependencies-snapshot.json"


# Mirrors CTX in examples/langgraph/vuln-intake/evidence/supply-chain/regenerate.py.
...
def _drift_hint() -> str:
    return (
        ...
        "via `PYTHONPATH=. python examples/langgraph/vuln-intake/"
