...
target under ``examples/langgraph/vuln-intake/evidence/crypto/`` against
...
``examples/langgraph/vuln-intake/evidence/crypto/regenerate.py``. The
...
EXAMPLE_DIR = REPO / "examples" / "langgraph" / "vuln-intake" / "evidence" / "crypto"
GOLDEN = EXAMPLE_DIR / "secret-handling-attestation.json"


# Mirrors CTX in examples/langgraph/vuln-intake/evidence/crypto/regenerate.py.
...
def _drift_hint() -> str:
    return (
        ...
        "example via `PYTHONPATH=. python examples/langgraph/vuln-intake/"
