...
The committed
``examples/langgraph/detection_engineering/evidence/rule-effectiveness-snapshot.json``
...
EXAMPLE = REPO / "examples" / "langgraph" / "detection_engineering"
...
def test_example_snapshot_matches_langgraph_adapter(tmp_path: Path) -> None:
    ...
    assert written.read_bytes() == SNAPSHOT.read_bytes(), (
        "examples/langgraph/detection_engineering/evidence/"
        "rule-effectiveness-snapshot.json drifted from the LangGraph "
        "adapter. If intentional, regenerate via "
        "`PYTHONPATH=. python examples/langgraph/detection_engineering/"
