"""F-G03-PARITY CORE — drift guard for examples/langgraph/contractual_obligations_tracker/.

Mirrors the asset_management LangGraph example test: re-runs
``compilers.langgraph.emit`` and ``compilers.langgraph.state`` against
the canonical contractual_obligations_tracker CACAO playbook and pins
...
drift guards for the contractual_obligations_tracker playbook.

This worked example closes the LangGraph end of the cross-target
parity ring (G-03) for the ``contractual_obligations_tracker``
playbook, alongside the n8n worked example under
``examples/n8n/contractual_obligations_tracker/`` and Temporal worked
example under ``examples/temporal/contractual_obligations_tracker/``.

Note: ``examples/langgraph/contractual_obligations_tracker/`` does not
...
EXAMPLE_DIR = REPO_ROOT / "examples" / "langgraph" / "contractual_obligations_tracker"
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/langgraph/contractual_obligations_tracker/playbook.cacao.json drifted "
        "from the canonical content/playbooks/contractual_obligations_tracker/playbook.cacao.json. "
        "Regenerate via `./examples/langgraph/contractual_obligations_tracker/regenerate.sh`."
    )
...
def test_graph_spec_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/langgraph/contractual_obligations_tracker/graph_spec.json drift. "
        "Regenerate via `./examples/langgraph/contractual_obligations_tracker/regenerate.sh` "
...
def test_state_bindings_matches_state_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/langgraph/contractual_obligations_tracker/state_bindings.py drift. "
        "Regenerate via `./examples/langgraph/contractual_obligations_tracker/regenerate.sh` "
        "and commit the result."
    )
