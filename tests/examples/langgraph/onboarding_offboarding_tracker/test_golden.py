"""F-G03-PARITY NEXT-BATCH-2 — drift guard for examples/langgraph/onboarding_offboarding_tracker/.
...
the canonical onboarding_offboarding_tracker CACAO playbook and pins
...
drift guards for the onboarding_offboarding_tracker playbook.

This worked example closes the LangGraph end of the cross-target
parity ring (G-03) for the ``onboarding_offboarding_tracker``
playbook, alongside the n8n worked example under
``examples/n8n/onboarding_offboarding_tracker/`` and Temporal worked
example under ``examples/temporal/onboarding_offboarding_tracker/``.

Note: ``examples/langgraph/onboarding_offboarding_tracker/`` does not
...
EXAMPLE_DIR = REPO_ROOT / "examples" / "langgraph" / "onboarding_offboarding_tracker"
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/langgraph/onboarding_offboarding_tracker/playbook.cacao.json drifted "
        "from the canonical content/playbooks/onboarding_offboarding_tracker/playbook.cacao.json. "
        "Regenerate via `./examples/langgraph/onboarding_offboarding_tracker/regenerate.sh`."
    )
...
def test_graph_spec_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/langgraph/onboarding_offboarding_tracker/graph_spec.json drift. "
        "Regenerate via `./examples/langgraph/onboarding_offboarding_tracker/regenerate.sh` "
...
def test_state_bindings_matches_state_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/langgraph/onboarding_offboarding_tracker/state_bindings.py drift. "
        "Regenerate via `./examples/langgraph/onboarding_offboarding_tracker/regenerate.sh` "
        "and commit the result."
    )
