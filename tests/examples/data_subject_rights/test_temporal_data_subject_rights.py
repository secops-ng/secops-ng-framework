"""Byte-parity golden for the Temporal data_subject_rights workflow (G-03).

The committed ``examples/temporal/data_subject_rights/workflow.temporal.py`` is the
Temporal compiler's output for the canonical CACAO playbook at
``content/playbooks/data_subject_rights/playbook.cacao.yaml``. Activity-name <-> CACAO
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRROR.read_bytes() == _canonical_mirror_bytes(), (
        "examples/temporal/data_subject_rights/playbook.cacao.json drifted from the "
        "canonical content/playbooks/data_subject_rights/playbook.cacao.yaml. Regenerate via "
        "`./examples/temporal/data_subject_rights/regenerate.sh`."
    )
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/data_subject_rights/workflow.temporal.py drifted from the "
