"""Byte-parity golden for the n8n data_subject_rights workflow (G-03).

The committed ``examples/n8n/data_subject_rights/workflow.n8n.json`` is the n8n
compiler's output for the canonical CACAO playbook at
``content/playbooks/data_subject_rights/playbook.cacao.yaml``. This module pins the
...
CANON_YAML = REPO / "content" / "playbooks" / "data_subject_rights" / "playbook.cacao.yaml"
EXAMPLE = REPO / "examples" / "n8n" / "data_subject_rights"
...
def test_mirrored_playbook_matches_canonical() -> None:
    assert MIRROR.read_bytes() == _canonical_mirror_bytes(), (
        "examples/n8n/data_subject_rights/playbook.cacao.json drifted from the "
        "canonical content/playbooks/data_subject_rights/playbook.cacao.yaml. "
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "data_subject_rights n8n workflow drifted. Regenerate via "
        "`./examples/n8n/data_subject_rights/regenerate.sh` and commit the new "
