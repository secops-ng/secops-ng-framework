"""Byte-parity golden for the Temporal iam_auditor workflow (G-03).

The committed ``examples/temporal/iam_auditor/workflow.temporal.py`` is the
Temporal compiler's output for the canonical CACAO playbook at
``content/playbooks/iam_auditor/playbook.cacao.json``. Activity-name <-> CACAO
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRROR.read_bytes() == CANON.read_bytes(), (
        "examples/temporal/iam_auditor/playbook.cacao.json drifted from the "
        "canonical content/playbooks/iam_auditor/playbook.cacao.json. Regenerate via "
        "`./examples/temporal/iam_auditor/regenerate.sh`."
    )
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/iam_auditor/workflow.temporal.py drifted from the "
