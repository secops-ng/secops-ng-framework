"""Byte-parity golden for the n8n iam_auditor workflow (G-03).

The committed ``examples/n8n/iam_auditor/workflow.n8n.json`` is the n8n
compiler's output for the canonical CACAO playbook at
``content/playbooks/iam_auditor/playbook.cacao.json``. This module pins the
...
CANON = REPO / "content" / "playbooks" / "iam_auditor" / "playbook.cacao.json"
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRROR.read_bytes() == CANON.read_bytes(), (
        "examples/n8n/iam_auditor/playbook.cacao.json drifted from the "
        "canonical content/playbooks/iam_auditor/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/iam_auditor/regenerate.sh`."
    )
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "iam_auditor n8n workflow drifted. Regenerate via "
