"""Byte-parity golden for the LangGraph iam_auditor example (G-03).

Re-runs ``compilers.langgraph.emit`` and ``compilers.langgraph.state``
against the canonical iam_auditor CACAO playbook and pins the committed
...
EXAMPLE_DIR = REPO_ROOT / "examples" / "langgraph" / "iam_auditor"
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == CANON.read_bytes(), (
        "examples/langgraph/iam_auditor/playbook.cacao.json drifted from the "
...
def test_graph_spec_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/langgraph/iam_auditor/graph_spec.json drift. Regenerate via "
        "`./examples/langgraph/iam_auditor/regenerate.sh`."
    )
