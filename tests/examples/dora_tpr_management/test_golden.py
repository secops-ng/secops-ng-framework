"""Golden tests for the dora_tpr_management worked example.

Pins the three reference-compiler outputs against the bytes that ship
under ``examples/{n8n,temporal,langgraph}/dora_tpr_management/``.
...
commands documented in each ``examples/<target>/dora_tpr_management/README.md``
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "dora_tpr_management n8n example drifted. Regenerate via "
...
def test_temporal_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "dora_tpr_management Temporal example drifted. Regenerate via "
...
def test_langgraph_graph_spec_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "dora_tpr_management LangGraph example drifted. Regenerate via "
