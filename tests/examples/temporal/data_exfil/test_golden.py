"""F-G03-PARITY EXTEND — Temporal worked example for data_exfil.

Pins the Temporal end of the three-target contract for the ``data_exfil``
playbook. The committed ``examples/temporal/data_exfil/workflow.temporal.py``
is the Temporal compiler's output for the canonical CACAO playbook at
``content/playbooks/data_exfil/playbook.cacao.json``.
...
If a change is intentional, regenerate the example::
    ...
    ./examples/temporal/data_exfil/regenerate.sh
...
EXAMPLE = REPO / "examples" / "temporal" / "data_exfil"
SOURCE = REPO / "content" / "playbooks" / "data_exfil" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/data_exfil/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/data_exfil/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/data_exfil/playbook.cacao.json drifted "
        "from the canonical content/playbooks/data_exfil/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/data_exfil/regenerate.sh`."
    )
