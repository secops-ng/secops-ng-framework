"""F-G03-PARITY EXTEND — Temporal worked example for executive_metrics.

Pins the Temporal end of the three-target contract for the ``executive_metrics``
playbook. The committed ``examples/temporal/executive_metrics/workflow.temporal.py``
...
EXAMPLE = REPO / "examples" / "temporal" / "executive_metrics"
SOURCE = REPO / "content" / "playbooks" / "executive_metrics" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/executive_metrics/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/executive_metrics/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/executive_metrics/playbook.cacao.json drifted "
        "from the canonical content/playbooks/executive_metrics/"
