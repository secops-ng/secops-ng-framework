"""F-WF-NIS2-SELF-ASSESS CORE — Temporal worked example for nis2_self_assessment.

Opens the cross-target parity lane for the ``nis2_self_assessment``
playbook (NIS2 Art. 21(2) whole-Article self-assessment roll-up) by pinning the Temporal end of the
three-target contract. The committed
``examples/temporal/nis2_self_assessment/workflow.temporal.py`` is the
...
EXAMPLE = REPO / "examples" / "temporal" / "nis2_self_assessment"
SOURCE = REPO / "content" / "playbooks" / "nis2_self_assessment" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/nis2_self_assessment/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/nis2_self_assessment/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/nis2_self_assessment/playbook.cacao.json drifted "
        "from the canonical content/playbooks/nis2_self_assessment/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/nis2_self_assessment/regenerate.sh`."
    )
