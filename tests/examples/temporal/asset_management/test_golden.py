"""F-G03-PARITY EXTEND — Temporal worked example for asset_management.

Pins the Temporal end of the three-target contract for the
``asset_management`` playbook. The committed
``examples/temporal/asset_management/workflow.temporal.py`` is the
...
If a change is intentional, regenerate the example::
    ...
    ./examples/temporal/asset_management/regenerate.sh
...
SOURCE = REPO / "content" / "playbooks" / "asset_management" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/asset_management/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/asset_management/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/asset_management/playbook.cacao.json drifted "
        "from the canonical content/playbooks/asset_management/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/asset_management/regenerate.sh`."
    )
