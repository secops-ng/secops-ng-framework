"""F-G03-PARITY NEXT-BATCH-2 — Temporal worked example for onboarding_offboarding_tracker.

Pins the Temporal end of the three-target contract for the
``onboarding_offboarding_tracker`` playbook. The committed
``examples/temporal/onboarding_offboarding_tracker/workflow.temporal.py`` is
...
SOURCE = REPO / "content" / "playbooks" / "onboarding_offboarding_tracker" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/onboarding_offboarding_tracker/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/onboarding_offboarding_tracker/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/onboarding_offboarding_tracker/playbook.cacao.json drifted "
        "from the canonical content/playbooks/onboarding_offboarding_tracker/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/onboarding_offboarding_tracker/regenerate.sh`."
    )
