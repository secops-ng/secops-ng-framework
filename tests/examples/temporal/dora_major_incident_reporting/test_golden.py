"""F-G03-PARITY EXTEND — Temporal worked example for dora_major_incident_reporting.

Pins the Temporal end of the three-target contract for the ``dora_major_incident_reporting``
playbook. The committed ``examples/temporal/dora_major_incident_reporting/workflow.temporal.py``
...
EXAMPLE = REPO / "examples" / "temporal" / "dora_major_incident_reporting"
SOURCE = REPO / "content" / "playbooks" / "dora_major_incident_reporting" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/dora_major_incident_reporting/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/dora_major_incident_reporting/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/dora_major_incident_reporting/playbook.cacao.json drifted "
