"""F-G03-PARITY EXTEND — Temporal worked example for eu_ai_act_risk_management.

Pins the Temporal end of the three-target contract for the ``eu_ai_act_risk_management``
playbook. The committed ``examples/temporal/eu_ai_act_risk_management/workflow.temporal.py``
...
EXAMPLE = REPO / "examples" / "temporal" / "eu_ai_act_risk_management"
SOURCE = REPO / "content" / "playbooks" / "eu_ai_act_risk_management" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/eu_ai_act_risk_management/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/eu_ai_act_risk_management/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/eu_ai_act_risk_management/playbook.cacao.json drifted "
        "from the canonical content/playbooks/eu_ai_act_risk_management/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/eu_ai_act_risk_management/regenerate.sh`."
    )
