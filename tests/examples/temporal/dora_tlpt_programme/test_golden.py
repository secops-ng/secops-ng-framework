"""F-WF-DORA-TLPT CORE-EXAMPLE — Temporal worked example for dora_tlpt_programme.

Pins the Temporal end of the three-target parity contract for the
``dora_tlpt_programme`` playbook (DORA Art. 24 / 26, the operator-side
DORA Chapter IV digital operational resilience testing (DORT)
programme lifecycle). The committed
``examples/temporal/dora_tlpt_programme/workflow.temporal.py`` is the
...
EXAMPLE = REPO / "examples" / "temporal" / "dora_tlpt_programme"
SOURCE = REPO / "content" / "playbooks" / "dora_tlpt_programme" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/dora_tlpt_programme/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/dora_tlpt_programme/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/dora_tlpt_programme/playbook.cacao.json drifted "
        "from the canonical content/playbooks/dora_tlpt_programme/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/dora_tlpt_programme/regenerate.sh`."
    )
