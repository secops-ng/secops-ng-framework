"""F-WF-CRYPTO SKELETON-EXAMPLE — Temporal worked example for crypto_posture_management.

Opens the cross-target parity lane for the ``crypto_posture_management``
playbook (NIS2 Art.21(2)(h)) by pinning the Temporal end of the
three-target contract. The committed
``examples/temporal/crypto_posture_management/workflow.temporal.py`` is
...
If a change is intentional, regenerate the example::
    ...
    ./examples/temporal/crypto_posture_management/regenerate.sh
...
EXAMPLE = REPO / "examples" / "temporal" / "crypto_posture_management"
SOURCE = REPO / "content" / "playbooks" / "crypto_posture_management" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/crypto_posture_management/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/crypto_posture_management/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/crypto_posture_management/playbook.cacao.json drifted "
        "from the canonical content/playbooks/crypto_posture_management/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/crypto_posture_management/regenerate.sh`."
    )
