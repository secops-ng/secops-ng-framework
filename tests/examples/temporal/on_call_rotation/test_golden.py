"""F-G03-PARITY EXTEND — Temporal worked example for on_call_rotation.

Pins the Temporal end of the three-target contract for the ``on_call_rotation``
playbook. The committed ``examples/temporal/on_call_rotation/workflow.temporal.py``
is the Temporal compiler's output for the canonical CACAO playbook at
``content/playbooks/on_call_rotation/playbook.cacao.json``.
...
If a change is intentional, regenerate the example::
    ...
    ./examples/temporal/on_call_rotation/regenerate.sh
...
EXAMPLE = REPO / "examples" / "temporal" / "on_call_rotation"
SOURCE = REPO / "content" / "playbooks" / "on_call_rotation" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/on_call_rotation/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/on_call_rotation/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/on_call_rotation/playbook.cacao.json drifted "
        "from the canonical content/playbooks/on_call_rotation/"
