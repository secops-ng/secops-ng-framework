"""F-WF-PATCH SKELETON-EXAMPLE — Temporal worked example for patch_management.

Pins the Temporal end of the three-target contract for the
``patch_management`` playbook (NIS2 Art.21(2)(e)). The committed
``examples/temporal/patch_management/workflow.temporal.py`` is the
...
EXAMPLE = REPO / "examples" / "temporal" / "patch_management"
SOURCE = REPO / "content" / "playbooks" / "patch_management" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/patch_management/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/patch_management/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/patch_management/playbook.cacao.json drifted "
        "from the canonical content/playbooks/patch_management/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/patch_management/regenerate.sh`."
    )
