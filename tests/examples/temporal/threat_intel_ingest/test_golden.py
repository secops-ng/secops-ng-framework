"""F-G03-PARITY EXTEND — Temporal worked example for threat_intel_ingest.

Pins the Temporal end of the three-target contract for the ``threat_intel_ingest``
playbook. The committed ``examples/temporal/threat_intel_ingest/workflow.temporal.py``
...
EXAMPLE = REPO / "examples" / "temporal" / "threat_intel_ingest"
SOURCE = REPO / "content" / "playbooks" / "threat_intel_ingest" / "playbook.cacao.json"
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        ...
        "from the canonical content/playbooks/threat_intel_ingest/"
