"""F-WF-DSR CORE — Temporal worked example for data_subject_rights.

Pins the Temporal end of the three-target contract for the
``data_subject_rights`` playbook (GDPR Art. 15-22). The committed
``examples/temporal/data_subject_rights/workflow.temporal.py`` is the
...
action step gets exactly one ``@activity.defn`` whose docstring
records the originating ``step_id``.
...
EXAMPLE = REPO / "examples" / "temporal" / "data_subject_rights"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/data_subject_rights/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/data_subject_rights/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == _canonical_mirror_bytes(), (
        "examples/temporal/data_subject_rights/playbook.cacao.json drifted "
        "from the canonical "
        "content/playbooks/data_subject_rights/playbook.cacao.yaml. "
        "Regenerate via `./examples/temporal/data_subject_rights/regenerate.sh`."
    )
