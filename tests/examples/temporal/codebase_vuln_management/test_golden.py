"""F-G03-PARITY CORE — Temporal worked example for codebase_vuln_management.

Pins the Temporal end of the three-target contract for the
``codebase_vuln_management`` playbook. The committed
``examples/temporal/codebase_vuln_management/workflow.temporal.py`` is
...
SOURCE = REPO / "content" / "playbooks" / "codebase_vuln_management" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/codebase_vuln_management/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/codebase_vuln_management/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/codebase_vuln_management/playbook.cacao.json drifted "
        "from the canonical content/playbooks/codebase_vuln_management/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/codebase_vuln_management/regenerate.sh`."
    )
