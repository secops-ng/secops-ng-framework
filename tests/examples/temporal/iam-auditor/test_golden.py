"""F-WF-08 SKELETON-FANOUT-TMP — byte-parity goldens for the temporal iam-auditor example.

The committed
``examples/temporal/iam-auditor/workflow.temporal.py`` and
``examples/temporal/iam-auditor/evidence/access-evidence.json``
...
EXAMPLE = REPO / "examples" / "temporal" / "iam-auditor"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/iam-auditor/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/iam-auditor/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/iam-auditor/playbook.cacao.json drifted "
        "from the canonical content/playbooks/iam-auditor/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/iam-auditor/regenerate.sh`."
    )
...
def test_example_access_evidence_matches_temporal_adapter(tmp_path: Path) -> None:
    ...
    assert written.read_bytes() == ACCESS_GOLDEN.read_bytes(), (
        "examples/temporal/iam-auditor/evidence/access-evidence.json drifted "
