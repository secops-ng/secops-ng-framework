"""F-WF-MFA SKELETON-EXAMPLE — Temporal worked example for mfa_secured_comms.

Opens the cross-target parity lane for the ``mfa_secured_comms``
playbook (NIS2 Art.21(2)(j)) by pinning the Temporal end of the
three-target contract. The committed
``examples/temporal/mfa_secured_comms/workflow.temporal.py`` is the
...
EXAMPLE = REPO / "examples" / "temporal" / "mfa_secured_comms"
SOURCE = REPO / "content" / "playbooks" / "mfa_secured_comms" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/mfa_secured_comms/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/mfa_secured_comms/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/mfa_secured_comms/playbook.cacao.json drifted "
        "from the canonical content/playbooks/mfa_secured_comms/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/mfa_secured_comms/regenerate.sh`."
    )
