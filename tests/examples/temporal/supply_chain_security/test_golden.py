"""F-G03-PARITY NEXT-BATCH-2 — Temporal worked example for supply_chain_security.

Pins the Temporal end of the three-target contract for the
``supply_chain_security`` playbook. The committed
...
SOURCE = REPO / "content" / "playbooks" / "supply_chain_security" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/supply_chain_security/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/supply_chain_security/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/supply_chain_security/playbook.cacao.json drifted "
        "from the canonical content/playbooks/supply_chain_security/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/supply_chain_security/regenerate.sh`."
    )
