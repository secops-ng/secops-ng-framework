"""F-SV-04 CORE (temporal) — byte-parity replay golden.

Pins the committed sovereignty worked example for the temporal target
under ``examples/temporal/infra_posture_management/evidence/sovereignty/``
against a fresh re-emission driven through the temporal adapter.
...
2. **Byte-parity with the committed example.** If the shared emitter or
   the temporal adapter intentionally changes serialisation, regenerate
   via ``PYTHONPATH=. python examples/temporal/infra_posture_management/evidence/sovereignty/regenerate.py``
...
from compilers.temporal.evidence import emit_sovereignty_artifact_activity
...
CTX = build_context(
    execution_id="temporal:infra_posture_sovereignty_example_0001",
    compile_target="temporal",
    source_url="https://temporal.example.invalid/namespaces/secops/workflows/infra_posture_management/runs/0001",
)
...
def test_replay_matches_committed_snapshot(tmp_path: Path) -> None:
    written = Path(
        asyncio.run(emit_sovereignty_artifact_activity(CTX, tmp_path))
    )
