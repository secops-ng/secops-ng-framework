"""F-G03-PARITY EXTEND — Temporal worked example for infra_posture_management.

Pins the Temporal end of the three-target contract for the ``infra_posture_management``
playbook. The committed ``examples/temporal/infra_posture_management/workflow.temporal.py``
...
SOURCE = REPO / "content" / "playbooks" / "infra_posture_management" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/infra_posture_management/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/infra_posture_management/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/infra_posture_management/playbook.cacao.json drifted "
