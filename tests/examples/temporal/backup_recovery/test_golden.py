"""F-WF-BCR SKELETON-EXAMPLE — Temporal worked example for backup_recovery.

Opens the cross-target parity lane for the ``backup_recovery`` playbook
(NIS2 Art.21(2)(c), DORA Art.12) by pinning the Temporal end of the
three-target contract. The committed
``examples/temporal/backup_recovery/workflow.temporal.py`` is the
...
EXAMPLE = REPO / "examples" / "temporal" / "backup_recovery"
SOURCE = REPO / "content" / "playbooks" / "backup_recovery" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/backup_recovery/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/backup_recovery/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/backup_recovery/playbook.cacao.json drifted "
        "from the canonical content/playbooks/backup_recovery/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/backup_recovery/regenerate.sh`."
    )
