"""F-NIS2-BCP CORE — Temporal worked example for business_continuity.

Pins the Temporal end of the three-target contract for the
``business_continuity`` playbook (NIS2 Art. 21(2)(c), Art. 23). The
committed ``examples/temporal/business_continuity/workflow.temporal.py``
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/business_continuity/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/business_continuity/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == _canonical_mirror_bytes(), (
        "examples/temporal/business_continuity/playbook.cacao.json drifted "
        "from the canonical "
        "content/playbooks/business_continuity/playbook.cacao.yaml. "
        "Regenerate via `./examples/temporal/business_continuity/regenerate.sh`."
    )
