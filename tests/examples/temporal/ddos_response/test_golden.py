"""F-WF-DOS SKELETON-EXAMPLE — Temporal worked example for ddos_response.

Opens the cross-target parity lane for the ``ddos_response`` playbook
(NIS2 Art.21(2)(b)) by pinning the Temporal end of the three-target
contract. The committed
``examples/temporal/ddos_response/workflow.temporal.py`` is the
...
EXAMPLE = REPO / "examples" / "temporal" / "ddos_response"
SOURCE = REPO / "content" / "playbooks" / "ddos_response" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/ddos_response/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/ddos_response/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/ddos_response/playbook.cacao.json drifted "
        "from the canonical content/playbooks/ddos_response/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/ddos_response/regenerate.sh`."
    )
