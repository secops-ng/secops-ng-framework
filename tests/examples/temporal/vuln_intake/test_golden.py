"""F-G03-PARITY EXTEND — Temporal worked example for vuln_intake.

Pins the Temporal end of the three-target contract for the ``vuln_intake``
playbook. The committed ``examples/temporal/vuln_intake/workflow.temporal.py``
...
EXAMPLE = REPO / "examples" / "temporal" / "vuln_intake"
SOURCE = REPO / "content" / "playbooks" / "vuln_intake" / "playbook.cacao.json"
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        ...
        "from the canonical content/playbooks/vuln_intake/"
