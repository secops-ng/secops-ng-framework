"""F-CP-06 EXTEND-tests-goldens (Temporal) — byte-parity replay golden.

Pins the committed effectiveness worked example for the Temporal
target under ``examples/temporal/vuln_intake/evidence/effectiveness/``
against a fresh re-emission driven through the Temporal activity
adapter at
:func:`compilers.temporal.evidence.emit_effectiveness_artifact_activity`.
...
file the shared emitter writes. This test re-runs the activity
adapter the way a Temporal worker would (typed
``EffectivenessContext`` in, absolute artifact path out via
``asyncio.run``), schema-validates the result against
...
``examples/temporal/vuln_intake/evidence/effectiveness/regenerate.py``.
...
CTX = EffectivenessContext(
    ...
    execution_id="temporal:vuln_intake_effectiveness_example_0001",
...
def _replay(output_dir: Path) -> Path:
    ...
    written_str = asyncio.run(
        emit_effectiveness_artifact_activity(CTX, output_dir)
    )
...
def _drift_hint() -> str:
    return (
        "Temporal effectiveness example drifted from a fresh adapter "
