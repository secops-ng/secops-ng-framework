"""F-G03-PARITY CORE — Temporal worked example for contractual_obligations_tracker.

Pins the Temporal end of the three-target contract for the
``contractual_obligations_tracker`` playbook. The committed
...
SOURCE = REPO / "content" / "playbooks" / "contractual_obligations_tracker" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/contractual_obligations_tracker/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/contractual_obligations_tracker/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/contractual_obligations_tracker/playbook.cacao.json drifted "
        "from the canonical content/playbooks/contractual_obligations_tracker/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/contractual_obligations_tracker/regenerate.sh`."
    )
