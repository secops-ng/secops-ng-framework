...
``examples/temporal/cyber_hygiene_training/workflow.temporal.py`` is the
...
EXAMPLE = REPO / "examples" / "temporal" / "cyber_hygiene_training"
SOURCE = REPO / "content" / "playbooks" / "cyber_hygiene_training" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/cyber_hygiene_training/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/cyber_hygiene_training/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/cyber_hygiene_training/playbook.cacao.json drifted "
        "from the canonical content/playbooks/cyber_hygiene_training/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/cyber_hygiene_training/regenerate.sh`."
    )
