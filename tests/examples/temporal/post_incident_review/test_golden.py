"""F-G03-PARITY EXTEND — Temporal worked example for post_incident_review.

Pins the Temporal end of the three-target contract for the ``post_incident_review``
playbook. The committed ``examples/temporal/post_incident_review/workflow.temporal.py``
is the Temporal compiler's output for the canonical CACAO playbook at
``content/playbooks/post_incident_review/playbook.cacao.json``.
...
EXAMPLE = REPO / "examples" / "temporal" / "post_incident_review"
SOURCE = REPO / "content" / "playbooks" / "post_incident_review" / "playbook.cacao.json"
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        ...
        "from the canonical content/playbooks/post_incident_review/"
