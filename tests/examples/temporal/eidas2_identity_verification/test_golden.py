"""F-G03-PARITY SKELETON — Temporal worked example for eidas2_identity_verification.

Pins the Temporal end of the three-target contract for the
``eidas2_identity_verification`` playbook. The committed
...
EXAMPLE = REPO / "examples" / "temporal" / "eidas2_identity_verification"
SOURCE = REPO / "content" / "playbooks" / "eidas2_identity_verification" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/eidas2_identity_verification/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/eidas2_identity_verification/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/eidas2_identity_verification/playbook.cacao.json drifted "
        "from the canonical content/playbooks/eidas2_identity_verification/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/eidas2_identity_verification/regenerate.sh`."
    )
