"""F-CP-03 EXTEND-tests-goldens (Temporal) — byte-parity replay golden.

Pins the committed supply-chain worked example for the Temporal target
under ``examples/temporal/vuln-intake/evidence/supply-chain/`` against
a fresh re-emission driven through the Temporal activity adapter at
:func:`compilers.temporal.evidence.emit_supply_chain_artifact_activity`.
...
way a Temporal worker would (typed ``SupplyChainContext`` in, absolute
artifact path out via ``asyncio.run``), schema-validates the result
against ``schemas/evidence/supply-chain.schema.json`` (with promoted
sibling vocabularies resolved), and asserts byte-equality with the
committed snapshot.
...
1. **Schema-conformant emit.** The re-emitted artifact validates
   against the supply-chain schema before the byte comparison runs.
2. **Byte-parity with the committed example.** The re-emitted
   artifact's on-disk bytes match the committed
   ``dependencies-snapshot.json`` exactly. Regenerate via
...
``examples/temporal/vuln-intake/evidence/supply-chain/regenerate.py``.
...
EXAMPLE_DIR = REPO / "examples" / "temporal" / "vuln-intake" / "evidence" / "supply-chain"
GOLDEN = EXAMPLE_DIR / "dependencies-snapshot.json"


# Mirrors CTX in examples/temporal/vuln-intake/evidence/supply-chain/regenerate.py.
...
def _replay(output_dir: Path) -> Path:
    ...
    written_str = asyncio.run(
        emit_supply_chain_artifact_activity(CTX, output_dir)
    )
...
def _drift_hint() -> str:
    return (
        "Temporal supply-chain example drifted from a fresh adapter "
        "replay. If the change is intentional, regenerate the example "
        "via `PYTHONPATH=. python examples/temporal/vuln-intake/evidence/"
...
def test_artifact_id_is_deterministic_sha256(tmp_path: Path) -> None:
    ...
    activity emission re-derives the same id, which the shared emitter
    encodes into the written artifact filename.
    """
