"""F-G03-PARITY SKELETON-EXAMPLE — Temporal worked example for security_awareness_training.

Pins the Temporal end of the three-target contract for the
``security_awareness_training`` playbook (NIS2 Art.20). The committed
``examples/temporal/security_awareness_training/workflow.temporal.py`` is the
...
SOURCE = REPO / "content" / "playbooks" / "security_awareness_training" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/security_awareness_training/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/security_awareness_training/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/security_awareness_training/playbook.cacao.json drifted "
        "from the canonical content/playbooks/security_awareness_training/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/security_awareness_training/regenerate.sh`."
    )
