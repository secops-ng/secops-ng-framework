"""F-G03-PARITY GOLDENS — Temporal worked example for nis2_art20_governance.

Pins the Temporal end of the three-target contract for the
``nis2_art20_governance`` playbook (NIS2 Art. 20 management-body
cyber-governance obligations). The committed
``examples/temporal/nis2_art20_governance/workflow.temporal.py`` is the
...
EXAMPLE = REPO / "examples" / "temporal" / "nis2_art20_governance"
SOURCE = REPO / "content" / "playbooks" / "nis2_art20_governance" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/nis2_art20_governance/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/nis2_art20_governance/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/nis2_art20_governance/playbook.cacao.json drifted "
        "from the canonical content/playbooks/nis2_art20_governance/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/nis2_art20_governance/regenerate.sh`."
    )
