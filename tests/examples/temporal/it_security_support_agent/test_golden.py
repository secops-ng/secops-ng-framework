"""F-G03-PARITY NEXT-BATCH — Temporal worked example for it_security_support_agent.

Pins the Temporal end of the three-target contract for the
``it_security_support_agent`` playbook. The committed
...
SOURCE = REPO / "content" / "playbooks" / "it_security_support_agent" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/it_security_support_agent/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/it_security_support_agent/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/it_security_support_agent/playbook.cacao.json drifted "
        "from the canonical content/playbooks/it_security_support_agent/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/it_security_support_agent/regenerate.sh`."
    )
