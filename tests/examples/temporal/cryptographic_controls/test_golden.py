"""F-WF-CRYPTOMGMT CORE-EXAMPLE — Temporal worked example for cryptographic_controls.

Pins the Temporal end of the three-target parity contract for the
``cryptographic_controls`` playbook (NIS2 Art. 21(2)(h), the write-side
lifecycle counterpart to ``crypto_posture_management``). The committed
...
EXAMPLE = REPO / "examples" / "temporal" / "cryptographic_controls"
SOURCE = REPO / "content" / "playbooks" / "cryptographic_controls" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/cryptographic_controls/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/cryptographic_controls/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/cryptographic_controls/playbook.cacao.json drifted "
        "from the canonical content/playbooks/cryptographic_controls/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/cryptographic_controls/regenerate.sh`."
    )
