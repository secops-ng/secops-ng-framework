"""F-WF-SOC2-EVIDENCE GOLDEN — Temporal worked example for soc2_evidence_collector.

Pins the Temporal end of the three-target contract for the
``soc2_evidence_collector`` playbook (AICPA TSC; FOUNDATION auditability). The committed
``examples/temporal/soc2_evidence_collector/workflow.temporal.py`` is the
Temporal compiler's output for the canonical CACAO playbook at
``content/playbooks/soc2_evidence_collector/playbook.cacao.json``.
...
EXAMPLE = REPO / "examples" / "temporal" / "soc2_evidence_collector"
SOURCE = REPO / "content" / "playbooks" / "soc2_evidence_collector" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/soc2_evidence_collector/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/soc2_evidence_collector/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/soc2_evidence_collector/playbook.cacao.json drifted "
        "from the canonical content/playbooks/soc2_evidence_collector/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/soc2_evidence_collector/regenerate.sh`."
    )
