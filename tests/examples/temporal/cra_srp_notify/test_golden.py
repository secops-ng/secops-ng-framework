"""F-G03-PARITY SKELETON — Temporal worked example for cra_srp_notify.

Pins the Temporal end of the three-target contract for the
``cra_srp_notify`` playbook. The committed
``examples/temporal/cra_srp_notify/workflow.temporal.py`` is
...
EXAMPLE = REPO / "examples" / "temporal" / "cra_srp_notify"
SOURCE = REPO / "content" / "playbooks" / "cra_srp_notify" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/cra_srp_notify/workflow.temporal.py drifted "
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/cra_srp_notify/playbook.cacao.json drifted "
        "from the canonical content/playbooks/cra_srp_notify/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/cra_srp_notify/regenerate.sh`."
    )
