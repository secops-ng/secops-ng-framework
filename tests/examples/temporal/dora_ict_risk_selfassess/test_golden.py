"""F-WF-DORA-SELFASSESS CORE — Temporal worked example for dora_ict_risk_selfassess.

Opens the cross-target parity lane for the ``dora_ict_risk_selfassess``
playbook (DORA Chapter II ICT risk management operator self-assessment
roll-up) by pinning the Temporal end of the three-target contract. The
committed
``examples/temporal/dora_ict_risk_selfassess/workflow.temporal.py`` is
...
EXAMPLE = REPO / "examples" / "temporal" / "dora_ict_risk_selfassess"
SOURCE = REPO / "content" / "playbooks" / "dora_ict_risk_selfassess" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/dora_ict_risk_selfassess/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/dora_ict_risk_selfassess/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/dora_ict_risk_selfassess/playbook.cacao.json drifted "
        "from the canonical content/playbooks/dora_ict_risk_selfassess/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/dora_ict_risk_selfassess/regenerate.sh`."
    )
