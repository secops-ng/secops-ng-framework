"""F-G03-PARITY EXTEND — Temporal worked example for agentic_threat_response.

Pins the Temporal end of the three-target contract for the
``agentic_threat_response`` playbook. The committed
``examples/temporal/agentic_threat_response/workflow.temporal.py`` is
...
If a change is intentional, regenerate the example::
    ...
    ./examples/temporal/agentic_threat_response/regenerate.sh
...
EXAMPLE = REPO / "examples" / "temporal" / "agentic_threat_response"
SOURCE = REPO / "content" / "playbooks" / "agentic_threat_response" / "playbook.cacao.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/agentic_threat_response/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/agentic_threat_response/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/agentic_threat_response/playbook.cacao.json drifted "
        "from the canonical content/playbooks/agentic_threat_response/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/agentic_threat_response/regenerate.sh`."
    )
