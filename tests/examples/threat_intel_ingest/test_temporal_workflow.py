"""Drift guard for the ``examples/temporal/threat-intel-ingest/`` worked example.
...
``examples/temporal/threat-intel-ingest/workflow.temporal.py``. Adds an
activity-name \u2194 CACAO action-id parity check so the one-to-one
mirroring contract documented in
``examples/temporal/threat-intel-ingest/README.md`` is enforced by
...
SOURCE = (
    REPO_ROOT / "content" / "playbooks" / "threat-intel-ingest" / "playbook.cacao.json"
)
EXAMPLE_DIR = REPO_ROOT / "examples" / "temporal" / "threat-intel-ingest"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/threat-intel-ingest/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/threat-intel-ingest/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/threat-intel-ingest/playbook.cacao.json drifted "
        "from the canonical content/playbooks/threat-intel-ingest/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/threat-intel-ingest/regenerate.sh`."
    )
