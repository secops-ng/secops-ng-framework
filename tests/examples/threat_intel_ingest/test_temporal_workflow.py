"""Drift guard for the ``examples/temporal/threat_intel_ingest/`` worked example.

Mirrors the data_exfil temporal example test: re-emits the Temporal
workflow stub from the canonical CACAO playbook and pins the result
byte-for-byte against the committed
``examples/temporal/threat_intel_ingest/workflow.temporal.py``. Adds an
activity-name \u2194 CACAO action-id parity check so the one-to-one
mirroring contract documented in
``examples/temporal/threat_intel_ingest/README.md`` is enforced by
...
SOURCE = (
    REPO_ROOT / "content" / "playbooks" / "threat_intel_ingest" / "playbook.cacao.json"
)
EXAMPLE_DIR = REPO_ROOT / "examples" / "temporal" / "threat_intel_ingest"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/threat_intel_ingest/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/threat_intel_ingest/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/threat_intel_ingest/playbook.cacao.json drifted "
        "from the canonical content/playbooks/threat_intel_ingest/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/threat_intel_ingest/regenerate.sh`."
    )
