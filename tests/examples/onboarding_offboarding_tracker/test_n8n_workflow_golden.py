"""F-WF-11 CORE-FANOUT-N8N — byte-parity golden for the n8n workflow.

The committed
``examples/n8n/onboarding_offboarding_tracker/workflow.n8n.json`` is
...
def test_mirrored_playbook_matches_canonical() -> None:
    ...
    assert MIRROR.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/onboarding_offboarding_tracker/playbook.cacao.json "
        "drifted from the canonical content/playbooks/"
        "onboarding_offboarding_tracker/playbook.cacao.json. Regenerate "
        "via `./examples/n8n/onboarding_offboarding_tracker/regenerate.sh`."
    )
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "onboarding_offboarding_tracker n8n workflow drifted. Regenerate "
        "via `./examples/n8n/onboarding_offboarding_tracker/regenerate.sh` "
