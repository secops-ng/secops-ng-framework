"""F-WF-11 CORE-FANOUT-TEMPORAL — byte-parity golden for the Temporal workflow.
...
def test_mirrored_playbook_matches_canonical() -> None:
    ...
    assert MIRROR.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/onboarding_offboarding_tracker/playbook.cacao.json "
        "drifted from the canonical content/playbooks/"
        "onboarding_offboarding_tracker/playbook.cacao.json. Regenerate "
        "via `./examples/temporal/onboarding_offboarding_tracker/regenerate.sh`."
    )
...
def test_temporal_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "onboarding_offboarding_tracker Temporal workflow drifted. "
        "Regenerate via "
        "`./examples/temporal/onboarding_offboarding_tracker/regenerate.sh` "
