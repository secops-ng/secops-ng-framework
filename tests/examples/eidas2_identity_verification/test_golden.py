"""Byte-parity golden tests for the eidas2_identity_verification worked examples (G-03).

Pins the three reference-compiler outputs against the bytes that ship
under ``examples/{n8n,temporal,langgraph}/eidas2_identity_verification/``.
...
regeneration of the canonical CACAO source (F-WF-EIDAS2-IDENTITY
playbook, NIS2 Art. 21(2)(i), DORA Art. 5, eIDAS 2.0), not a
hand-edited copy.

If an emitter change is intentional, regenerate the artifacts with the
commands documented in each
``examples/<target>/eidas2_identity_verification/README.md`` and
...
N8N_EXAMPLE_DIR = (
    REPO_ROOT / "examples" / "n8n" / "eidas2_identity_verification"
)
TEMPORAL_EXAMPLE_DIR = (
    REPO_ROOT / "examples" / "temporal" / "eidas2_identity_verification"
)
LANGGRAPH_EXAMPLE_DIR = (
    REPO_ROOT / "examples" / "langgraph" / "eidas2_identity_verification"
)
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "eidas2_identity_verification n8n example drifted. Regenerate via "
...
def test_temporal_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "eidas2_identity_verification Temporal example drifted. Regenerate via "
        "`PYTHONPATH=. python -m tools.compile "
...
def test_langgraph_graph_spec_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "eidas2_identity_verification LangGraph example drifted. Regenerate via "
