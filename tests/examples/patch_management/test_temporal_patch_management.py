"""Byte-parity golden for the Temporal patch_management workflow (G-03).

The committed ``examples/temporal/patch_management/workflow.temporal.py`` is the
Temporal compiler's output for the canonical CACAO playbook at
``content/playbooks/patch_management/playbook.cacao.json``. Activity-name <-> CACAO
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRROR.read_bytes() == CANON.read_bytes(), (
        "examples/temporal/patch_management/playbook.cacao.json drifted from the "
        "canonical content/playbooks/patch_management/playbook.cacao.json. Regenerate via "
        "`./examples/temporal/patch_management/regenerate.sh`."
    )
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/patch_management/workflow.temporal.py drifted from the "
