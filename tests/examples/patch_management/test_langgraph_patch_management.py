"""Byte-parity golden for the LangGraph patch_management example (G-03).

Re-runs ``compilers.langgraph.emit`` and ``compilers.langgraph.state``
against the canonical patch_management CACAO playbook and pins the committed
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == CANON.read_bytes(), (
        "examples/langgraph/patch_management/playbook.cacao.json drifted from the "
...
def test_graph_spec_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/langgraph/patch_management/graph_spec.json drift. Regenerate via "
        "`./examples/langgraph/patch_management/regenerate.sh`."
    )
