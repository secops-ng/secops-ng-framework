...
Mirrors the vuln-intake temporal example test: re-emits the Temporal
workflow stub from the canonical CACAO playbook and pins the result
byte-for-byte against the committed
``examples/temporal/cloud-misconfiguration/workflow_stub.py``. Adds an
activity-name ↔ CACAO action-id parity check so the one-to-one mirroring
contract documented in ``examples/temporal/cloud-misconfiguration/README.md``
is enforced by tests, not by convention.

Regenerate via::

    python -m compilers.temporal \\
        content/playbooks/cloud-misconfiguration/playbook.cacao.json \\
        --out examples/temporal/cloud-misconfiguration/workflow_stub.py
"""
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/cloud-misconfiguration/workflow_stub.py drifted "
        "from the Temporal emitter output. Regenerate via `python -m "
        "compilers.temporal content/playbooks/cloud-misconfiguration/"
        "playbook.cacao.json --out examples/temporal/cloud-misconfiguration/"
        "workflow_stub.py` and commit the new bytes."
    )
