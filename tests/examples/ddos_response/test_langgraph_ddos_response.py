"""Byte-parity golden for the LangGraph ddos_response example (G-03).

Re-runs ``compilers.langgraph.emit`` and ``compilers.langgraph.state``
against the canonical ddos_response CACAO playbook and pins the committed
...
CANON = REPO_ROOT / "content" / "playbooks" / "ddos_response" / "playbook.cacao.json"
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == CANON.read_bytes(), (
        "examples/langgraph/ddos_response/playbook.cacao.json drifted from the "
...
def test_graph_spec_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/langgraph/ddos_response/graph_spec.json drift. Regenerate via "
        "`./examples/langgraph/ddos_response/regenerate.sh`."
    )
