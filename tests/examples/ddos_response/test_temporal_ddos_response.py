"""Byte-parity golden for the Temporal ddos_response workflow (G-03).

The committed ``examples/temporal/ddos_response/workflow.temporal.py`` is the
Temporal compiler's output for the canonical CACAO playbook at
``content/playbooks/ddos_response/playbook.cacao.json``. Activity-name <-> CACAO
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRROR.read_bytes() == CANON.read_bytes(), (
        "examples/temporal/ddos_response/playbook.cacao.json drifted from the "
        "canonical content/playbooks/ddos_response/playbook.cacao.json. Regenerate via "
        "`./examples/temporal/ddos_response/regenerate.sh`."
    )
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/ddos_response/workflow.temporal.py drifted from the "
