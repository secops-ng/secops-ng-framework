"""Byte-parity golden for the Temporal network_security workflow (G-03).

The committed ``examples/temporal/network_security/workflow.temporal.py`` is the
Temporal compiler's output for the canonical CACAO playbook at
``content/playbooks/network_security/playbook.cacao.yaml``. Activity-name <-> CACAO
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRROR.read_bytes() == CANON_YAML.read_bytes(), (
        "examples/temporal/network_security/playbook.cacao.yaml drifted from the "
        "canonical content/playbooks/network_security/playbook.cacao.yaml. Regenerate via "
        "`./examples/temporal/network_security/regenerate.sh`."
    )
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/network_security/workflow.temporal.py drifted from the "
