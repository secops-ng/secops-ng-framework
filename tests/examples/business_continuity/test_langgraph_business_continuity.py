"""Byte-parity golden for the LangGraph business_continuity example (G-03).

Re-runs ``compilers.langgraph.emit`` and ``compilers.langgraph.state``
against the canonical business_continuity CACAO playbook and pins the committed
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == _canonical_mirror_bytes(), (
        "examples/langgraph/business_continuity/playbook.cacao.json drifted from the "
...
def test_graph_spec_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/langgraph/business_continuity/graph_spec.json drift. Regenerate via "
        "`./examples/langgraph/business_continuity/regenerate.sh`."
    )
