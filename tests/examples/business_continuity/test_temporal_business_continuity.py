"""Byte-parity golden for the Temporal business_continuity workflow (G-03).

The committed ``examples/temporal/business_continuity/workflow.temporal.py`` is the
Temporal compiler's output for the canonical CACAO playbook at
``content/playbooks/business_continuity/playbook.cacao.yaml``. Activity-name <-> CACAO
...
EXAMPLE = REPO / "examples" / "temporal" / "business_continuity"
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRROR.read_bytes() == _canonical_mirror_bytes(), (
        "examples/temporal/business_continuity/playbook.cacao.json drifted from the "
        "canonical content/playbooks/business_continuity/playbook.cacao.yaml. Regenerate via "
        "`./examples/temporal/business_continuity/regenerate.sh`."
    )
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/business_continuity/workflow.temporal.py drifted from the "
