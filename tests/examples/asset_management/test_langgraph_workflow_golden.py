"""F-WF-ASSET CORE-FANOUT-LANGGRAPH — byte-parity golden for the LangGraph workflow.

The committed
``examples/langgraph/asset_management/graph_spec.json`` and
``examples/langgraph/asset_management/state_bindings.py`` are the
LangGraph compiler's output for the canonical CACAO playbook at
...
caught at the byte level. Tool-function-name <-> CACAO action-id
parity is verified too — every CACAO action step gets exactly one
``@tool`` async def whose docstring records the originating
``step_id``.
...
SOURCE = (
    REPO / "content" / "playbooks" / "asset_management" / "playbook.cacao.json"
)
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        ...
        "from the canonical content/playbooks/asset_management/"
...
def test_graph_spec_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/langgraph/asset_management/graph_spec.json drifted "
        "from the LangGraph emitter output. Regenerate via "
        "`./examples/langgraph/asset_management/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_state_bindings_matches_state_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/langgraph/asset_management/state_bindings.py drifted "
        "from the LangGraph state emitter output. Regenerate via "
        "`./examples/langgraph/asset_management/regenerate.sh` and commit "
        "the new bytes."
    )
...
def _tool_blocks_from_stub() -> list[tuple[str, str]]:
    ...
    return [
        ...
        for m in _TOOL_DEFN_RE.finditer(text)
    ]
