"""F-WF-ASSET CORE-FANOUT-N8N — byte-parity golden for the n8n workflow.

The committed
``examples/n8n/asset_management/workflow.n8n.json``
...
def test_mirrored_playbook_matches_canonical() -> None:
    ...
    assert MIRROR.read_bytes() == SOURCE.read_bytes(), (
        ...
        "the canonical content/playbooks/asset_management/playbook.cacao."
        "json. Regenerate via "
        "`./examples/n8n/asset_management/regenerate.sh`."
    )
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "asset_management n8n workflow drifted. Regenerate via "
        "`./examples/n8n/asset_management/regenerate.sh` and commit "
