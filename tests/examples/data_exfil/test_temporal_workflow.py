"""Drift guard for the ``examples/temporal/data_exfil/`` worked example.
...
``examples/temporal/data_exfil/workflow.temporal.py``. Adds an
activity-name \u2194 CACAO action-id parity check so the one-to-one
mirroring contract documented in
``examples/temporal/data_exfil/README.md`` is enforced by tests, not
by convention.
...
SOURCE = (
    REPO_ROOT / "content" / "playbooks" / "data_exfil" / "playbook.cacao.json"
)
EXAMPLE_DIR = REPO_ROOT / "examples" / "temporal" / "data_exfil"
