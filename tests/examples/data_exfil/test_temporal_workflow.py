"""Drift guard for the ``examples/temporal/data-exfil/`` worked example.
...
``examples/temporal/data-exfil/workflow.temporal.py``. Adds an
activity-name \u2194 CACAO action-id parity check so the one-to-one
mirroring contract documented in
``examples/temporal/data-exfil/README.md`` is enforced by tests, not
by convention.
...
Regenerate via::
    ...
    ./examples/temporal/data-exfil/regenerate.sh
"""
...
SOURCE = (
    REPO_ROOT / "content" / "playbooks" / "data-exfil" / "playbook.cacao.json"
)
EXAMPLE_DIR = REPO_ROOT / "examples" / "temporal" / "data-exfil"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/data-exfil/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/data-exfil/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/data-exfil/playbook.cacao.json drifted "
        "from the canonical content/playbooks/data-exfil/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/data-exfil/regenerate.sh`."
    )
