...
``examples/n8n/vuln-intake/workflow.json``. Adds a node-id ↔ CACAO
...
WORKED_EXAMPLE = REPO_ROOT / "examples" / "n8n" / "vuln-intake" / "workflow.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/vuln-intake/workflow.json drifted from the n8n "
        "emitter output. Regenerate via `PYTHONPATH=. python -m "
        "tools.compile content/playbooks/vuln-intake/playbook.cacao.json "
        "--target n8n --out examples/n8n/vuln-intake/workflow.json` and "
