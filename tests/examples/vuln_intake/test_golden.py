"""Golden tests for the vuln_intake worked example.

Pins the three reference-compiler outputs against the bytes that ship
under ``examples/{n8n,temporal,langgraph}/vuln_intake/``.
...
commands documented in each ``examples/<target>/vuln_intake/README.md``
...
N8N_GOLDEN = (
    REPO_ROOT / "examples" / "n8n" / "vuln_intake" / "workflow.n8n.json"
)
TEMPORAL_GOLDEN = (
    REPO_ROOT
    / "examples"
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "vuln_intake n8n example drifted. Regenerate via "
...
def test_temporal_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "vuln_intake Temporal example drifted. Regenerate via "
...
def test_langgraph_graph_spec_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "vuln_intake LangGraph example drifted. Regenerate via "
