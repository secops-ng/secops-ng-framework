"""Golden tests for the vuln-intake worked example.

Pins the three reference-compiler outputs against the bytes that ship
under ``examples/{n8n,temporal,langgraph}/vuln-intake/``.
...
commands documented in each ``examples/<target>/vuln-intake/README.md``
...
N8N_GOLDEN = (
    REPO_ROOT / "examples" / "n8n" / "vuln-intake" / "workflow.n8n.json"
)
TEMPORAL_GOLDEN = (
    REPO_ROOT
    / "examples"
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "vuln-intake n8n example drifted. Regenerate via "
...
def test_temporal_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "vuln-intake Temporal example drifted. Regenerate via "
...
def test_langgraph_graph_spec_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "vuln-intake LangGraph example drifted. Regenerate via "
