"""Drift guard for the ``examples/temporal/vuln_intake/`` worked example.
...
``examples/temporal/vuln_intake/workflow.temporal.py``. Adds an
activity-name \u2194 CACAO action-id parity check so the one-to-one
mirroring contract documented in
``examples/temporal/vuln_intake/README.md`` is enforced by tests, not by
convention.
...
SOURCE = (
    REPO_ROOT / "content" / "playbooks" / "vuln_intake" / "playbook.cacao.json"
)
EXAMPLE_DIR = REPO_ROOT / "examples" / "temporal" / "vuln_intake"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/vuln_intake/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/vuln_intake/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/vuln_intake/playbook.cacao.json drifted "
        "from the canonical content/playbooks/vuln_intake/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/vuln_intake/regenerate.sh`."
    )
