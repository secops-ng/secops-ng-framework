"""Drift guard for the ``examples/n8n/on-call-rotation/`` worked example.
...
``examples/n8n/on-call-rotation/workflow.n8n.json``. Adds a node-id
\u2194 CACAO action-id parity check so the one-to-one mirroring contract
documented in ``examples/n8n/on-call-rotation/README.md`` is enforced
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/on-call-rotation/workflow.n8n.json drifted "
        "from the n8n emitter output. Regenerate via "
        "`./examples/n8n/on-call-rotation/regenerate.sh` and "
...
def test_co_located_cacao_mirror_matches_canonical() -> None:
    ...
    assert mirror.exists(), (
        "examples/n8n/on-call-rotation/playbook.cacao.json missing — "
        "run ./examples/n8n/on-call-rotation/regenerate.sh"
    )
    assert mirror.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/on-call-rotation/playbook.cacao.json drifted "
        "from the canonical CACAO source. Re-run "
        "./examples/n8n/on-call-rotation/regenerate.sh and commit."
    )
