"""Drift guard for the ``examples/temporal/on-call-rotation/`` worked example.
...
``examples/temporal/on-call-rotation/workflow.temporal.py``. Adds an
activity-name \u2194 CACAO action-id parity check so the one-to-one
mirroring contract documented in
``examples/temporal/on-call-rotation/README.md`` is enforced by
...
SOURCE = (
    REPO_ROOT / "content" / "playbooks" / "on-call-rotation" / "playbook.cacao.json"
)
EXAMPLE_DIR = REPO_ROOT / "examples" / "temporal" / "on-call-rotation"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/on-call-rotation/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/on-call-rotation/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/on-call-rotation/playbook.cacao.json drifted "
        "from the canonical content/playbooks/on-call-rotation/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/on-call-rotation/regenerate.sh`."
    )
