"""Drift guard for the ``examples/temporal/on_call_rotation/`` worked example.
...
``examples/temporal/on_call_rotation/workflow.temporal.py``. Adds an
activity-name \u2194 CACAO action-id parity check so the one-to-one
mirroring contract documented in
``examples/temporal/on_call_rotation/README.md`` is enforced by
...
SOURCE = (
    REPO_ROOT / "content" / "playbooks" / "on_call_rotation" / "playbook.cacao.json"
)
EXAMPLE_DIR = REPO_ROOT / "examples" / "temporal" / "on_call_rotation"
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        ...
        "Regenerate via `./examples/temporal/on_call_rotation/regenerate.sh`."
    )
