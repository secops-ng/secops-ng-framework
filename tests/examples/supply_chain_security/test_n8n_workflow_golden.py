"""F-WF-SCS CORE-FANOUT-N8N — byte-parity golden for the n8n workflow.

The committed
``examples/n8n/supply_chain_security/workflow.n8n.json`` is the n8n
...
def test_mirrored_playbook_matches_canonical() -> None:
    ...
    assert MIRROR.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/supply_chain_security/playbook.cacao.json "
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "supply_chain_security n8n workflow drifted. Regenerate via "
        "`./examples/n8n/supply_chain_security/regenerate.sh` and "
...
def test_n8n_workflow_emit_is_deterministic() -> None:
    playbook = parse_file(SOURCE)
    assert _serialise_n8n(emit_n8n(playbook)) == _serialise_n8n(
        emit_n8n(playbook)
    )


def test_canonical_declares_n8n_compile_target() -> None:
    """The canonical playbook must declare ``n8n`` so the unified CLI
    dispatches to the n8n emitter without an out-of-band override.

    Temporal and LangGraph siblings will land alongside their own
    target strings — this test only pins the n8n binding owned by
    this card.
    """
