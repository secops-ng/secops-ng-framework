"""Byte-parity golden for the LangGraph dora_tlpt_programme example (G-03).

Re-runs ``compilers.langgraph.emit`` and ``compilers.langgraph.state``
against the canonical dora_tlpt_programme CACAO playbook and pins the committed
...
CANON = REPO_ROOT / "content" / "playbooks" / "dora_tlpt_programme" / "playbook.cacao.json"
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == CANON.read_bytes(), (
        "examples/langgraph/dora_tlpt_programme/playbook.cacao.json drifted from the "
...
def test_graph_spec_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/langgraph/dora_tlpt_programme/graph_spec.json drift. Regenerate via "
        "`./examples/langgraph/dora_tlpt_programme/regenerate.sh`."
    )
