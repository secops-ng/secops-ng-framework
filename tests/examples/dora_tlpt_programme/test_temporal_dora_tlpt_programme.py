"""Byte-parity golden for the Temporal dora_tlpt_programme workflow (G-03).

The committed ``examples/temporal/dora_tlpt_programme/workflow.temporal.py`` is the
Temporal compiler's output for the canonical CACAO playbook at
``content/playbooks/dora_tlpt_programme/playbook.cacao.json``. Activity-name <-> CACAO
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRROR.read_bytes() == CANON.read_bytes(), (
        "examples/temporal/dora_tlpt_programme/playbook.cacao.json drifted from the "
        "canonical content/playbooks/dora_tlpt_programme/playbook.cacao.json. Regenerate via "
        "`./examples/temporal/dora_tlpt_programme/regenerate.sh`."
    )
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/dora_tlpt_programme/workflow.temporal.py drifted from the "
