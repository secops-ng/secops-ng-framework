"""F-WF-10 CORE-FANOUT-LANGGRAPH — byte-parity golden for the LangGraph workflow.

The committed
``examples/langgraph/contractual_obligations_tracker/graph_spec.json``
and
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/langgraph/contractual_obligations_tracker/playbook.cacao.json "
        "drifted from the canonical content/playbooks/"
        "contractual_obligations_tracker/playbook.cacao.json. Regenerate "
        "via `./examples/langgraph/contractual_obligations_tracker/regenerate.sh`."
    )
