"""F-WF-10 CORE-FANOUT-N8N — byte-parity golden for the n8n workflow.

The committed
``examples/n8n/contractual_obligations_tracker/workflow.n8n.json`` is
...
def test_mirrored_playbook_matches_canonical() -> None:
    ...
    assert MIRROR.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/contractual_obligations_tracker/playbook.cacao.json "
        "drifted from the canonical content/playbooks/"
        "contractual_obligations_tracker/playbook.cacao.json. Regenerate "
        "via `./examples/n8n/contractual_obligations_tracker/regenerate.sh`."
    )
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "contractual_obligations_tracker n8n workflow drifted. Regenerate "
        "via `./examples/n8n/contractual_obligations_tracker/regenerate.sh` "
