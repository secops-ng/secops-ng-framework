...
Mirrors the vuln-intake and cloud-misconfiguration temporal example
tests: re-emits the Temporal workflow stub from the canonical CACAO
playbook and pins the result byte-for-byte against the committed
``examples/temporal/ransomware-containment/workflow_stub.py``. Adds an
...
WORKED_EXAMPLE = (
    REPO_ROOT / "examples" / "temporal" / "ransomware-containment" / "workflow_stub.py"
)
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/ransomware-containment/workflow_stub.py drifted from "
        "the Temporal emitter output. Regenerate via `python -m "
        "compilers.temporal content/playbooks/ransomware-containment/playbook.cacao.json "
        "--out examples/temporal/ransomware-containment/workflow_stub.py` and "
