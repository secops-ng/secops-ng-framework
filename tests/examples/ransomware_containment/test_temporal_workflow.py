"""Drift guard for the ``examples/temporal/ransomware-containment/`` worked example.

Mirrors the data-exfil temporal example test: re-emits the Temporal
workflow stub from the canonical CACAO playbook and pins the result
byte-for-byte against the committed
``examples/temporal/ransomware-containment/workflow.temporal.py``. Adds
an activity-name \u2194 CACAO action-id parity check so the one-to-one
mirroring contract documented in
``examples/temporal/ransomware-containment/README.md`` is enforced by
...
Regenerate via::
    ...
    ./examples/temporal/ransomware-containment/regenerate.sh
"""
...
SOURCE = (
    REPO_ROOT / "content" / "playbooks" / "ransomware-containment" / "playbook.cacao.json"
)
EXAMPLE_DIR = REPO_ROOT / "examples" / "temporal" / "ransomware-containment"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/ransomware-containment/workflow.temporal.py drifted "
        "from the Temporal emitter output. Regenerate via "
        "`./examples/temporal/ransomware-containment/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/ransomware-containment/playbook.cacao.json drifted "
        "from the canonical content/playbooks/ransomware-containment/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/ransomware-containment/regenerate.sh`."
    )
