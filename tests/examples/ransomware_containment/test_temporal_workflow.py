"""Drift guard for the ``examples/temporal/ransomware_containment/`` worked example.
...
``examples/temporal/ransomware_containment/workflow.temporal.py``. Adds
an activity-name \u2194 CACAO action-id parity check so the one-to-one
mirroring contract documented in
``examples/temporal/ransomware_containment/README.md`` is enforced by
...
SOURCE = (
    REPO_ROOT / "content" / "playbooks" / "ransomware_containment" / "playbook.cacao.json"
)
EXAMPLE_DIR = REPO_ROOT / "examples" / "temporal" / "ransomware_containment"
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        ...
        "Regenerate via `./examples/temporal/ransomware_containment/regenerate.sh`."
    )
