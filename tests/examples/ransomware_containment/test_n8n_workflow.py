...
``examples/n8n/ransomware-containment/workflow.json``. Adds a node-id ↔
...
WORKED_EXAMPLE = REPO_ROOT / "examples" / "n8n" / "ransomware-containment" / "workflow.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/ransomware-containment/workflow.json drifted from the n8n "
        "emitter output. Regenerate via `PYTHONPATH=. python -m "
        "tools.compile content/playbooks/ransomware-containment/playbook.cacao.json "
        "--target n8n --out examples/n8n/ransomware-containment/workflow.json` and "
