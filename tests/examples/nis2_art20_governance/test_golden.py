"""Byte-parity golden tests for the nis2_art20_governance worked examples (G-03).

Pins the three reference-compiler outputs against the bytes that ship
under ``examples/{n8n,temporal,langgraph}/nis2_art20_governance/``.
...
regeneration of the canonical CACAO source (F-CACAO-NIS2-ART20
playbook, NIS2 Art. 20 management-body cyber-governance obligations),
not a hand-edited copy.

If an emitter change is intentional, regenerate the artifacts with the
commands documented in each
``examples/<target>/nis2_art20_governance/README.md`` and commit the
...
N8N_EXAMPLE_DIR = (
    REPO_ROOT / "examples" / "n8n" / "nis2_art20_governance"
)
TEMPORAL_EXAMPLE_DIR = (
    REPO_ROOT / "examples" / "temporal" / "nis2_art20_governance"
)
LANGGRAPH_EXAMPLE_DIR = (
    REPO_ROOT / "examples" / "langgraph" / "nis2_art20_governance"
)
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "nis2_art20_governance n8n example drifted. Regenerate via "
...
def test_temporal_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "nis2_art20_governance Temporal example drifted. Regenerate via "
        "`PYTHONPATH=. python -m tools.compile "
...
def test_langgraph_graph_spec_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "nis2_art20_governance LangGraph example drifted. Regenerate via "
