"""Byte-parity golden for the Temporal mfa_secured_comms workflow (G-03).

The committed ``examples/temporal/mfa_secured_comms/workflow.temporal.py`` is the
Temporal compiler's output for the canonical CACAO playbook at
``content/playbooks/mfa_secured_comms/playbook.cacao.json``. Activity-name <-> CACAO
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRROR.read_bytes() == CANON.read_bytes(), (
        "examples/temporal/mfa_secured_comms/playbook.cacao.json drifted from the "
        "canonical content/playbooks/mfa_secured_comms/playbook.cacao.json. Regenerate via "
        "`./examples/temporal/mfa_secured_comms/regenerate.sh`."
    )
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/temporal/mfa_secured_comms/workflow.temporal.py drifted from the "
