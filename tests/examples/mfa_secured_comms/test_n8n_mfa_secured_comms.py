"""Byte-parity golden for the n8n mfa_secured_comms workflow (G-03).

The committed ``examples/n8n/mfa_secured_comms/workflow.n8n.json`` is the n8n
compiler's output for the canonical CACAO playbook at
``content/playbooks/mfa_secured_comms/playbook.cacao.json``. This module pins the
...
CANON = REPO / "content" / "playbooks" / "mfa_secured_comms" / "playbook.cacao.json"
EXAMPLE = REPO / "examples" / "n8n" / "mfa_secured_comms"
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRROR.read_bytes() == CANON.read_bytes(), (
        "examples/n8n/mfa_secured_comms/playbook.cacao.json drifted from the "
        "canonical content/playbooks/mfa_secured_comms/playbook.cacao.json. "
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "mfa_secured_comms n8n workflow drifted. Regenerate via "
        "`./examples/n8n/mfa_secured_comms/regenerate.sh` and commit the new "
