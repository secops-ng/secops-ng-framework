"""Byte-parity golden for the LangGraph mfa_secured_comms example (G-03).

Re-runs ``compilers.langgraph.emit`` and ``compilers.langgraph.state``
against the canonical mfa_secured_comms CACAO playbook and pins the committed
...
CANON = REPO_ROOT / "content" / "playbooks" / "mfa_secured_comms" / "playbook.cacao.json"
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == CANON.read_bytes(), (
        "examples/langgraph/mfa_secured_comms/playbook.cacao.json drifted from the "
...
def test_graph_spec_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/langgraph/mfa_secured_comms/graph_spec.json drift. Regenerate via "
        "`./examples/langgraph/mfa_secured_comms/regenerate.sh`."
    )
