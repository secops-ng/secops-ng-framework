"""F-WF-06 CORE-LANGGRAPH — byte-parity golden for the LangGraph workflow.

The committed
``examples/langgraph/infra_posture_management/graph_spec.json`` and
``examples/langgraph/infra_posture_management/state_bindings.py`` are
...
SOURCE = (
    REPO / "content" / "playbooks" / "infra_posture_management" / "playbook.cacao.json"
)
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/langgraph/infra_posture_management/playbook.cacao.json "
        "drifted from the canonical content/playbooks/infra_posture_management/"
...
def test_graph_spec_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/langgraph/infra_posture_management/graph_spec.json drifted "
        "from the LangGraph emitter output. Regenerate via "
        "`./examples/langgraph/infra_posture_management/regenerate.sh` and "
