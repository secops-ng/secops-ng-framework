"""F-WF-DORA-SELFASSESS CORE — drift guard for the n8n dora_ict_risk_selfassess example.

Mirrors the nis2_self_assessment n8n example test: parses the canonical
CACAO playbook, emits the n8n workflow JSON, and pins the result
byte-for-byte against the committed
``examples/n8n/dora_ict_risk_selfassess/workflow.n8n.json``. Adds a
node-id <-> CACAO action-id parity check, a Set-node-uplift check, and
...
(DORA Chapter II ICT risk management operator self-assessment roll-up),
alongside the Temporal and LangGraph siblings under
``examples/temporal/dora_ict_risk_selfassess/`` and
``examples/langgraph/dora_ict_risk_selfassess/``.
...
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "dora_ict_risk_selfassess"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/dora_ict_risk_selfassess/workflow.n8n.json drifted from "
        "the n8n emitter output. Regenerate via "
        "`./examples/n8n/dora_ict_risk_selfassess/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/dora_ict_risk_selfassess/playbook.cacao.json drifted "
        "from the canonical "
        "content/playbooks/dora_ict_risk_selfassess/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/dora_ict_risk_selfassess/regenerate.sh`."
    )
...
def _action_without_commands_steps() -> dict[str, dict]:
    ...
    rendering the primitive call rather than the Set-node uplift that
    surfaces the CACAO contract verbatim — the primitive call is the
    contract. ``dora_ict_risk_selfassess`` has no CORE-bound steps, so
    every action step falls into the Set-node uplift bucket.
    """
...
def test_action_without_commands_steps_emit_set_nodes() -> None:
    ...
    steps = _action_without_commands_steps()
    assert steps, "expected at least one action step in dora_ict_risk_selfassess"
