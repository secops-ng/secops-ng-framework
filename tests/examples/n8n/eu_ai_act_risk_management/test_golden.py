"""F-G03-PARITY EXTEND — drift guard for the n8n eu_ai_act_risk_management example.
...
``examples/n8n/eu_ai_act_risk_management/workflow.n8n.json``. Adds a node-id <-> CACAO
...
``eu_ai_act_risk_management`` playbook, alongside the Temporal and LangGraph siblings under
``tests/examples/{temporal,langgraph}/eu_ai_act_risk_management/``.
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "eu_ai_act_risk_management" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "eu_ai_act_risk_management"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/eu_ai_act_risk_management/workflow.n8n.json drifted from the "
        "n8n emitter output. Regenerate via "
        "`./examples/n8n/eu_ai_act_risk_management/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/eu_ai_act_risk_management/playbook.cacao.json drifted from "
        "the canonical content/playbooks/eu_ai_act_risk_management/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/eu_ai_act_risk_management/regenerate.sh`."
    )
