"""F-WF-PATCH SKELETON-EXAMPLE-N8N — drift guard for the n8n patch_management example.
...
``examples/n8n/patch_management/workflow.n8n.json``. Adds a node-id <->
...
for the ``patch_management`` playbook (NIS2 Art.21(2)(e)). Temporal and
...
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "patch_management"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/patch_management/workflow.n8n.json drifted from the "
        "n8n emitter output. Regenerate via "
        "`./examples/n8n/patch_management/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/patch_management/playbook.cacao.json drifted from "
        "the canonical content/playbooks/patch_management/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/patch_management/regenerate.sh`."
    )
