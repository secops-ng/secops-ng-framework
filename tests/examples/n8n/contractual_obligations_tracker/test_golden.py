"""F-G03-PARITY CORE — drift guard for the n8n contractual_obligations_tracker example.

Mirrors the cra_cvd n8n example test: parses the canonical CACAO
playbook, emits the n8n workflow JSON, and pins the result
byte-for-byte against the committed
``examples/n8n/contractual_obligations_tracker/workflow.n8n.json``. Adds
...
``contractual_obligations_tracker`` playbook, alongside the Temporal and
LangGraph siblings under
``tests/examples/{temporal,langgraph}/contractual_obligations_tracker/``.
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "contractual_obligations_tracker" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "contractual_obligations_tracker"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/contractual_obligations_tracker/workflow.n8n.json drifted from the "
        "n8n emitter output. Regenerate via "
        "`./examples/n8n/contractual_obligations_tracker/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/contractual_obligations_tracker/playbook.cacao.json drifted from "
        "the canonical content/playbooks/contractual_obligations_tracker/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/contractual_obligations_tracker/regenerate.sh`."
    )
...
def test_core_body_steps_emit_code_nodes() -> None:
    ...
    assert core_steps, "expected at least one CORE-bound step in contractual_obligations_tracker"
