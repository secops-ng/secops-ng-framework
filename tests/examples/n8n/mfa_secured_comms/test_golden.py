"""F-WF-MFA SKELETON-EXAMPLE — drift guard for the n8n mfa_secured_comms example.

Mirrors the backup_recovery / ransomware_containment n8n example tests:
parses the canonical CACAO playbook, emits the n8n workflow JSON, and
pins the result byte-for-byte against the committed
``examples/n8n/mfa_secured_comms/workflow.n8n.json``. Adds a node-id <->
...
parity lane for the ``mfa_secured_comms`` playbook (NIS2 Art.21(2)(j)),
alongside the already-shipped Temporal worked example under
``examples/temporal/mfa_secured_comms/``. The LangGraph leg lands in a
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "mfa_secured_comms" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "mfa_secured_comms"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/mfa_secured_comms/workflow.n8n.json drifted from the "
        "n8n emitter output. Regenerate via "
        "`./examples/n8n/mfa_secured_comms/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/mfa_secured_comms/playbook.cacao.json drifted from "
        "the canonical content/playbooks/mfa_secured_comms/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/mfa_secured_comms/regenerate.sh`."
    )
...
def test_core_body_steps_emit_code_nodes() -> None:
    ...
    assert core_steps, "expected at least one CORE-bound step in mfa_secured_comms"
