"""F-G03-PARITY NEXT-BATCH — drift guard for the n8n data_protection_impact_assessment example.

Mirrors the codebase_vuln_management n8n example test: parses the
mirrored CACAO playbook JSON, emits the n8n workflow JSON, and pins
the result byte-for-byte against the committed
``examples/n8n/data_protection_impact_assessment/workflow.n8n.json``. Adds
...
Note: for ``data_protection_impact_assessment`` the canonical source
lives in YAML (``content/playbooks/data_protection_impact_assessment/
playbook.cacao.yaml``) so it can carry inline commentary. The
``regenerate.sh`` script mirrors it into a byte-deterministic JSON
form (sorted keys, 2-space indent, trailing newline) that the emitter
consumes; the JSON mirror is what this test feeds through the parser
and pins against.

This closes the n8n end of the cross-target parity ring (G-03) for the
``data_protection_impact_assessment`` playbook, alongside the Temporal
and LangGraph siblings under
``tests/examples/{temporal,langgraph}/data_protection_impact_assessment/``.
...
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "data_protection_impact_assessment"
...
def _canonical_mirror() -> str:
    """Deterministic JSON mirror derived from the canonical YAML source.

    Matches the transformation applied by ``regenerate.sh``: safe-load
    the YAML, then dump JSON with sorted keys, 2-space indent, and a
    trailing newline.
    """
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/data_protection_impact_assessment/workflow.n8n.json drifted from the "
        "n8n emitter output. Regenerate via "
        "`./examples/n8n/data_protection_impact_assessment/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_text(encoding="utf-8") == _canonical_mirror(), (
        "examples/n8n/data_protection_impact_assessment/playbook.cacao.json drifted from "
        "the canonical content/playbooks/data_protection_impact_assessment/playbook.cacao.yaml. "
        "Regenerate via `./examples/n8n/data_protection_impact_assessment/regenerate.sh`."
    )
