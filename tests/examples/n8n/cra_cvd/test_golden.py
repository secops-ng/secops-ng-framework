"""F-G03-PARITY SKELETON — drift guard for the n8n cra_cvd example.
...
``examples/n8n/cra_cvd/workflow.n8n.json``. Adds a
...
``cra_cvd`` playbook, alongside the Temporal and
LangGraph siblings under ``tests/examples/{temporal,langgraph}/cra_cvd/``.
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "cra_cvd" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "cra_cvd"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/cra_cvd/workflow.n8n.json drifted from the "
        "n8n emitter output. Regenerate via "
        "`./examples/n8n/cra_cvd/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/cra_cvd/playbook.cacao.json drifted from "
        "the canonical content/playbooks/cra_cvd/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/cra_cvd/regenerate.sh`."
    )
...
def test_core_body_steps_emit_code_nodes() -> None:
    ...
    assert core_steps, "expected at least one CORE-bound step in cra_cvd"
