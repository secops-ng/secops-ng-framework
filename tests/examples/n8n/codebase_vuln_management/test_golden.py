"""F-G03-PARITY CORE — drift guard for the n8n codebase_vuln_management example.
...
``examples/n8n/codebase_vuln_management/workflow.n8n.json``. Adds
...
``codebase_vuln_management`` playbook, alongside the Temporal and
LangGraph siblings under
``tests/examples/{temporal,langgraph}/codebase_vuln_management/``.
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "codebase_vuln_management" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "codebase_vuln_management"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/codebase_vuln_management/workflow.n8n.json drifted from the "
        "n8n emitter output. Regenerate via "
        "`./examples/n8n/codebase_vuln_management/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/codebase_vuln_management/playbook.cacao.json drifted from "
        "the canonical content/playbooks/codebase_vuln_management/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/codebase_vuln_management/regenerate.sh`."
    )
...
def test_core_body_steps_emit_code_nodes() -> None:
    ...
    assert core_steps, "expected at least one CORE-bound step in codebase_vuln_management"
