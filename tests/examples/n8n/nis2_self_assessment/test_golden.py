"""F-WF-NIS2-SELF-ASSESS CORE — drift guard for the n8n nis2_self_assessment example.

Mirrors the mfa_secured_comms / data_subject_rights n8n example tests:
parses the canonical CACAO playbook, emits the n8n workflow JSON, and
pins the result byte-for-byte against the committed
``examples/n8n/nis2_self_assessment/workflow.n8n.json``. Adds a node-id
<-> CACAO action-id parity check, a Set-node-uplift check, and a
mirror check against the canonical CACAO source so the
...
(NIS2 Art. 21(2) whole-Article self-assessment roll-up), alongside
the Temporal and LangGraph siblings under
``examples/temporal/nis2_self_assessment/`` and
``examples/langgraph/nis2_self_assessment/``.
...
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "nis2_self_assessment"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/nis2_self_assessment/workflow.n8n.json drifted from "
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/nis2_self_assessment/playbook.cacao.json drifted "
        "from the canonical "
        "content/playbooks/nis2_self_assessment/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/nis2_self_assessment/regenerate.sh`."
    )
...
def _action_without_commands_steps() -> dict[str, dict]:
    ...
    contract. ``nis2_self_assessment`` has no CORE-bound steps, so every
    action step falls into the Set-node uplift bucket.
    """
...
def test_action_without_commands_steps_emit_set_nodes() -> None:
    ...
    assert steps, "expected at least one action step in nis2_self_assessment"
