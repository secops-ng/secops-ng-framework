"""F-G03-PARITY EXTEND — drift guard for the n8n asset_management example.

Mirrors the vulnerability_management / backup_recovery n8n example tests:
parses the canonical CACAO playbook, emits the n8n workflow JSON, and
pins the result byte-for-byte against the committed
``examples/n8n/asset_management/workflow.n8n.json``. Adds a node-id <->
...
``asset_management`` playbook, alongside the Temporal and LangGraph
siblings under ``tests/examples/{temporal,langgraph}/asset_management/``.

Regenerate via::

    ./examples/n8n/asset_management/regenerate.sh
"""
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "asset_management" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "asset_management"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/asset_management/workflow.n8n.json drifted from the "
        "n8n emitter output. Regenerate via "
        "`./examples/n8n/asset_management/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/asset_management/playbook.cacao.json drifted from "
        "the canonical content/playbooks/asset_management/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/asset_management/regenerate.sh`."
    )
...
def _action_without_commands_steps() -> dict[str, dict]:
    ...
    rendering the primitive call (CORE-MECH-EMIT-N8N) rather than the Set-node
    uplift, so they are excluded from the Set-node contract checks below.

    As of the CORE-WIRE change all six action steps in this playbook are
    bound, so this returns ``{}`` and the three Set-node tests that consume it
    are vacuous here. They are kept rather than deleted: they are the contract
    that catches a step being *un*bound again, and they would start asserting
    the moment a future card adds an unbound step. The live check for the
    bound steps is :func:`test_core_body_steps_emit_code_nodes`.
    """
...
def test_core_body_steps_emit_code_nodes() -> None:
    ...
    ``core_body`` and regenerating leaves this suite green, since the
    ...
    assert len(core_steps) == 6, (
        f"expected all six asset_management action steps bound at CORE-WIRE, "
        f"found {len(core_steps)}"
    )
