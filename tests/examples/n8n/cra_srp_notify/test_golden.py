"""F-G03-PARITY SKELETON — drift guard for the n8n cra_srp_notify example.
...
``examples/n8n/cra_srp_notify/workflow.n8n.json``. Adds a
...
``cra_srp_notify`` playbook, alongside the Temporal and
LangGraph siblings under ``tests/examples/{temporal,langgraph}/cra_srp_notify/``.
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "cra_srp_notify" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "cra_srp_notify"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/cra_srp_notify/workflow.n8n.json drifted from the "
        "n8n emitter output. Regenerate via "
        "`./examples/n8n/cra_srp_notify/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/cra_srp_notify/playbook.cacao.json drifted from "
        "the canonical content/playbooks/cra_srp_notify/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/cra_srp_notify/regenerate.sh`."
    )
