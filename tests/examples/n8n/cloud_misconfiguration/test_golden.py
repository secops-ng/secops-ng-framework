"""F-G03-PARITY EXTEND — drift guard for the n8n cloud_misconfiguration example.

Mirrors the phishing_triage / backup_recovery n8n example tests: parses
the canonical CACAO playbook, emits the n8n workflow JSON, and pins the
result byte-for-byte against the committed
``examples/n8n/cloud_misconfiguration/workflow.json``. Adds a node-id <-> CACAO
step-id parity check, a Set-node-uplift check, and a mirror check
...
``cloud_misconfiguration`` playbook, alongside the Temporal and LangGraph siblings under
``tests/examples/{temporal,langgraph}/cloud_misconfiguration/``.
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "cloud_misconfiguration" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "cloud_misconfiguration"
WORKED_EXAMPLE = EXAMPLE / "workflow.json"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        ...
        "`./examples/n8n/cloud_misconfiguration/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/cloud_misconfiguration/playbook.cacao.json drifted from "
        "the canonical content/playbooks/cloud_misconfiguration/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/cloud_misconfiguration/regenerate.sh`."
    )
