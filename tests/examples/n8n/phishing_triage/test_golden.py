"""F-G03-PARITY EXTEND — drift guard for the n8n phishing_triage example.
...
``examples/n8n/phishing_triage/workflow.n8n.json``. Adds a
...
``phishing_triage`` playbook, alongside the Temporal and
LangGraph siblings under ``tests/examples/{temporal,langgraph}/phishing_triage/``.
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "phishing_triage" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "phishing_triage"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/phishing_triage/workflow.n8n.json drifted from the "
        "n8n emitter output. Regenerate via "
        "`./examples/n8n/phishing_triage/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/phishing_triage/playbook.cacao.json drifted from "
        "the canonical content/playbooks/phishing_triage/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/phishing_triage/regenerate.sh`."
    )
