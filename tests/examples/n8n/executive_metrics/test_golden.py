"""F-G03-PARITY EXTEND — drift guard for the n8n executive_metrics example.
...
``examples/n8n/executive_metrics/workflow.n8n.json``. Adds a node-id
...
``executive_metrics`` playbook, alongside the Temporal and LangGraph
siblings under
``tests/examples/{temporal,langgraph}/executive_metrics/``.
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "executive_metrics" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "executive_metrics"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/executive_metrics/workflow.n8n.json drifted from the "
        "n8n emitter output. Regenerate via "
        "`./examples/n8n/executive_metrics/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/executive_metrics/playbook.cacao.json drifted from "
        "the canonical content/playbooks/executive_metrics/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/executive_metrics/regenerate.sh`."
    )
