"""F-WF-HYG SKELETON-EXAMPLE — drift guard for the n8n cyber_hygiene_training example.

Mirrors the mfa_secured_comms / backup_recovery / ransomware_containment n8n
example tests: parses the canonical CACAO playbook, emits the n8n workflow
JSON, and pins the result byte-for-byte against the committed
``examples/n8n/cyber_hygiene_training/workflow.n8n.json``. Adds a node-id
<-> CACAO action-id parity check, a Set-node-uplift check, and a mirror
check against the canonical CACAO source so the ``regenerate.sh`` contract
(mirror + emit) cannot drift unnoticed.

This worked example pins the n8n leg (target 2 of 3) of the cross-target
parity lane for the ``cyber_hygiene_training`` playbook (NIS2 Art.21(2)(g)),
alongside the already-shipped Temporal worked example under
``examples/temporal/cyber_hygiene_training/``. The LangGraph leg lands in a
separate sibling card.
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "cyber_hygiene_training" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "cyber_hygiene_training"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/cyber_hygiene_training/workflow.n8n.json drifted from "
        "the n8n emitter output. Regenerate via "
        "`./examples/n8n/cyber_hygiene_training/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/cyber_hygiene_training/playbook.cacao.json drifted "
        "from the canonical "
...
def _action_without_commands_steps() -> dict[str, dict]:
    ...
    return {
        ...
        if step.get("type") == "action" and all(c.get("type") == "manual" for c in step.get("commands") or ())
    }
