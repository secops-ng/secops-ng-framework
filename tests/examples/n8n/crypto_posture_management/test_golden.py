"""F-WF-CRYPTO SKELETON-EXAMPLE-N8N — drift guard for the n8n crypto_posture_management example.

Mirrors the mfa_secured_comms / backup_recovery n8n example tests:
parses the canonical CACAO playbook, emits the n8n workflow JSON, and
pins the result byte-for-byte against the committed
``examples/n8n/crypto_posture_management/workflow.n8n.json``. Adds a
...
parity lane for the ``crypto_posture_management`` playbook
(NIS2 Art.21(2)(h)), alongside the already-shipped Temporal worked
example under ``examples/temporal/crypto_posture_management/``. The
LangGraph leg lands in a separate sibling card.
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "crypto_posture_management" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "crypto_posture_management"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/crypto_posture_management/workflow.n8n.json drifted "
        "from the n8n emitter output. Regenerate via "
        "`./examples/n8n/crypto_posture_management/regenerate.sh` and "
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/crypto_posture_management/playbook.cacao.json drifted "
        "from the canonical "
