"""F-G03-PARITY GOLDENS — drift guard for the n8n nis2_art20_governance example.

Mirrors the asset_management / vulnerability_management n8n example tests:
parses the canonical CACAO playbook, emits the n8n workflow JSON, and
pins the result byte-for-byte against the committed
``examples/n8n/nis2_art20_governance/workflow.n8n.json``. Adds a node-id
...
``nis2_art20_governance`` playbook (NIS2 Art. 20 management-body
cyber-governance obligations), alongside the Temporal and LangGraph
siblings under ``tests/examples/{temporal,langgraph}/nis2_art20_governance/``.
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "nis2_art20_governance" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "nis2_art20_governance"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/nis2_art20_governance/workflow.n8n.json drifted from "
        "the n8n emitter output. Regenerate via "
        "`./examples/n8n/nis2_art20_governance/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/nis2_art20_governance/playbook.cacao.json drifted "
        "from the canonical content/playbooks/nis2_art20_governance/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/nis2_art20_governance/regenerate.sh`."
    )
...
def _action_without_commands_steps() -> dict[str, dict]:
    """Action steps without CACAO commands, excluding those uplifted to
    Python code nodes via ``x_secops_ng.core_body`` — those emit
    ``n8n-nodes-base.code`` rather than Set nodes.
    """
...
def test_core_body_steps_emit_code_nodes() -> None:
    ...
    assert core_steps, "expected at least one CORE-bound step in nis2_art20_governance"
