"""F-G03-PARITY EXTEND — drift guard for the n8n dora_major_incident_reporting example.
...
``examples/n8n/dora_major_incident_reporting/workflow.n8n.json``. Adds a node-id
...
``dora_major_incident_reporting`` playbook, alongside the Temporal and LangGraph
siblings under
``tests/examples/{temporal,langgraph}/dora_major_incident_reporting/``.
...
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "dora_major_incident_reporting"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/dora_major_incident_reporting/workflow.n8n.json drifted from the "
        "n8n emitter output. Regenerate via "
        "`./examples/n8n/dora_major_incident_reporting/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/dora_major_incident_reporting/playbook.cacao.json drifted from "
        "the canonical content/playbooks/dora_major_incident_reporting/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/dora_major_incident_reporting/regenerate.sh`."
    )
