"""F-WF-CRYPTOMGMT CORE-EXAMPLE-N8N — drift guard for the n8n cryptographic_controls example.

Mirrors the crypto_posture_management n8n example test: parses the
canonical CACAO playbook, emits the n8n workflow JSON, and pins the
result byte-for-byte against the committed
``examples/n8n/cryptographic_controls/workflow.n8n.json``. Adds a
...
cross-target parity lane (G-03) for the ``cryptographic_controls``
playbook (NIS2 Art. 21(2)(h)), the write-side lifecycle counterpart to
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "cryptographic_controls" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "cryptographic_controls"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/cryptographic_controls/workflow.n8n.json drifted "
        "from the n8n emitter output. Regenerate via "
        "`./examples/n8n/cryptographic_controls/regenerate.sh` and "
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/cryptographic_controls/playbook.cacao.json drifted "
        "from the canonical "
...
def test_core_body_steps_emit_code_nodes() -> None:
    ...
    *unbound* steps, which left its bound steps with nothing asserting the
    ...
    assert core_steps, "expected at least one CORE-bound step in cryptographic_controls"
...
def test_lifecycle_switch_routes_every_event_class() -> None:
    """The lifecycle switch is real topology, not a promise.

    The SKELETON scaffolded the three disciplines as a linear chain and
    deferred branch selection to a sibling card. The bound primitives judge
    one event class each and refuse the others, so the linear form cannot
    execute a run — this pins the switch that replaced it: every value of
    the closed ``__lifecycle_event__`` vocabulary routes to the step whose
    primitive accepts it, and all three branches converge on the attestation.
    """
    raw = json.loads(SOURCE.read_text(encoding="utf-8"))
    workflow = raw["workflow"]
    switches = {
        sid: s for sid, s in workflow.items() if s.get("type") == "switch-condition"
    }
    assert len(switches) == 1, "expected exactly one lifecycle switch"
    (switch_id, switch), = switches.items()
    assert switch["switch"] == "__lifecycle_event__"

    by_name = {s.get("name"): sid for sid, s in workflow.items()}
    expected = {
        "key-generate": "key lifecycle",
        "key-rotate": "key lifecycle",
        "key-revoke": "key lifecycle",
        "cert-issue": "certificate lifecycle",
        "cert-renew": "certificate lifecycle",
        "cert-revoke": "certificate lifecycle",
        "enforcement-gate": "enforce encryption",
    }
    assert set(switch["cases"]) == set(expected), (
    ...
    attestation = by_name["record lifecycle evidence"]
    for step_name in ("key lifecycle", "certificate lifecycle", "enforce encryption"):
        assert workflow[by_name[step_name]]["on_completion"] == attestation, (
            f"branch {step_name!r} must converge on the attestation step"
        )

    nodes_by_id = {node["id"]: node for node in
                   json.loads(WORKED_EXAMPLE.read_text(encoding="utf-8"))["nodes"]}
    assert nodes_by_id[switch_id]["type"] == "n8n-nodes-base.switch"
