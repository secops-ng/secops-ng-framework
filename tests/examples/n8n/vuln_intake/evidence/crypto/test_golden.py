"""F-CP-05 EXTEND-tests-goldens (n8n) — byte-parity replay golden.

Pins the committed crypto-attestation worked example for the n8n target
under ``examples/n8n/vuln_intake/evidence/crypto/`` against a fresh
re-emission driven through the n8n adapter at
:func:`compilers.n8n.evidence.emit_crypto_attestation_artifact_n8n`.
...
the adapter writes. This test re-runs the adapter against the same
JSON-native payload ``examples/n8n/vuln_intake/evidence/crypto/regenerate.py``
ships, schema-validates the result against
``schemas/evidence/crypto-attestation.schema.json`` (with promoted
sibling schemas resolved), and asserts byte-equality with the committed
snapshot.

Coverage axes:

1. **Schema-conformant emit.** The re-emitted artifact validates against
   the crypto-attestation schema before the byte comparison runs, so a
   shape regression in the n8n adapter surfaces with a precise
   diagnostic.
2. **Byte-parity with the committed example.** The re-emitted artifact's
   on-disk bytes match the committed ``secret-handling-attestation.json``
   exactly. If the shared emitter or n8n adapter intentionally changes
   serialisation, the example must be regenerated via
...
in ``examples/n8n/vuln_intake/evidence/crypto/regenerate.py``. The
...
EXAMPLE_DIR = REPO / "examples" / "n8n" / "vuln_intake" / "evidence" / "crypto"
GOLDEN = EXAMPLE_DIR / "secret-handling-attestation.json"


# Mirrors PAYLOAD in examples/n8n/vuln_intake/evidence/crypto/regenerate.py.
...
def test_replay_artifact_validates_against_schema(tmp_path: Path) -> None:
    ...
    """
    result = emit_crypto_attestation_artifact_n8n(PAYLOAD, tmp_path)
...
def _drift_hint() -> str:
    return (
        "n8n crypto-attestation example drifted from a fresh adapter "
        "replay. If the change is intentional, regenerate the example "
        "via `PYTHONPATH=. python examples/n8n/vuln_intake/evidence/"
        "crypto/regenerate.py` and commit the new bytes alongside the "
        "emitter / adapter change."
    )
