"""F-G03-PARITY EXTEND — drift guard for the n8n incident_management example.

Mirrors the cloud_misconfiguration / backup_recovery n8n example tests:
parses the mirrored CACAO playbook, emits the n8n workflow JSON, and
pins the result byte-for-byte against the committed
``examples/n8n/incident_management/workflow.n8n.json``. Adds a node-id
<-> CACAO step-id parity check, a Set-node-uplift check, and a mirror
check against the (canonical CACAO source + core_body overlay) so the
``regenerate.sh`` contract (mirror + emit) cannot drift unnoticed.

F-WF-05 CORE-WIRE-N8N (SKELETON wave) seam.
============================================
...
diverges via ``examples/n8n/incident_management/core_body.overlay.json``.
The legacy invariant ``mirror == canonical`` is replaced by
``mirror == canonical + overlay`` — see the sibling drift guard at
``tests/examples/test_n8n_incident_management.py`` for the overlay
boundary check. This module pins the emitter output against the
overlay-applied mirror, so the emitter contract stays enforced.

This closes the n8n end of the cross-target parity ring (G-03) for the
``incident_management`` playbook, alongside the Temporal and LangGraph
siblings under
``tests/examples/{temporal,langgraph}/incident_management/``.
...
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "incident_management"
WORKED_EXAMPLE = EXAMPLE / "workflow.n8n.json"
MIRRORED_CACAO = EXAMPLE / "playbook.cacao.json"

# For the incident_management SKELETON wave the emitter reads the
# overlay-applied mirror, not the canonical source directly. The
# mirror-vs-canonical overlay boundary check lives in the sibling
# ``tests/examples/test_n8n_incident_management.py``.
...
def test_example_artifacts_are_committed() -> None:
    for path in (WORKED_EXAMPLE, MIRRORED_CACAO, CANON_SOURCE):
...
def test_core_body_steps_emit_code_nodes() -> None:
    ...
    *unbound* steps, which left its bound steps with nothing asserting the
    ...
    assert core_steps, "expected at least one CORE-bound step in incident_management"
