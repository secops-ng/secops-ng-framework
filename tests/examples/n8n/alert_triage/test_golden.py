"""F-G03-PARITY EXTEND — drift guard for the n8n alert_triage example.

Mirrors the cra_cvd / backup_recovery n8n example tests, adapted for the
alert_triage playbook whose canonical source is YAML at
``content/playbooks/alert_triage.cacao.yaml`` (not a JSON directory).

Parses the mirrored JSON, emits the n8n workflow, and pins the result
byte-for-byte against the committed
``examples/n8n/alert_triage/workflow.n8n.json``. Adds a node-id <-> CACAO
step-id parity check, a Set-node-uplift check, and a mirror check that
the co-located ``playbook.cacao.json`` is the byte-deterministic
``yaml.safe_load`` + ``json.dumps(indent=2, sort_keys=True)`` transcode
of the canonical YAML source.
...
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "alert_triage"
...
def test_example_artifacts_are_committed() -> None:
    for path in (CANON_YAML, WORKED_EXAMPLE, MIRRORED_CACAO):
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        ...
        "`./examples/n8n/alert_triage/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_yaml_source() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/alert_triage/playbook.cacao.json drifted from "
        "the canonical content/playbooks/alert_triage.cacao.yaml transcode. "
        "Regenerate via `./examples/n8n/alert_triage/regenerate.sh`."
    )
