"""F-G03-PARITY EXTEND — drift guard for the n8n threat_intel_ingest example.
...
``examples/n8n/threat_intel_ingest/workflow.n8n.json``. Adds a node-id
...
``threat_intel_ingest`` playbook, alongside the Temporal and LangGraph
siblings under
``tests/examples/{temporal,langgraph}/threat_intel_ingest/``.
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "threat_intel_ingest" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "threat_intel_ingest"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/threat_intel_ingest/workflow.n8n.json drifted from the "
        "n8n emitter output. Regenerate via "
        "`./examples/n8n/threat_intel_ingest/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/threat_intel_ingest/playbook.cacao.json drifted from "
        "the canonical content/playbooks/threat_intel_ingest/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/threat_intel_ingest/regenerate.sh`."
    )
