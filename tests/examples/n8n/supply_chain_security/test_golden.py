"""F-G03-PARITY NEXT-BATCH-2 — drift guard for the n8n supply_chain_security example.
...
``examples/n8n/supply_chain_security/workflow.n8n.json``. Adds
...
``supply_chain_security`` playbook, alongside the Temporal and
LangGraph siblings under
``tests/examples/{temporal,langgraph}/supply_chain_security/``.
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "supply_chain_security" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "supply_chain_security"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/supply_chain_security/workflow.n8n.json drifted from the "
        "n8n emitter output. Regenerate via "
        "`./examples/n8n/supply_chain_security/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/supply_chain_security/playbook.cacao.json drifted from "
        "the canonical content/playbooks/supply_chain_security/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/supply_chain_security/regenerate.sh`."
    )
...
def test_core_body_steps_emit_code_nodes() -> None:
    ...
    assert core_steps, "expected at least one CORE-bound step in supply_chain_security"
