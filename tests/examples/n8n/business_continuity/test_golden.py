"""F-NIS2-BCP CORE — drift guard for the n8n business_continuity example.

Parses the canonical CACAO playbook, emits the n8n workflow JSON, and
pins the result byte-for-byte against the committed
``examples/n8n/business_continuity/workflow.n8n.json``. Adds a node-id
...
(G-03) for the ``business_continuity`` playbook (NIS2 Art. 21(2)(c),
Art. 23), the plan-lifecycle sibling of the ``backup_recovery``
exercise-lifecycle playbook.
...
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "business_continuity"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/business_continuity/workflow.n8n.json drifted from "
        "the n8n emitter output. Regenerate via "
        "`./examples/n8n/business_continuity/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == _canonical_mirror_bytes(), (
        "examples/n8n/business_continuity/playbook.cacao.json drifted "
        "from the canonical "
