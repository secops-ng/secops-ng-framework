"""F-WF-SOC2-EVIDENCE GOLDEN-N8N — drift guard for the n8n soc2_evidence_collector example.

Mirrors the ddos_response n8n example tests: parses the canonical CACAO
playbook, emits the n8n workflow JSON, and pins the result
byte-for-byte against the committed
``examples/n8n/soc2_evidence_collector/workflow.n8n.json``. Adds a node-id <->
...
for the ``soc2_evidence_collector`` playbook (AICPA TSC; FOUNDATION
auditability). The Temporal and LangGraph siblings land in the same
golden card — the maturity-ladder audit (#937) found the examples
committed but unpinned on all three targets.

Regenerate via::

    ./examples/n8n/soc2_evidence_collector/regenerate.sh
"""
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "soc2_evidence_collector" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "soc2_evidence_collector"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/soc2_evidence_collector/workflow.n8n.json drifted from the "
        "n8n emitter output. Regenerate via "
        "`./examples/n8n/soc2_evidence_collector/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/soc2_evidence_collector/playbook.cacao.json drifted from "
        "the canonical content/playbooks/soc2_evidence_collector/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/soc2_evidence_collector/regenerate.sh`."
    )
...
def _action_without_commands_steps() -> dict[str, dict]:
    ...
    command-less *and* unbound step still surfaces the contract on a Set node.
    The CORE wiring bound all four action steps, so selecting on command-less
    alone would assert the pre-binding shape and this dict is now empty.
    """
...
def test_core_body_steps_emit_code_nodes() -> None:
    ...
    assert bound, "expected soc2_evidence_collector to carry core_body bindings"
