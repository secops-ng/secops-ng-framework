"""F-G03-PARITY SKELETON-EXAMPLE-N8N — drift guard for the n8n security_awareness_training example.
...
``examples/n8n/security_awareness_training/workflow.n8n.json``. Adds a node-id <->
...
for the ``security_awareness_training`` playbook (NIS2 Art.20). Temporal and
LangGraph siblings land in separate cards.
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "security_awareness_training" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "security_awareness_training"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/security_awareness_training/workflow.n8n.json drifted from the "
        "n8n emitter output. Regenerate via "
        "`./examples/n8n/security_awareness_training/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/security_awareness_training/playbook.cacao.json drifted from "
        "the canonical content/playbooks/security_awareness_training/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/security_awareness_training/regenerate.sh`."
    )
