"""F-G03-PARITY EXTEND — drift guard for the n8n ransomware_containment example.
...
``ransomware_containment`` playbook, alongside the Temporal and LangGraph
siblings under
``tests/examples/{temporal,langgraph}/ransomware_containment/``.
...
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "ransomware_containment"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/ransomware_containment/workflow.n8n.json drifted from the "
        "n8n emitter output. Regenerate via "
        "`./examples/n8n/ransomware_containment/regenerate.sh` and commit the "
        "new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/ransomware_containment/playbook.cacao.json drifted from "
        "the canonical content/playbooks/ransomware_containment/playbook.cacao.json. "
        "Regenerate via `./examples/n8n/ransomware_containment/regenerate.sh`."
    )
