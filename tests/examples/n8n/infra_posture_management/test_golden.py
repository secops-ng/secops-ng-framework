"""F-G03-PARITY EXTEND — drift guard for the n8n infra_posture_management example.
...
``examples/n8n/infra_posture_management/workflow.n8n.json``. Adds a
node-id <-> CACAO step-id parity check, a Set-node-uplift check, and a
...
``infra_posture_management`` playbook, alongside the Temporal and
LangGraph siblings under
``tests/examples/{temporal,langgraph}/infra_posture_management/``.
...
SOURCE = REPO_ROOT / "content" / "playbooks" / "infra_posture_management" / "playbook.cacao.json"
EXAMPLE = REPO_ROOT / "examples" / "n8n" / "infra_posture_management"
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/infra_posture_management/workflow.n8n.json drifted "
        "from the n8n emitter output. Regenerate via "
        "`./examples/n8n/infra_posture_management/regenerate.sh` and commit "
        "the new bytes."
    )
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/infra_posture_management/playbook.cacao.json drifted "
        "from the canonical content/playbooks/infra_posture_management/"
...
def test_core_body_steps_emit_code_nodes() -> None:
    ...
    *unbound* steps, which left its bound steps with nothing asserting the
    ...
    assert core_steps, "expected at least one CORE-bound step in infra_posture_management"
