"""Byte-parity golden for the n8n security_awareness_training workflow (G-03).

The committed ``examples/n8n/security_awareness_training/workflow.n8n.json`` is the n8n
compiler's output for the canonical CACAO playbook at
``content/playbooks/security_awareness_training/playbook.cacao.json``. This module pins the
...
EXAMPLE = REPO / "examples" / "n8n" / "security_awareness_training"
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRROR.read_bytes() == CANON.read_bytes(), (
        "examples/n8n/security_awareness_training/playbook.cacao.json drifted from the "
        "canonical content/playbooks/security_awareness_training/playbook.cacao.json. "
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "security_awareness_training n8n workflow drifted. Regenerate via "
        "`./examples/n8n/security_awareness_training/regenerate.sh` and commit the new "
