...
SKELETON-stage scope: every CORE action body is a stub raising
``NotImplementedError``. Primitives binding lands in the CORE-PRIM card
(card 5 of the F-WF-05 wave).
...
def test_committed_artefacts_exist() -> None:
    for path in (MIRRORED_CACAO, WORKED_EXAMPLE):
...
def test_mirrored_cacao_matches_canonical_source() -> None:
    assert MIRRORED_CACAO.read_bytes() == SOURCE.read_bytes(), (
        "examples/temporal/incident-management/playbook.cacao.json drifted "
        "from the canonical content/playbooks/incident-management/playbook.cacao.json. "
        "Regenerate via `./examples/temporal/incident-management/regenerate.sh`."
    )
