"""Golden tests for the executive_metrics worked example.

Pins the three reference-compiler outputs against the bytes that ship
under ``examples/{n8n,temporal,langgraph}/executive_metrics/``.
...
commands documented in each ``examples/<target>/executive_metrics/README.md``
...
N8N_GOLDEN = (
    REPO_ROOT / "examples" / "n8n" / "executive_metrics" / "workflow.n8n.json"
)
TEMPORAL_GOLDEN = (
    REPO_ROOT
    / "examples"
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "executive_metrics n8n example drifted. Regenerate via "
...
def test_temporal_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "executive_metrics Temporal example drifted. Regenerate via "
...
def test_langgraph_graph_spec_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "executive_metrics LangGraph example drifted. Regenerate via "
