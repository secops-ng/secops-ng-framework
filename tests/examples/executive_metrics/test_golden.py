"""Golden tests for the executive-metrics worked example.

Pins the three reference-compiler outputs against the bytes that ship
under ``examples/{n8n,temporal,langgraph}/executive-metrics/``.
...
commands documented in each ``examples/<target>/executive-metrics/README.md``
...
N8N_GOLDEN = (
    REPO_ROOT / "examples" / "n8n" / "executive-metrics" / "workflow.n8n.json"
)
TEMPORAL_GOLDEN = (
    REPO_ROOT
    / "examples"
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "executive-metrics n8n example drifted. Regenerate via "
...
def test_temporal_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "executive-metrics Temporal example drifted. Regenerate via "
...
def test_langgraph_graph_spec_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "executive-metrics LangGraph example drifted. Regenerate via "
