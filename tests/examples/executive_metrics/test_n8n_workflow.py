"""Drift guard for the ``examples/n8n/executive_metrics/`` worked example.

Mirrors the post_incident_review / vuln_intake / data_exfil n8n example
...
documented in ``examples/n8n/executive_metrics/README.md`` is enforced
...
def test_worked_example_matches_emitter_output() -> None:
    ...
    assert rendered == expected, (
        "examples/n8n/executive_metrics/workflow.n8n.json drifted "
        "from the n8n emitter output. Regenerate via "
        "`./examples/n8n/executive_metrics/regenerate.sh` and "
...
# ---------------------------------------------------------------------------
# CORE semantic checks — Set-node uplift parity with sibling n8n examples.
# ---------------------------------------------------------------------------
...
def test_co_located_cacao_mirror_matches_canonical() -> None:
    ...
    assert mirror.exists(), (
        "examples/n8n/executive_metrics/playbook.cacao.json missing — "
        "run ./examples/n8n/executive_metrics/regenerate.sh"
    )
    assert mirror.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/executive_metrics/playbook.cacao.json drifted "
        "from the canonical CACAO source. Re-run "
        "./examples/n8n/executive_metrics/regenerate.sh and commit."
    )
