"""F-WF-12 CORE-FANOUT-N8N-WIRE — byte-parity golden for the n8n workflow.

The committed
``examples/n8n/it_security_support_agent/workflow.n8n.json`` is the
n8n compiler's output for the canonical CACAO playbook at
...
interaction-evidence fixture are owned by the GOLDEN sibling card
(CORE-FANOUT-N8N-GOLDEN) — this module covers the workflow graph only.
"""
...
def test_mirrored_playbook_matches_canonical() -> None:
    ...
    assert MIRROR.read_bytes() == SOURCE.read_bytes(), (
        "examples/n8n/it_security_support_agent/playbook.cacao.json "
...
def test_n8n_workflow_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "it_security_support_agent n8n workflow drifted. Regenerate "
        "via `./examples/n8n/it_security_support_agent/regenerate.sh` "
