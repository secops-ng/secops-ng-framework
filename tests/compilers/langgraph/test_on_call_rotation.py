"""Golden tests for the LangGraph reference compiler — on_call_rotation.

Mirrors ``test_golden.py`` (vuln_intake) and ``test_identity_compromise.py``
for the on_call_rotation fixture: pins both the GraphSpec JSON and the
...
def test_graph_spec_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "LangGraph graph-spec golden drift (on_call_rotation). "
...
def test_graph_spec_golden_has_expected_shape() -> None:
    """Sanity-check the golden carries the on_call_rotation topology."""
...
def test_module_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "LangGraph state-module golden drift (on_call_rotation). "
