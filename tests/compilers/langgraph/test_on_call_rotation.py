"""Golden tests for the LangGraph reference compiler — on-call-rotation.

Mirrors ``test_golden.py`` (vuln-intake) and ``test_identity_compromise.py``
for the on-call-rotation fixture: pins both the GraphSpec JSON and the
state + tool bindings module byte-for-byte.

Regenerate via::

    python -m compilers.langgraph.emit \\
        tests/compilers/_shared/fixtures/on_call_rotation.cacao.json \\
        > tests/compilers/langgraph/golden/on_call_rotation.graph_spec.json

    python -m compilers.langgraph.state \\
        tests/compilers/_shared/fixtures/on_call_rotation.cacao.json \\
        > tests/compilers/langgraph/golden/on_call_rotation.expected.py
"""
...
GOLDEN_GRAPH = (
    Path(__file__).parent / "golden" / "on_call_rotation.graph_spec.json"
)
GOLDEN_MODULE = (
    Path(__file__).parent / "golden" / "on_call_rotation.expected.py"
)
...
def test_graph_spec_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "LangGraph graph-spec golden drift (on-call-rotation). "
...
def test_graph_spec_golden_has_expected_shape() -> None:
    """Sanity-check the golden carries the on-call-rotation topology."""
    payload = json.loads(GOLDEN_GRAPH.read_text(encoding="utf-8"))
    assert payload["stable_id"] == "playbook.on_call_rotation@v1"
    assert payload["end_sentinel"] == "__END__"
    # 4 action steps + 1 if-condition; start/end are not materialised.
...
def test_module_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "LangGraph state-module golden drift (on-call-rotation). "
...
def test_module_golden_exposes_registry_symbols() -> None:
    ...
    assert "STATE_SCHEMA = PlaybookOnCallRotationV1State" in src
    assert "AGENTIC_HOOK = llm_step" in src
    assert (
        "TOOLS = (load_rotation_roster, bind_escalation_tiers, "
        "generate_handoff_brief, notify_incoming_on_call,)"
    ) in src
