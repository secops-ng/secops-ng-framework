...
Regenerate via::
    ...
    python -m compilers.langgraph.emit \\
        ...
        > tests/compilers/langgraph/golden/threat_intel_ingest.graph_spec.json

    python -m compilers.langgraph.state \\
        tests/compilers/_shared/fixtures/threat_intel_ingest.cacao.json \\
        > tests/compilers/langgraph/golden/threat_intel_ingest.expected.py
"""
...
GOLDEN_GRAPH = (
    Path(__file__).parent / "golden" / "threat_intel_ingest.graph_spec.json"
)
GOLDEN_MODULE = (
    Path(__file__).parent / "golden" / "threat_intel_ingest.expected.py"
)
...
def test_module_golden_exposes_registry_symbols() -> None:
    ...
    assert "STATE_SCHEMA = PlaybookThreatIntelIngestV1State" in src
    assert "AGENTIC_HOOK = llm_step" in src
    assert (
        "TOOLS = (pull_upstream_feed, normalise_stix_to_ocsf, "
        "propagate_to_blocklist, activate_detection_rule,)"
    ) in src
