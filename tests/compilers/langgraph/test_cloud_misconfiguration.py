...
Regenerate via::
    ...
    python -m compilers.langgraph.emit \\
        ...
        > tests/compilers/langgraph/golden/cloud_misconfiguration.graph_spec.json

    python -m compilers.langgraph.state \\
        tests/compilers/_shared/fixtures/cloud_misconfiguration.cacao.json \\
        > tests/compilers/langgraph/golden/cloud_misconfiguration.expected.py
"""
...
GOLDEN_GRAPH = (
    Path(__file__).parent / "golden" / "cloud_misconfiguration.graph_spec.json"
)
GOLDEN_MODULE = (
    Path(__file__).parent / "golden" / "cloud_misconfiguration.expected.py"
)
...
def test_graph_spec_golden_has_expected_shape() -> None:
    ...
    # 7 action steps + 2 if-conditions; start/end are not materialised.
...
def test_module_golden_exposes_registry_symbols() -> None:
    ...
    assert "STATE_SCHEMA = PlaybookCloudMisconfigurationV1State" in src
    assert "AGENTIC_HOOK = llm_step" in src
    assert (
        "TOOLS = (ingest_finding, enrich_resource_and_owner, "
        "suppress_and_close, notify_owner, guided_remediation, "
        "re_scan, escalate,)"
    ) in src
