"""Golden tests for the LangGraph reference compiler — post-incident-review.

Mirrors ``test_golden.py`` (vuln-intake) for the post-incident-review
...
Regenerate via::
    ...
    python -m compilers.langgraph.emit \\
        ...
        > tests/compilers/langgraph/golden/post_incident_review.graph_spec.json

    python -m compilers.langgraph.state \\
        tests/compilers/_shared/fixtures/post_incident_review.cacao.json \\
        > tests/compilers/langgraph/golden/post_incident_review.expected.py
"""
...
GOLDEN_GRAPH = (
    Path(__file__).parent / "golden" / "post_incident_review.graph_spec.json"
)
GOLDEN_MODULE = (
    Path(__file__).parent / "golden" / "post_incident_review.expected.py"
)
...
def test_graph_spec_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "LangGraph graph-spec golden drift (post-incident-review). "
...
def test_graph_spec_golden_has_expected_shape() -> None:
    """Sanity-check the golden carries the post-incident-review topology."""
    payload = json.loads(GOLDEN_GRAPH.read_text(encoding="utf-8"))
    assert payload["stable_id"] == "playbook.post_incident_review@v1"
    assert payload["end_sentinel"] == "__END__"
    # 3 action steps, linear chain; no conditional branches.
    assert len(payload["nodes"]) == 3
    assert len(payload["conditional_edges"]) == 0
...
def test_module_matches_golden() -> None:
    ...
    assert rendered == expected, (
        "LangGraph state-module golden drift (post-incident-review). "
...
def test_module_golden_exposes_registry_symbols() -> None:
    ...
    assert "STATE_SCHEMA = PlaybookPostIncidentReviewV1State" in src
    assert "AGENTIC_HOOK = llm_step" in src
    assert (
        "TOOLS = (timeline_collation, blameless_review_template, "
