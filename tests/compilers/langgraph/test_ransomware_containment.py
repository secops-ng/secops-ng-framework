...
Regenerate via::
    ...
    python -m compilers.langgraph.emit \\
        ...
        > tests/compilers/langgraph/golden/ransomware_containment.graph_spec.json

    python -m compilers.langgraph.state \\
        tests/compilers/_shared/fixtures/ransomware_containment.cacao.json \\
        > tests/compilers/langgraph/golden/ransomware_containment.expected.py
"""
...
GOLDEN_GRAPH = (
    Path(__file__).parent / "golden" / "ransomware_containment.graph_spec.json"
)
GOLDEN_MODULE = (
    Path(__file__).parent / "golden" / "ransomware_containment.expected.py"
)
...
def test_graph_spec_golden_has_expected_shape() -> None:
    ...
    # 6 action steps + 2 if-condition; start/end are not materialised.
...
def test_module_golden_exposes_registry_symbols() -> None:
    ...
    assert "STATE_SCHEMA = PlaybookRansomwareContainmentV1State" in src
    assert "AGENTIC_HOOK = llm_step" in src
    assert (
        "TOOLS = (triage_signal, endpoint_isolation_edr_isolate, "
        "endpoint_isolation_network_acl_deny_fallback, identity_revocation, "
        "backup_verification, comms_plan,)"
    ) in src
