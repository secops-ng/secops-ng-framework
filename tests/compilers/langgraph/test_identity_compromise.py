...
Regenerate via::
    ...
    python -m compilers.langgraph.emit \\
        ...
        > tests/compilers/langgraph/golden/identity_compromise.graph_spec.json

    python -m compilers.langgraph.state \\
        tests/compilers/_shared/fixtures/identity_compromise.cacao.json \\
        > tests/compilers/langgraph/golden/identity_compromise.expected.py
"""
...
GOLDEN_GRAPH = (
    Path(__file__).parent / "golden" / "identity_compromise.graph_spec.json"
)
GOLDEN_MODULE = (
    Path(__file__).parent / "golden" / "identity_compromise.expected.py"
)
...
def test_module_golden_exposes_registry_symbols() -> None:
    ...
    assert "STATE_SCHEMA = PlaybookIdentityCompromiseV1State" in src
    assert "AGENTIC_HOOK = llm_step" in src
    assert (
        "TOOLS = (triage_identity_signal, reset_mfa_factors, "
        "revoke_active_sessions, lateral_movement_hunt, "
        "iam_audit_and_persistence_removal,)"
    ) in src
