...
Regenerate via::
    ...
    python -m compilers.langgraph.emit \\
        ...
        > tests/compilers/langgraph/golden/data_exfil.graph_spec.json

    python -m compilers.langgraph.state \\
        tests/compilers/_shared/fixtures/data_exfil.cacao.json \\
        > tests/compilers/langgraph/golden/data_exfil.expected.py
"""
...
GOLDEN_GRAPH = (
    Path(__file__).parent / "golden" / "data_exfil.graph_spec.json"
)
GOLDEN_MODULE = (
    Path(__file__).parent / "golden" / "data_exfil.expected.py"
)
...
def test_graph_spec_golden_has_expected_shape() -> None:
    ...
    # 5 action steps + 2 if-condition steps; start/end are not materialised.
...
def test_module_golden_exposes_registry_symbols() -> None:
    ...
    assert "STATE_SCHEMA = PlaybookDataExfilV1State" in src
