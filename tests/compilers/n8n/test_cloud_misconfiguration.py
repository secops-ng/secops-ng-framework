"""Golden test for the n8n reference compiler — cloud_misconfiguration.

Mirrors ``test_golden.py`` (vuln_intake) for the
``cloud_misconfiguration`` fixture: parses the CACAO v2 playbook, emits
...
Regenerate via::
    ...
    PYTHONPATH=. python -m tools.compile \\
        ...
        --target n8n \\
        --out tests/compilers/n8n/golden/cloud_misconfiguration.n8n.json
"""
...
GOLDEN = Path(__file__).parent / "golden" / "cloud_misconfiguration.n8n.json"
...
def test_cloud_misconfiguration_golden_matches() -> None:
    playbook = parse_file(FIXTURE)
    workflow = emit(playbook)
    rendered = _serialise(workflow)
    expected = GOLDEN.read_text(encoding="utf-8")
    assert rendered == expected, (
        "n8n golden drift (cloud_misconfiguration). Regenerate via "
        "`python -m tools.compile tests/compilers/_shared/fixtures/"
        "cloud_misconfiguration.cacao.json --target n8n --out "
...
def test_emit_file_writes_golden(tmp_path: Path) -> None:
    out = tmp_path / "cloud_misconfiguration.n8n.json"
...
def test_fixture_and_golden_are_in_sync() -> None:
    ...
    if rendered != GOLDEN.read_text(encoding="utf-8"):
        pytest.fail(
            ...
            "`python -m tools.compile tests/compilers/_shared/fixtures/"
            "cloud_misconfiguration.cacao.json --target n8n --out "
            "tests/compilers/n8n/golden/cloud_misconfiguration.n8n.json` "
