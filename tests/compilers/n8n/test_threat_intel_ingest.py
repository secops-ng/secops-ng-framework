"""Golden test for the n8n reference compiler — threat-intel-ingest.

Mirrors ``test_golden.py`` (vuln-intake) for the ``threat_intel_ingest``
...
Regenerate via::
    ...
    PYTHONPATH=. python -m tools.compile \\
        ...
        --target n8n \\
        --out tests/compilers/n8n/golden/threat_intel_ingest.n8n.json
"""
...
GOLDEN = Path(__file__).parent / "golden" / "threat_intel_ingest.n8n.json"
...
def test_threat_intel_ingest_golden_matches() -> None:
    playbook = parse_file(FIXTURE)
    workflow = emit(playbook)
    rendered = _serialise(workflow)
    expected = GOLDEN.read_text(encoding="utf-8")
    assert rendered == expected, (
        "n8n golden drift (threat-intel-ingest). Regenerate via "
        "`python -m tools.compile tests/compilers/_shared/fixtures/"
        "threat_intel_ingest.cacao.json --target n8n --out "
...
def test_emit_file_writes_golden(tmp_path: Path) -> None:
    out = tmp_path / "threat_intel_ingest.n8n.json"
