"""Tests for the EU AI Act orphan-CI assertion (G-02 KRI, eu_ai_act axis).

Mirrors ``test_cra_playbook_orphans`` against the framework-parametrised
linter at ``tools.lint_playbook_orphans``. Real-tree pass case covers
the shipped ``content/mappings/eu_ai_act/`` plus the audited
``_orphan_skip.yaml`` manifest; synthetic fixtures cover grace window,
regression lane, skip-manifest validation, and KRI emission shape.
...
def test_real_tree_passes_orphan_ci() -> None:
    ...
    G-02 floor (eu_ai_act axis) is zero HIGH findings; the assertion
    fires only on regressions or net-new orphans past the grace window.
    """
            ...
            skip:
              - slug: intentionally_unmapped
                rationale: >-
                  ...
                  no EU AI Act edge is appropriate here.
            """
        ),
    )
    findings, summary = check(
