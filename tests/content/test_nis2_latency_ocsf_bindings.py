"""Tests for the NIS2-latency-cluster OCSF source-data-shape binding assertion.

Defends the G-04 catalogue-maturity OCSF dimension for the NIS2
Article 23 significant-incident notification dispatch-latency KRI
triad: every metric whose ``stable_id`` is in the NIS2-latency
...
* the shipped tree passes (every NIS2-latency metric on main is
...
* a synthetic NIS2-latency-cluster metric missing its OCSF ref is
  caught;
* metrics whose ``stable_id`` is outside the allow-list are
  correctly excluded;
* stripping the OCSF ref from any shipped NIS2-latency metric trips
...
from tools.lint_nis2_latency_ocsf_bindings import (
...
def test_shipped_tree_has_no_unbound_nis2_latency_metrics() -> None:
    """Every NIS2-latency-cluster metric on main must carry OCSF."""
    findings = scan()
    assert findings == [], (
        "shipped NIS2-latency cluster has metrics without an OCSF "
        "source-data-shape binding: "
        + ", ".join(f.metric_stable_id for f in findings)
    )


def test_real_tree_covers_nis2_latency_anchors() -> None:
    """Sanity check: the scan classifies the three NIS2 Article 23
    notification dispatch-latency KRI anchors as cluster-class.
    ...
    F-MET-NIS2-LATENCY SKELETON; a future CORE wave may widen the
    expected set.
    """
    ...
    assert not missing, (
        "NIS2-latency-cluster classifier no longer identifies the "
...
# ---------------------------------------------------------------------------
# Synthetic positive: NIS2-latency-cluster metric missing OCSF binding
# ---------------------------------------------------------------------------
...
def test_nis2_latency_metric_without_ocsf_ref_is_flagged(
    ...
    _write_metric(
        ...
        {
            "stable_id": "kri.nis2_incident_early_warning_latency_hours@v1",
            "kind": "kri",
            "telemetry_refs": [],
        },
    )
...
def test_nis2_latency_metric_with_ocsf_ref_passes(
    synthetic_metrics_dir: Path,
) -> None:
    _write_metric(
        synthetic_metrics_dir,
...
def test_out_of_allowlist_stable_id_is_not_nis2_latency(
    synthetic_metrics_dir: Path,
) -> None:
    """Metrics whose ``stable_id`` is not in the NIS2-latency
    ...
    _write_metric(
        ...
        {
            "stable_id": "kri.nis2_something_else_latency_hours@v1",
            "kind": "kri",
            "playbook_refs": [
...
def test_metric_without_stable_id_is_not_nis2_latency(
    synthetic_metrics_dir: Path,
) -> None:
    _write_metric(
        synthetic_metrics_dir,
...
# ---------------------------------------------------------------------------
# Negative regression for every NIS2-latency metric in the cluster:
...
@pytest.mark.parametrize("metric_name", NIS2_LATENCY_METRICS)
def test_stripping_ocsf_ref_trips_assertion(
    ...
    assert original_ocsf_refs, (
        ...
        "NIS2-latency cluster is supposed to leave every metric bound; "
    ...
    assert len(findings) == 1, (
        ...
        "the NIS2-latency-cluster assertion "
...
def test_is_nis2_latency_metric_helper() -> None:
    assert is_nis2_latency_metric(
        "kri.nis2_incident_early_warning_latency_hours@v1"
    )
    assert is_nis2_latency_metric(
        "kri.nis2_incident_final_report_latency_days@v1"
    )
    assert not is_nis2_latency_metric(None)
    assert not is_nis2_latency_metric("")
    assert not is_nis2_latency_metric(
        "kri.nis2_something_else_latency_hours@v1"
    )
    assert not is_nis2_latency_metric(
...
def test_cli_passes_on_shipped_tree() -> None:
    proc = subprocess.run(
        [sys.executable, "-m", "tools.lint_nis2_latency_ocsf_bindings"],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
    )
    assert proc.returncode == 0, proc.stdout + proc.stderr
    assert "nis2-latency-ocsf-bindings: PASS" in proc.stdout
...
def test_cli_json_output_shape(tmp_path: Path, capsys) -> None:
    ...
    assert payload["nis2_latency_cluster"] == sorted(
        NIS2_LATENCY_STABLE_IDS
    )
    assert (
