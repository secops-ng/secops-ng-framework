"""Tests for the SOC 2 orphan-CI assertion (F-MAP-ORPHAN-PARITY stage 1).

Mirrors the five EU-axis modules against the framework-parametrised
linter at ``tools.lint_playbook_orphans``. Real-tree pass case covers
the shipped ``content/mappings/soc2/`` plus the audited
``_orphan_skip.yaml`` manifest (stage 1 classifies every finalized
playbook: 26 already cited, soc2_evidence_collector gains its real
CC4.1 citation, 9 durable other-regime exclusions, 12 interim entries
whose criteria-level citations #931 owes); synthetic fixtures cover
grace window, regression lane, skip-manifest validation, and KRI
emission shape.

This axis existed unguarded while the EU manifests justified their own
exclusions with "inbound graph closed on the home axis under
content/mappings/soc2/" — arming it is what makes that claim
verifiable. Fixtures are built in tmp_path; the real ``content/`` tree
is never mutated.
"""
...
def test_home_axis_claim_holds() -> None:
    """soc2_evidence_collector carries a real inbound soc2 citation.

    The EU-axis manifests justify excluding this playbook with
    "inbound graph closed on the home axis" — this pin is what makes
    that claim CI-verified instead of asserted. It must never live in
    this axis's skip manifest.
    """
    findings, summary = check(
        REPO_ROOT, framework=FRAMEWORK, baseline_ref=None, grace_days=7
    )
    assert "soc2_evidence_collector" not in summary["skipped"], (
        "soc2_evidence_collector must be CITED on its home axis, not "
        "skipped — the EU manifests' exclusion rationales depend on it"
    )
    assert not any(f.slug == "soc2_evidence_collector" for f in findings)
...
def test_orphan_regression_against_baseline_trips_immediately(
    ...
    _add_mapping(root, "tsc-test-cited", ["regressing_pb"])
    _git("add", "-A", cwd=root)
    ...
    (root / "content" / "mappings" / FRAMEWORK / "tsc-test-cited.yaml").unlink()
    _git("add", "-A", cwd=root)
            ...
            skip:
              - slug: intentionally_unmapped
                rationale: >-
                  ...
                  no SOC 2 criterion edge is appropriate here.
            """
        ),
    )
    findings, summary = check(
...
def test_kri_emission_status_ok_on_clean_tree(tmp_path: Path, capsys) -> None:
    ...
    _add_mapping(root, "tsc-some", ["cited_pb"])
    rc = main([
        "--framework", FRAMEWORK,
        "--root", str(root),
        "--baseline-ref", "",
