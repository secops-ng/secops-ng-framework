"""Tests for the GDPR-latency-cluster OCSF source-data-shape binding assertion.

Defends the G-04 catalogue-maturity OCSF dimension for the GDPR
Article 33/34 personal-data-breach notification dispatch-latency KRI
triad: every metric whose ``stable_id`` is in the GDPR-latency
...
* the shipped tree passes (every GDPR-latency metric on main is
...
* a synthetic GDPR-latency-cluster metric missing its OCSF ref is
  caught;
* metrics whose ``stable_id`` is outside the allow-list are
  correctly excluded;
* stripping the OCSF ref from any shipped GDPR-latency metric trips
...
from tools.lint_gdpr_latency_ocsf_bindings import (
...
def test_shipped_tree_has_no_unbound_gdpr_latency_metrics() -> None:
    """Every GDPR-latency-cluster metric on main must carry OCSF."""
    findings = scan()
    assert findings == [], (
        "shipped GDPR-latency cluster has metrics without an OCSF "
        "source-data-shape binding: "
        + ", ".join(f.metric_stable_id for f in findings)
    )


def test_real_tree_covers_gdpr_latency_anchors() -> None:
    """Sanity check: the scan classifies the three GDPR Article 33/34
    breach-notification dispatch-latency KRI anchors as cluster-class.
    ...
    F-MET-GDPR-LATENCY SKELETON; a future CORE wave may widen the
    expected set.
    """
    ...
    assert not missing, (
        "GDPR-latency-cluster classifier no longer identifies the "
...
# ---------------------------------------------------------------------------
# Synthetic positive: GDPR-latency-cluster metric missing OCSF binding
# ---------------------------------------------------------------------------
...
def test_gdpr_latency_metric_without_ocsf_ref_is_flagged(
    ...
    _write_metric(
        ...
        {
            "stable_id": "kri.gdpr_breach_supervisory_authority_notification_latency_hours@v1",
            "kind": "kri",
            "telemetry_refs": [],
        },
    )
...
def test_gdpr_latency_metric_with_ocsf_ref_passes(
    synthetic_metrics_dir: Path,
) -> None:
    _write_metric(
        synthetic_metrics_dir,
...
def test_out_of_allowlist_stable_id_is_not_gdpr_latency(
    synthetic_metrics_dir: Path,
) -> None:
    """Metrics whose ``stable_id`` is not in the GDPR-latency
    allow-list must be kept out — even if their file name is
    suggestive or they share ``playbook.data_subject_rights@v1``
    with a sibling GDPR chain."""
    _write_metric(
        synthetic_metrics_dir,
        "synthetic_out_of_allowlist",
        {
            "stable_id": "kri.gdpr_something_else_latency_hours@v1",
            "kind": "kri",
            "playbook_refs": [
                {"playbook_id": "playbook.data_subject_rights@v1"}
            ],
            "telemetry_refs": [],
        },
    )
...
def test_metric_without_stable_id_is_not_gdpr_latency(
    synthetic_metrics_dir: Path,
) -> None:
    _write_metric(
        synthetic_metrics_dir,
...
# ---------------------------------------------------------------------------
# Negative regression for every GDPR-latency metric in the cluster:
...
GDPR_LATENCY_METRICS = (
    "gdpr_breach_supervisory_authority_notification_latency_hours",
    "gdpr_breach_data_subject_notification_latency_hours",
    "gdpr_breach_dpa_escalation_latency_days",
)


@pytest.mark.parametrize("metric_name", GDPR_LATENCY_METRICS)
def test_stripping_ocsf_ref_trips_assertion(
    ...
    assert original_ocsf_refs, (
        ...
        "GDPR-latency cluster is supposed to leave every metric bound; "
    ...
    assert len(findings) == 1, (
        ...
        "the GDPR-latency-cluster assertion "
...
def test_is_gdpr_latency_metric_helper() -> None:
    assert is_gdpr_latency_metric(
        "kri.gdpr_breach_supervisory_authority_notification_latency_hours@v1"
    )
    assert is_gdpr_latency_metric(
        "kri.gdpr_breach_dpa_escalation_latency_days@v1"
    )
    assert not is_gdpr_latency_metric(None)
    assert not is_gdpr_latency_metric("")
    assert not is_gdpr_latency_metric(
        "kri.gdpr_something_else_latency_hours@v1"
    )
    assert not is_gdpr_latency_metric(
...
def test_cli_passes_on_shipped_tree() -> None:
    proc = subprocess.run(
        [sys.executable, "-m", "tools.lint_gdpr_latency_ocsf_bindings"],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
    )
    assert proc.returncode == 0, proc.stdout + proc.stderr
    assert "gdpr-latency-ocsf-bindings: PASS" in proc.stdout
...
def test_cli_json_output_shape(tmp_path: Path, capsys) -> None:
    ...
    assert payload["gdpr_latency_cluster"] == sorted(
        GDPR_LATENCY_STABLE_IDS
    )
    assert (
