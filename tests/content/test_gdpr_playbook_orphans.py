"""Tests for the GDPR orphan-CI assertion (G-02 KRI, gdpr axis).

Mirrors ``test_dora_playbook_orphans`` and ``test_nis2_playbook_orphans``
against the framework-parametrised linter at
``tools.lint_playbook_orphans``. Real-tree pass case covers the shipped
``content/mappings/gdpr/`` plus the audited ``_orphan_skip.yaml``
manifest (the SKELETON ships every finalized playbook as a skip entry
pointing at the existing ``data-flow-<slug>.md`` doc as interim
coverage); synthetic fixtures cover grace window, regression lane,
skip-manifest validation, and KRI emission shape.
...
def test_real_tree_passes_orphan_ci() -> None:
    ...
    G-02 floor (gdpr axis) is zero HIGH findings; the assertion fires
    only on regressions or net-new orphans past the grace window. The
    SKELETON ships every currently-finalized playbook as an audited
    skip entry pointing at the existing ``data-flow-<slug>.md`` doc
    under ``content/mappings/gdpr/`` as interim coverage until CORE
    authors machine-readable per-clause YAML.
    """
...
def test_orphan_regression_against_baseline_trips_immediately(
    ...
    (root / "content" / "mappings" / FRAMEWORK / "art-test-cited.yaml").unlink()
    _git("add", "-A", cwd=root)
            ...
            skip:
              - slug: intentionally_unmapped
                rationale: >-
                  ...
                  no GDPR edge is appropriate here.
            """
        ),
    )
    findings, summary = check(
