...
CROSSWALK_PATH = REPO_ROOT / "content" / "mappings" / "d3fend" / "gdpr.yaml"
...
@pytest.fixture(scope="module")
def gdpr_entry_ids() -> set[str]:
    """Every ``entries[*].id`` under ``content/mappings/gdpr/``.
    ...
    """
    ids: set[str] = set()
    for path in sorted(GDPR_MAPPINGS_DIR.glob("*.yaml")):
    ...
    assert ids, "no GDPR mapping entries discovered"
    return ids
...
def test_regulation_refs_resolve(
    entries: list[dict], gdpr_entry_ids: set[str]
) -> None:
    unresolved: list[tuple[str, str]] = []
    for e in entries:
        eid = str(e.get("id", "<unknown>"))
        ...
        for ref in refs:
            ...
            assert ref.get("regime") == "gdpr", (
                f"entry {eid!r} regulation_refs regime must be 'gdpr', "
                f"got {ref.get('regime')!r}"
            )
            target = ref.get("entry_id")
            if not (isinstance(target, str) and target in gdpr_entry_ids):
                unresolved.append((eid, str(target)))
    assert not unresolved, (
        "regulation_refs.entry_id values that do not resolve to a real "
        f"entry under content/mappings/gdpr/: {unresolved}"
    )
