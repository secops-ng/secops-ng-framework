"""Round-trip assertion for the ISO/IEC 27001:2022 Annex A OSCAL component-definition.

EXTEND-step complement to ``test_oscal_iso27001_component_definition.py``:
the SKELETON+CORE tests already assert schema conformance, YAML→OSCAL
pair coverage, and a baseline implemented-requirement count. This
...
* The implemented-requirement count meets or exceeds the CORE baseline
  established by ``test_oscal_iso27001_component_definition.py``
  (72 IRs across the four Annex A theme files as of the CORE lane).

The A.8.18–A.8.22 batch that was pending on a sibling branch at
SKELETON time (see the README) has since landed on ``main``; a future
crosswalk landing that broadens the ``content/mappings/iso27001/``
surface will lift the IR count and the CORE baseline will be revised
alongside the regenerated component definition.

Pure stdlib + PyYAML. No network. Sibling of the per-framework
D3FEND crosswalk EXTEND tests under ``tests/content/``.
"""
...
# CORE baseline pinned by ``test_oscal_iso27001_component_definition.py``
# (see PR #694). The EXTEND lane guards the same floor so a regression
# in either module hard-fails independently.
CORE_MIN_IMPLEMENTED_REQUIREMENTS = 72
...
def yaml_entries() -> list[dict]:
    """Every entry across ``annex-a-*.yaml`` theme files."""
    out: list[dict] = []
    for path in YAML_PATHS:
        ...
        for entry in doc.get("entries", []) or []:
            if isinstance(entry, dict):
                out.append(entry)
    assert out, "no ISO 27001 entries discovered under content/mappings/iso27001/"
    return out
...
def test_implemented_requirement_count_meets_core_baseline(
    ...
    """IR count is at or above the CORE baseline pinned in the sibling test."""
    ...
    assert count >= CORE_MIN_IMPLEMENTED_REQUIREMENTS, (
        ...
        f"implemented-requirements, below CORE baseline "
...
def test_every_yaml_entry_with_control_refs_appears_in_oscal(
    ...
    assert expected_entry_ids, (
        "ISO 27001 YAMLs declare no entry with non-empty control_refs — "
