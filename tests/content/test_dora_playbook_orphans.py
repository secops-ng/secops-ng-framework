"""Tests for the DORA orphan-CI assertion (G-02 KRI, dora axis).

Mirrors ``test_nis2_playbook_orphans`` against the framework-parametrised
linter at ``tools.lint_playbook_orphans``. Real-tree pass case covers
the shipped ``content/mappings/dora/`` plus the audited
...
def test_real_tree_passes_orphan_ci() -> None:
    ...
    G-02 floor (dora axis) is zero HIGH findings; the assertion fires
    only on regressions or net-new orphans past the grace window.
    """
            ...
            skip:
              - slug: intentionally_unmapped
                rationale: >-
                  ...
                  no DORA edge is appropriate here.
            """
        ),
    )
    findings, summary = check(
