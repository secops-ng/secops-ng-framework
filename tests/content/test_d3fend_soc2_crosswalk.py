"""Schema-shape validation for the D3FEND ↔ SOC 2 TSC crosswalk.
...
gate for ``content/mappings/d3fend/soc2.yaml`` and the nightly CI lane
that owns G-06 for the SOC 2 crosswalk.
...
* each entry's ``regulation_refs[*].entry_id`` resolves to a real entry
  ``id`` under ``content/mappings/soc2/tsc-*.yaml``;
...
CROSSWALK_PATH = REPO_ROOT / "content" / "mappings" / "d3fend" / "soc2.yaml"
CONTROLS_DIR = REPO_ROOT / "content" / "controls"
SOC2_MAPPINGS_DIR = REPO_ROOT / "content" / "mappings" / "soc2"
...
@pytest.fixture(scope="module")
def soc2_entry_ids() -> set[str]:
    """Every ``entries[*].id`` under ``content/mappings/soc2/tsc-*.yaml``.

    Excludes underscore-prefixed manifests, matching the convention used
    by ``test_mappings.py`` and the orphan linter.
    """
    ids: set[str] = set()
    for path in sorted(SOC2_MAPPINGS_DIR.glob("tsc-*.yaml")):
    ...
    assert ids, "no SOC 2 mapping entries discovered"
    return ids
...
def test_regulation_refs_resolve(
    entries: list[dict], soc2_entry_ids: set[str]
) -> None:
    unresolved: list[tuple[str, str]] = []
    for e in entries:
        eid = str(e.get("id", "<unknown>"))
        ...
        for ref in refs:
            ...
            assert ref.get("regime") == "soc2", (
                f"entry {eid!r} regulation_refs regime must be 'soc2', "
                f"got {ref.get('regime')!r}"
            )
            target = ref.get("entry_id")
            if not (isinstance(target, str) and target in soc2_entry_ids):
                unresolved.append((eid, str(target)))
    assert not unresolved, (
        "regulation_refs.entry_id values that do not resolve to a real "
        f"entry under content/mappings/soc2/: {unresolved}"
    )
