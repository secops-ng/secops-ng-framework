"""Schema-shape validation for the D3FEND ↔ CRA crosswalk.

Sibling of ``test_d3fend_gdpr_crosswalk.py``. Shape gate for
``content/mappings/d3fend/cra.yaml`` at CORE coverage, and the nightly
CI lane that owns G-02 for the CRA crosswalk.
...
* each entry's ``regulation_refs[*].entry_id`` resolves to a real entry
  ``id`` under ``content/mappings/cra/*.yaml`` and each ref carries
  ``regime: cra``;
...
CROSSWALK_PATH = REPO_ROOT / "content" / "mappings" / "d3fend" / "cra.yaml"
CONTROLS_DIR = REPO_ROOT / "content" / "controls"
REGULATION_MAPPINGS_DIR = REPO_ROOT / "content" / "mappings" / "cra"

REGULATION_REGIME = "cra"
...
def regulation_entry_ids() -> set[str]:
    """Every ``entries[*].id`` under ``content/mappings/cra/``.
