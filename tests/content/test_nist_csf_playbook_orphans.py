"""Tests for the NIST CSF 2.0 orphan-CI assertion (F-MAP-ORPHAN-PARITY pkg 2).
...
``content/mappings/nist_csf/`` plus the audited ``_orphan_skip.yaml``
manifest (package 2 classifies every finalized playbook: 36 already
cited by the core-functions crosswalk, 8 durable other-regime
exclusions, 4 interim entries whose CSF citations #943 owes);
synthetic fixtures cover grace window, regression lane, skip-manifest
            ...
            skip:
              - slug: intentionally_unmapped
                rationale: >-
                  ...
                  no nist_csf edge is appropriate here.
            """
        ),
    )
    findings, summary = check(
