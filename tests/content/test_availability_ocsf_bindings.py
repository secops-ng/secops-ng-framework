"""Tests for the availability-cluster OCSF source-data-shape binding assertion.

Defends the G-04 catalogue-maturity OCSF dimension for the service-
availability KPI/KRI cluster (NIS2 Art. 21 / DORA Art. 11): every
metric whose ``stable_id`` is in the availability allow-list must
declare at least one ``telemetry.ocsf.*`` ref.

Coverage mirrors ``test_dora_latency_ocsf_bindings.py`` (this
cluster also classifies by ``stable_id`` allow-list, not
``playbook_refs``):

* the shipped tree passes (every availability metric on main is
  bound to at least one OCSF source-data shape);
* the classifier identifies the six anchor metrics from the
  SKELETON+EXTEND PRs (structural anchor against silent no-op
  degradation);
* a synthetic availability-cluster metric missing its OCSF ref is
  caught;
* metrics whose ``stable_id`` is outside the allow-list are
  correctly excluded;
* stripping the OCSF ref from any shipped availability metric trips
...
from tools.lint_availability_ocsf_bindings import (
...
def test_shipped_tree_has_no_unbound_availability_metrics() -> None:
    """Every availability-cluster metric on main must carry OCSF."""
    findings = scan()
    assert findings == [], (
        "shipped availability cluster has metrics without an OCSF "
        "source-data-shape binding: "
        + ", ".join(f.metric_stable_id for f in findings)
    )


def test_real_tree_covers_availability_anchors() -> None:
    """Sanity check: the scan classifies the six NIS2 Art. 21 / DORA
    Art. 11 service-availability KPI/KRI anchors as cluster-class.
    ...
    the wrong reason. Anchors on the three KPIs shipped in
    F-MET-AVAILABILITY SKELETON and the three KRIs shipped in
    F-MET-AVAILABILITY EXTEND; a future wave may widen the expected
    set.
    """
    ...
    assert not missing, (
        "Availability-cluster classifier no longer identifies the "
...
# ---------------------------------------------------------------------------
# Synthetic positive: availability-cluster metric missing OCSF binding
# ---------------------------------------------------------------------------
...
def test_availability_metric_without_ocsf_ref_is_flagged(
    ...
    _write_metric(
        ...
        {
            "stable_id": "kpi.service_availability_rate@v1",
            "kind": "kpi",
            "telemetry_refs": [],
        },
    )
...
def test_availability_metric_with_ocsf_ref_passes(
    synthetic_metrics_dir: Path,
) -> None:
    _write_metric(
        synthetic_metrics_dir,
...
def test_out_of_allowlist_stable_id_is_not_availability(
    synthetic_metrics_dir: Path,
) -> None:
    """Metrics whose ``stable_id`` is not in the availability
    allow-list must be kept out — even if their file name is
    suggestive or they share a host-chain playbook_ref with the
    NIS2 / DORA continuity mappings."""
    _write_metric(
        synthetic_metrics_dir,
        "synthetic_out_of_allowlist",
        {
            "stable_id": "kpi.some_other_availability_thing@v1",
            "kind": "kpi",
            "playbook_refs": [
...
def test_metric_without_stable_id_is_not_availability(
    synthetic_metrics_dir: Path,
) -> None:
    _write_metric(
        synthetic_metrics_dir,
...
# ---------------------------------------------------------------------------
# Negative regression for every availability metric in the cluster:
...
AVAILABILITY_METRICS = (
    ...
    "availability_below_target_exposure",
    "rto_overrun_exposure_count",
    "continuity_test_overdue",
)


@pytest.mark.parametrize("metric_name", AVAILABILITY_METRICS)
def test_stripping_ocsf_ref_trips_assertion(
    ...
    assert original_ocsf_refs, (
        ...
        "availability cluster is supposed to leave every metric bound; "
    ...
    assert len(findings) == 1, (
        ...
        "the availability-cluster assertion "
...
def test_is_availability_metric_helper() -> None:
    assert is_availability_metric("kpi.service_availability_rate@v1")
    assert is_availability_metric("kri.continuity_test_overdue@v1")
    assert not is_availability_metric(None)
    assert not is_availability_metric("")
    assert not is_availability_metric(
        "kpi.some_other_availability_thing@v1"
    )
    assert not is_availability_metric(
...
def test_cli_passes_on_shipped_tree() -> None:
    proc = subprocess.run(
        [sys.executable, "-m", "tools.lint_availability_ocsf_bindings"],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
    )
    assert proc.returncode == 0, proc.stdout + proc.stderr
    assert "availability-ocsf-bindings: PASS" in proc.stdout
...
def test_cli_json_output_shape(tmp_path: Path, capsys) -> None:
    ...
    assert payload["availability_cluster"] == sorted(
        AVAILABILITY_STABLE_IDS
    )
    assert (
