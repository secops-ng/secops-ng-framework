"""Tests for the on-call-rotation-cluster OCSF source-data-shape binding assertion.

Defends the G-04 catalogue-maturity OCSF dimension for the
on-call-rotation metric family: every metric whose ``playbook_refs``
resolve exclusively to the on-call-rotation cluster must declare at
least one ``telemetry.ocsf.*`` ref.

Coverage mirrors ``test_incident_response_ocsf_bindings.py``:

* the shipped tree passes (every on-call-rotation metric on main is
...
* a synthetic on-call-rotation-cluster metric missing its OCSF ref
  is caught;
* fan-out metrics that span on-call-rotation + other playbooks are
  correctly excluded from the cluster gate;
* stripping the OCSF ref from any shipped on-call-rotation metric
...
from tools.lint_on_call_rotation_ocsf_bindings import (
...
def test_shipped_tree_has_no_unbound_on_call_rotation_metrics() -> None:
    """Every on-call-rotation-cluster metric on main must carry OCSF."""
    findings = scan()
    assert findings == [], (
        "shipped on-call-rotation cluster has metrics without an "
        "OCSF source-data-shape binding: "
        + ", ".join(f.metric_stable_id for f in findings)
    )


def test_real_tree_covers_full_on_call_rotation_cluster() -> None:
    """Sanity check: the scan classifies every shipped on-call-
    rotation metric as cluster-class.
    ...
    for path in sorted((REPO_ROOT / "content" / "metrics").glob("*.yaml")):
        ...
        if is_on_call_rotation_metric(pb):
    ...
    assert not missing, (
        "on-call-rotation-cluster classifier no longer identifies "
...
# ---------------------------------------------------------------------------
# Synthetic positive: on-call-rotation-cluster metric missing OCSF binding
# ---------------------------------------------------------------------------
...
def test_on_call_rotation_metric_without_ocsf_ref_is_flagged(
    synthetic_metrics_dir: Path,
) -> None:
    _write_metric(
        synthetic_metrics_dir,
        "synthetic_unbound_on_call_ack",
        {
            "stable_id": "kpi.synthetic_unbound_on_call_ack@v1",
            "kind": "kpi",
            "playbook_refs": [
                {"playbook_id": "playbook.on_call_rotation@v1"}
            ],
            "telemetry_refs": [],
        },
    )
    ...
    assert (
        ...
        == "kpi.synthetic_unbound_on_call_ack@v1"
    )


def test_on_call_rotation_metric_with_ocsf_ref_passes(
    synthetic_metrics_dir: Path,
) -> None:
    _write_metric(
        synthetic_metrics_dir,
        "synthetic_bound_on_call",
        {
            "stable_id": "kpi.synthetic_bound_on_call@v1",
            "kind": "kpi",
            "playbook_refs": [
                {"playbook_id": "playbook.on_call_rotation@v1"}
            ],
...
def test_fanout_incident_metric_is_not_on_call_rotation(
    synthetic_metrics_dir: Path,
) -> None:
    """Fan-out metrics that span on-call-rotation plus non-cluster
    playbooks (e.g. incident_management) must be kept out by the
    exclusivity gate — those belong to the incident-response cluster,
    not this one."""
    _write_metric(
        synthetic_metrics_dir,
...
def test_fanout_executive_metrics_is_not_on_call_rotation(
    synthetic_metrics_dir: Path,
) -> None:
    """Fan-out metrics that span on-call-rotation plus the
...
def test_metric_without_playbook_refs_is_not_on_call_rotation(
    synthetic_metrics_dir: Path,
) -> None:
    _write_metric(
        synthetic_metrics_dir,
...
# ---------------------------------------------------------------------------
# Negative regression for every on-call-rotation metric in the
...
@pytest.mark.parametrize("metric_name", ON_CALL_ROTATION_METRICS)
def test_stripping_ocsf_ref_trips_assertion(
    ...
    assert original_ocsf_refs, (
        ...
        "on-call-rotation cluster is supposed to leave every metric "
    ...
    assert len(findings) == 1, (
        ...
        "the on-call-rotation-cluster assertion "
...
def test_has_ocsf_binding_helper() -> None:
    ...
    assert has_ocsf_binding(
        ["telemetry.internal.something@v1", "telemetry.ocsf.incident_finding@v1"]
    )
    assert not has_ocsf_binding([])
    assert not has_ocsf_binding(["telemetry.internal.something@v1"])


def test_is_on_call_rotation_metric_helper() -> None:
    assert is_on_call_rotation_metric(["playbook.on_call_rotation@v1"])
    assert not is_on_call_rotation_metric([])
    assert not is_on_call_rotation_metric(
        [
    ...
    assert not is_on_call_rotation_metric(["playbook.incident_management@v1"])


def test_cli_passes_on_shipped_tree() -> None:
    proc = subprocess.run(
        [sys.executable, "-m", "tools.lint_on_call_rotation_ocsf_bindings"],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
    )
    assert proc.returncode == 0, proc.stdout + proc.stderr
    assert "on-call-rotation-ocsf-bindings: PASS" in proc.stdout
...
def test_cli_json_output_shape(tmp_path: Path, capsys) -> None:
    ...
    assert payload["on_call_rotation_cluster"] == sorted(
        ON_CALL_ROTATION_PLAYBOOK_IDS
    )
    assert (
