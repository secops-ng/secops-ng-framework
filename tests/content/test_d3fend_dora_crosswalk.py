"""Schema-shape validation for the D3FEND ↔ DORA crosswalk.

Sibling of ``test_d3fend_gdpr_crosswalk.py``. Shape gate for
``content/mappings/d3fend/dora.yaml`` at CORE coverage, and the nightly
CI lane that owns G-02 for the DORA crosswalk.
...
* each entry's ``regulation_refs[*].entry_id`` resolves to a real entry
  ``id`` under ``content/mappings/dora/*.yaml`` and each ref carries
  ``regime: dora``;
...
CROSSWALK_PATH = REPO_ROOT / "content" / "mappings" / "d3fend" / "dora.yaml"
CONTROLS_DIR = REPO_ROOT / "content" / "controls"
REGULATION_MAPPINGS_DIR = REPO_ROOT / "content" / "mappings" / "dora"

REGULATION_REGIME = "dora"
...
def regulation_entry_ids() -> set[str]:
    """Every ``entries[*].id`` under ``content/mappings/dora/``.
