"""Regenerate driver for examples/temporal/it_security_support_agent.

F-WF-12 CORE-FANOUT-TEMPORAL: the Temporal activity adapter under
:func:`compilers.temporal.evidence.emit_interaction_evidence_artifact_activity`
is bound, the five action bodies on the workflow stub carry the same
``core_body`` bindings into
``content.playbooks.it_security_support_agent.primitives.*`` the n8n
sibling does, and the per-execution interaction-evidence artifact is
materialised through the Temporal activity against
``schemas/evidence/incidents.schema.json`` (reused F-CP-02 stream).

Inputs are byte-identical to the n8n sibling at
``examples/n8n/it_security_support_agent/regenerate.py`` so the
per-target adapters write byte-identical records — the n8n adapter
and the Temporal activity both delegate to the workflow-local
:func:`content.playbooks.it_security_support_agent.primitives.artifact.build_interaction_artifact`
primitive, which is the F-WF-12 CORE invariant. A cross-target
byte-parity test under ``tests/examples/it_security_support_agent/``
pins this.
...
Run from the repo root after any change to the Temporal activity, the
...
``<artifact_id>.json`` the activity writes is dropped after the copy.
"""
...
# JSON-native raw support request — exactly what a Temporal activity
...
# per AGENTS.md §3. Byte-identical to the n8n sibling so the
# cross-target byte-parity invariant holds.
RAW_REQUEST: dict = {
...
def _build_payload() -> dict:
    """Drive the primitive chain to produce the Temporal activity payload.

    Mirrors the CACAO state machine: ingest → classify → resolution →
    handoff → emit. Each step's output is JSON-native and feeds the
    next. The payload is keyed identically to the n8n sibling's
    payload so both adapters resolve to the same primitive call and
    write byte-identical bytes — the F-WF-12 cross-target invariant.
    """
    ...
    return {
        ...
        # The execution_id is byte-identical to the n8n sibling. The
        # interaction-evidence artifact is target-agnostic on the wire
        # (the schema carries no compile_target field), so re-using
        # the same execution_id is what pins the cross-target byte
        # parity invariant — same (workflow_id, execution_id) ⇒ same
        # incident_id ⇒ same artifact_id ⇒ byte-identical record.
...
def main() -> None:
    ...
    written_str = asyncio.run(
        emit_interaction_evidence_artifact_activity(payload, EVIDENCE_DIR)
    )
