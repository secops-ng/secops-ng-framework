"""Regenerate the committed Temporal worked example for supply_chain_security.

F-WF-SCS CORE-FANOUT-TMP — the supply-chain-security workflow ingests
a raw supply-chain signal, canonicalises it into a closed assessment
block (verdict-keyed, supplier-handle-pinned, component-set
canonicalised), and emits a JSON-native supply-chain-evidence artifact
shaped against ``schemas/evidence/supply-chain.schema.json`` (stream:
...
emitting the artifact via the F-CP-03 Temporal activity adapter at
``compilers.temporal.evidence.emit_supply_chain_artifact_activity`` —
exactly what a Temporal worker would do in an operator's instance.
The typed :class:`SupplyChainContext` is passed in, the activity
delegates to the shared helper, and the artifact is written to disk
under ``examples/temporal/supply_chain_security/evidence/``.

Inputs are kept aligned with the n8n sibling at
``examples/n8n/supply_chain_security/regenerate.py`` so the per-target
adapters exercise the same shared helper through their own
compile-target wiring. ``execution_id`` differs by design (Temporal vs
n8n run id) — that is the field the schema's ``artifact_id``
derivation joins on alongside ``workflow_id`` and ``captured_at`` — but
every other anchor (workflow_id, regulation_refs, control_refs,
dependencies, captured_at, owner) is identical so a cross-target
reviewer sees the same shape on both sides.
...
emitter, the Temporal adapter, the F-WF-SCS primitives, or the
canonical playbook::
...
# Operator-supplied raw signal envelope. Mirrors the n8n sibling so the
# F-WF-SCS ``assess-supplier-signal`` primitive yields the same closed
# assessment block on both targets.
RAW_SIGNAL: dict = {
...
def _build_context() -> SupplyChainContext:
    """Drive the CORE primitive chain and produce the activity context.
    ...
    materialised by the F-CP-03 Temporal activity adapter from the
    operator-declared dependency surface and the assessment supplier
    handle.
    """
    assessment = assess_supplier_signal(**RAW_SIGNAL)
    # Sanity-check the join the activity will validate: the assessed
    # supplier handle must appear in the declared dependency surface.
    assert assessment["affected_supplier_handle"] == (
        "provider.upstream_dep_eu@v1"
    ), "assessment supplier handle drifted from the n8n sibling"

    return SupplyChainContext(
        workflow_id="supply_chain_security",
        execution_id="temporal:exec-scs-0001",
...
def main() -> None:
    ...
    written_str = asyncio.run(
        emit_supply_chain_artifact_activity(ctx, EVIDENCE_DIR)
    )
