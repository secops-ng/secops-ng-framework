...
The vulnerability-intake playbook compiled to Temporal exercises the
...
``examples/temporal/vuln_intake/evidence/{supply-chain,crypto}/``. This
...
This script drives the Temporal activity adapter at
``compilers.temporal.evidence.emit_bundle_manifest_activity`` exactly
as a Temporal worker would: the typed :class:`BundleContext` is passed
in, the activity delegates to the shared helper, and
``bundle.manifest.json`` is written atomically into the bundle root.

Run from the repo root after any change to the bundle shared collector
or the Temporal activity adapter::
...
# Temporal vuln_intake compile target. The bundle inlines copies of
