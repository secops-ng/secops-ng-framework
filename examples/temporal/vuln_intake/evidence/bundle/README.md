# examples/temporal/vuln_intake/evidence/bundle
...
playbook compiled by the Temporal reference compiler.
...
The vuln_intake workflow exercises two of the seven shipped evidence
...
from the siblings under `examples/temporal/vuln_intake/evidence/crypto/`
and `examples/temporal/vuln_intake/evidence/supply-chain/`, so the
bundle directory is fully self-contained — a reviewer never needs to
leave it to verify the chain. The other five streams (risk-analysis,
...
The manifest is produced by the Temporal activity adapter at
`compilers/temporal/evidence/bundle_activity.py`, which wraps the
shared collector under `compilers/_shared/evidence/bundle.py`. The
activity is invoked from a Temporal worker with a typed
`BundleContext` carrying the bundle window, the regulatory anchors,
and the bundle's own root directory; the collector walks
`content/evidence/<stream>/` under that root and assembles a manifest
conforming to `schemas/evidence/bundle.schema.json`.

## Layout

| Path                                                       | Source                                                        | Format            |
|------------------------------------------------------------|---------------------------------------------------------------|-------------------|
| `bundle.manifest.json`                                     | `compilers.temporal.evidence.bundle_activity`                 | manifest JSON     |
| `content/evidence/crypto/secret-handling-attestation.json` | copy of `examples/temporal/vuln_intake/evidence/crypto/`      | F-CP-05 artifact  |
| `content/evidence/supply-chain/dependencies-snapshot.json` | copy of `examples/temporal/vuln_intake/evidence/supply-chain/` | F-CP-03 artifact |
...
PYTHONPATH=. python examples/temporal/vuln_intake/evidence/bundle/regenerate.py
```
