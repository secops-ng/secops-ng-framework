...
The incident-management workflow exercises one of the seven shipped
...
```sh
PYTHONPATH=. python examples/temporal/incident-management/evidence/bundle/regenerate.py
```

Re-runs reproduce the bundle byte-for-byte. The script writes the
manifest via the Temporal activity and re-emits the inlined incidents
