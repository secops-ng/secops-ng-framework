...
activity bodies to real connectors (vendor register, contract
repository, criticality-scoring inputs, exit-strategy documentation
store, periodic-review signal source) is the operator's job.
...
Equivalent direct invocation:
    ...
    PYTHONPATH=. python -m tools.compile \
        content/playbooks/dora_tpr_management/playbook.cacao.json \
        --target temporal \
        --out examples/temporal/dora_tpr_management/workflow.temporal.py

The drift guard between the committed worked example and the emitter
output is pinned by
`tests/examples/dora_tpr_management/test_golden.py`.
