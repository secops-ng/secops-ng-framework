...
class PlaybookDoraIctRiskSelfassessV1Workflow:
    ...
    # Human-in-the-loop scaffold for CACAO step action--92000000-0000-4000-8000-000000000002.
    # State + signal + query — the integrator wires `run()` to
    # await `_collect_section_evidence_decision is not None` before continuing.
    _collect_section_evidence_decision: bool | None = None
    _collect_section_evidence_reason: str | None = None

    @workflow.signal
    def collect_section_evidence_approve(self, decision: bool, reason: str | None = None) -> None:
        """Signal handler — operator releases the workflow with decision/reason."""
        self._collect_section_evidence_decision = decision
        self._collect_section_evidence_reason = reason

    @workflow.query
    def collect_section_evidence_status(self) -> str:
        """Query handler — `pending` until a signal arrives, then `approved`/`denied`."""
        if self._collect_section_evidence_decision is None:
            return "pending"
        return "approved" if self._collect_section_evidence_decision else "denied"

    # Human-in-the-loop scaffold for CACAO step action--92000000-0000-4000-8000-000000000003.
    # State + signal + query — the integrator wires `run()` to
    # await `_map_evidence_to_sections_decision is not None` before continuing.
    _map_evidence_to_sections_decision: bool | None = None
    _map_evidence_to_sections_reason: str | None = None

    @workflow.signal
    def map_evidence_to_sections_approve(self, decision: bool, reason: str | None = None) -> None:
        """Signal handler — operator releases the workflow with decision/reason."""
        self._map_evidence_to_sections_decision = decision
        self._map_evidence_to_sections_reason = reason

    @workflow.query
    def map_evidence_to_sections_status(self) -> str:
        """Query handler — `pending` until a signal arrives, then `approved`/`denied`."""
        if self._map_evidence_to_sections_decision is None:
            return "pending"
        return "approved" if self._map_evidence_to_sections_decision else "denied"

    # Human-in-the-loop scaffold for CACAO step action--92000000-0000-4000-8000-000000000004.
    # State + signal + query — the integrator wires `run()` to
    # await `_score_per_section_coverage_decision is not None` before continuing.
    _score_per_section_coverage_decision: bool | None = None
    _score_per_section_coverage_reason: str | None = None

    @workflow.signal
    def score_per_section_coverage_approve(self, decision: bool, reason: str | None = None) -> None:
        """Signal handler — operator releases the workflow with decision/reason."""
        self._score_per_section_coverage_decision = decision
        self._score_per_section_coverage_reason = reason

    @workflow.query
    def score_per_section_coverage_status(self) -> str:
        """Query handler — `pending` until a signal arrives, then `approved`/`denied`."""
        if self._score_per_section_coverage_decision is None:
            return "pending"
        return "approved" if self._score_per_section_coverage_decision else "denied"

    # Human-in-the-loop scaffold for CACAO step action--92000000-0000-4000-8000-000000000005.
    # State + signal + query — the integrator wires `run()` to
