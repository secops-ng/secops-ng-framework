"""Regenerate scaffold for examples/temporal/contractual_obligations_tracker.

F-WF-10 SKELETON-FANOUT-TMP: scaffold-only. At this layer the
...
so there is no Temporal compiler emitter binding to drive and no
...
no-op pending the F-WF-10 CORE-FANOUT-TMP sibling card.
...
def main() -> None:
    ...
    print(
        f"contractual_obligations_tracker (temporal): mirrored canonical CACAO source -> "
        f"{MIRROR.relative_to(REPO_ROOT)}."
    )
    print("Workflow emission and obligation-evidence artefact deferred to F-WF-10 CORE-FANOUT-TMP.")
