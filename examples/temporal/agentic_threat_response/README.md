# examples/temporal/agentic_threat_response
...
activity bodies to real connectors (agentic-threat detection source,
IdP session / token revocation, network micro-segmentation call,
incident-management hand-off, evidence bundle store) is the operator's
job.
...
Equivalent direct invocation:
    ...
    PYTHONPATH=. python -m tools.compile \
        content/playbooks/agentic_threat_response/playbook.cacao.json \
        --target temporal \
        --out examples/temporal/agentic_threat_response/workflow.temporal.py

The drift guard between the committed worked example and the emitter
output is pinned by
`tests/examples/agentic_threat_response/test_golden.py`.
