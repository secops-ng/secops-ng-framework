...
Worked example: the `playbook.patch_management@v1` CACAO v2 playbook
...
real connectors (patch-availability feed, patch-criticality classifier,
canary-ring deployer, canary health-gate probe, fan-out deployer,
dated-attestation evidence store, and the maintenance-owner
notification channel) is the operator's job.

This worked example pins the Temporal end of the three-target contract
for the `patch_management` playbook (NIS2 Art.21(2)(e)). The
n8n and LangGraph siblings land in separate SKELETON-EXAMPLE cards;
until then, this directory is the Temporal end alone.
