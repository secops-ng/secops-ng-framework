...
class PlaybookEuAiActRiskManagementV1Workflow:
    ...
    async def run(self) -> None:
        with _TRACER.start_as_current_span(
            ...
            attributes={'secops_ng.compile.target': 'temporal', 'secops_ng.playbook.id': 'playbook--40a1b2c3-d4e5-4f60-8a1b-c2d3e4f5a6b8', 'secops_ng.playbook.version': '0.3.0'},
        ):
            AuditTrail.current().append(
                AuditRecord(span_name='workflow.playbook.eu_ai_act_risk_management@v1', attributes={'secops_ng.compile.target': 'temporal', 'secops_ng.playbook.id': 'playbook--40a1b2c3-d4e5-4f60-8a1b-c2d3e4f5a6b8', 'secops_ng.playbook.version': '0.3.0'})
            )
            raise NotImplementedError(
