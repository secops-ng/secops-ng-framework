"""Regenerate scaffold for examples/temporal/infra_posture_management.
...
no-op pending the F-WF-06 CORE-FANOUT-TMP sibling card.
...
``schemas/evidence/posture.schema.json``) when CORE-FANOUT-TMP lands.
"""
...
def main() -> None:
    ...
    print(
        f"infra_posture_management (temporal): mirrored canonical CACAO source -> "
        f"{MIRROR.relative_to(REPO_ROOT)}."
    )
    print("Workflow emission and posture-evidence artefact deferred to F-WF-06 CORE-FANOUT-TMP.")
