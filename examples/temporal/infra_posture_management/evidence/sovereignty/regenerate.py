"""Regenerate the committed sovereignty evidence worked example (temporal).
...
representative execution by driving the Temporal activity adapter at ``compilers.temporal.evidence.emit_sovereignty_artifact_activity`` exactly as a Temporal worker would (typed context in, absolute artifact path out via ``asyncio.run``).
...
emitter or the temporal adapter::
    ...
    PYTHONPATH=. python examples/temporal/infra_posture_management/evidence/sovereignty/regenerate.py
...
def main() -> None:
    written = asyncio.run(
        emit_sovereignty_artifact_activity(CTX, HERE)
    )
