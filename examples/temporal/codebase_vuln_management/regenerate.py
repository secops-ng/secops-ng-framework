"""Regenerate the committed codebase disclosure-timeline worked example (Temporal).

F-WF-07 CORE-TEMPORAL — the codebase-vulnerability-management workflow
emits one disclosure-timeline record per (SBOM, advisory, component)
finding. This script materialises one such record for one
representative finding by driving the Temporal activity adapter at
``compilers.temporal.evidence.emit_disclosure_timeline_artifact_activity``
exactly as a Temporal worker would: a typed
:class:`DisclosureTimelineContext` is passed in, the activity
delegates to the shared helper, and the artifact is written to disk
under ``examples/temporal/codebase_vuln_management/evidence/``.
...
``examples/n8n/codebase_vuln_management/`` so the per-target adapters
write byte-identical records — the cross-target byte-parity guarantee
the F-WF-07 CORE siblings collectively pin (see the F-WF-07 EXTEND
golden sibling for the explicit cross-target byte-parity test once it
ships).
...
``<id>.json`` written by the activity is the SHA-256-named sibling of
the same bytes.
"""
...
# examples/n8n/codebase_vuln_management/regenerate.py so the per-target
# adapters emit byte-identical records. The underlying advisory
...
def main() -> None:
    ...
    written_str = asyncio.run(
        emit_disclosure_timeline_artifact_activity(CTX, EVIDENCE_DIR)
    )
    written = Path(written_str)
    # The activity writes <id>.json; copy to the stable human-friendly
