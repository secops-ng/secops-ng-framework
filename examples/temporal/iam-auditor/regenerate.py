"""Regenerate the committed iam-auditor access-evidence artifact (Temporal).

F-WF-08 SKELETON-FANOUT-TMP — the iam-auditor capability-inventory
...
``examples/temporal/iam-auditor/evidence/``.
...
``examples/n8n/iam-auditor/regenerate.py`` so the per-target adapters
...
# ``examples/n8n/iam-auditor/regenerate.py``; ``compile_target`` and
...
CTX = AccessContext(
    ...
    caller_identity=CallerIdentity(
        ...
        # invoked the compiled iam-auditor workflow. No individual
