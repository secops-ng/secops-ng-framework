...
async def define_dort_scope(testing_window: str) -> str:
    ...
    with _TRACER.start_as_current_span(
        ...
        raise NotImplementedError(
            f"CACAO action stub not implemented: step_id='action--55000000-0000-4000-8000-000000000002'"
        )
...
async def tlpt_trigger_and_planning_gate(testing_window: str, entity_significance_tier: str, dort_scope_catalogue: str) -> str:
    ...
    with _TRACER.start_as_current_span(
        ...
        raise NotImplementedError(
            f"CACAO action stub not implemented: step_id='action--55000000-0000-4000-8000-000000000003'"
        )
...
async def red_team_scoping_approval(dort_scope_catalogue: str, tlpt_trigger_decision: str) -> str:
    ...
    with _TRACER.start_as_current_span(
        ...
        raise NotImplementedError(
            f"CACAO action stub not implemented: step_id='action--55000000-0000-4000-8000-000000000004'"
        )
...
async def remediation_tracking(dort_scope_catalogue: str, red_team_scoping_id: str) -> dict[str, object]:
    ...
    with _TRACER.start_as_current_span(
        ...
        raise NotImplementedError(
            f"CACAO action stub not implemented: step_id='action--55000000-0000-4000-8000-000000000005'"
        )
...
class PlaybookDoraTlptProgrammeV1Workflow:
    ...
    async def run(self) -> None:
        with _TRACER.start_as_current_span(
            ...
            attributes={'secops_ng.compile.target': 'temporal', 'secops_ng.playbook.id': 'playbook--55d0e1f2-3a4b-4c5d-8e6f-1a2b3c4d5e6f', 'secops_ng.playbook.version': '0.1.0'},
        ):
            AuditTrail.current().append(
                AuditRecord(span_name='workflow.playbook.dora_tlpt_programme@v1', attributes={'secops_ng.compile.target': 'temporal', 'secops_ng.playbook.id': 'playbook--55d0e1f2-3a4b-4c5d-8e6f-1a2b3c4d5e6f', 'secops_ng.playbook.version': '0.1.0'})
            )
            raise NotImplementedError(
