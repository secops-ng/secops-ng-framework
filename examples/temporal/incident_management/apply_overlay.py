...
CACAO source to produce the Temporal mirror at this directory's
``playbook.cacao.json``.

F-WF-05 CORE-WIRE-TMPRL (SKELETON wave) seam: the canonical CACAO source
at ``content/playbooks/incident_management/playbook.cacao.json`` does
not yet carry ``x_secops_ng.core_body`` blocks. The Temporal SKELETON
example diverges from the canonical to demonstrate the primitive
wire-in shape ahead of the sibling LangGraph CORE-WIRE card. When that
lands, the canonical gets the core_body blocks promoted upward and this
...
Bindings are cell-for-cell identical to the n8n sibling overlay so the
three compile targets stay in lock-step.
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Apply the F-WF-05 CORE-WIRE-TMPRL SKELETON-wave per-step "
