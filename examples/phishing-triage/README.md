...
bytes out. To regenerate this folder from a clean checkout:
    ...
    PB=content/playbooks/phishing-triage/playbook.cacao.json

    PYTHONPATH=. python -m tools.compile "$PB" \
        --target n8n \
        --out examples/phishing-triage/n8n/workflow.json

    PYTHONPATH=. python -m compilers.temporal "$PB" \
        --out examples/phishing-triage/temporal/workflow.py

    PYTHONPATH=. python -m compilers.langgraph.emit "$PB" \
        > examples/phishing-triage/langgraph/graph_spec.json
