# alert_triage — LangGraph worked example

End-to-end demonstration of the SecOps-NG LangGraph reference compiler
on the alert_triage CACAO source playbook. It is aimed at an
integrator who already runs LangGraph and wants to adopt the portable
SecOps-NG alert_triage playbook without re-platforming: the example
...
| File | Role |
|------|------|
| `playbook.cacao.json` | Portable CACAO v2 playbook — byte-deterministic JSON mirror of `content/playbooks/alert_triage.cacao.yaml`, the canonical source. The mirror exists because the LangGraph emitter consumes JSON; the two formats round-trip through `yaml.safe_load` + `json.dumps`. |
...
the CACAO branch label. For alert_triage:
...
export OTEL_SERVICE_NAME="alert_triage"
```
...
`content/playbooks/alert_triage/primitives/`:
...
under `content/playbooks/alert_triage/primitives/`:
...
The walkthrough in
[`../../../docs/cookbook/alert_triage.md`](../../../docs/cookbook/alert_triage.md)
