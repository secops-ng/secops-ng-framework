# examples/langgraph/codebase-vuln-management

Worked example: one disclosure-timeline evidence record emitted by the
**LangGraph** target for the F-WF-07 *codebase vulnerability
management* workflow. The canonical CACAO playbook lives at
`../../../content/playbooks/codebase-vuln-management/playbook.cacao.json`;
this folder holds one representative artefact emitted by the LangGraph
node adapter, plus the regeneration script.

## Scope

CORE-LANGGRAPH — the LangGraph target only. The disclosure-timeline-record
schema lives at
...
Temporal adapter at `../../temporal/codebase-vuln-management/`. The
per-target byte-parity goldens and the cookbook walkthrough land in
separate EXTEND sibling cards.

## Source

| Path                                                     | Purpose                                |
|----------------------------------------------------------|----------------------------------------|
| `regenerate.py`                                          | Drives the LangGraph node end-to-end   |
| `evidence/disclosure-timeline-record.json`               | One emitted finding                    |

Regenerate after any change to the shared emitter or the LangGraph
adapter, from the repo root:

    PYTHONPATH=. python examples/langgraph/codebase-vuln-management/regenerate.py

The node writes a deterministic `<id>.json`; the script copies it to
...
byte-identical to the n8n and Temporal siblings so the per-target
adapters write byte-identical records.

## What this example does not do

- It does not emit a runnable `graph.langgraph.py`. The merged F-WF-07
  SKELETON playbook ships placeholder action bodies that the
  topology-translating CACAO → LangGraph emitter cannot lower today;
  the CORE-LANGGRAPH scope here is the *evidence emitter node*, not
  the workflow translator. A runnable graph lands once the CORE-FANOUT
  card wires action bodies into the playbook.
