# cloud-misconfiguration — LangGraph worked example

End-to-end demonstration of the SecOps-NG LangGraph reference compiler
on the cloud-misconfiguration CACAO playbook. It is aimed at an
...
```bash
./examples/langgraph/cloud-misconfiguration/regenerate.sh
```
...
The cloud-misconfiguration playbook branches twice — once to suppress
known false positives, once to escalate when a remediation fails to
verify:

1. `ingest finding` — pull the misconfiguration finding from the
   detection source the operator already runs (CSPM, cloud-native
   posture tool, custom rule).
2. `enrich resource and owner` — attach resource metadata and the
   accountable owner so the rest of the workflow has context.
3. `known false positive?` — an `if-condition` step. Enrichment writes
   a status into `state["step_status"][src]`; the router maps `success`
   (known FP) to the `suppress and close` branch, and `failure`
   (genuine finding) to the remediation chain.
4. Remediation chain — `notify owner`, `guided remediation`, then
   `re-scan` to verify the fix.
5. `remediation verified?` — second `if-condition`. The router maps
   `success` to the main end sentinel, and `failure` to `escalate`
   before terminating.
...
- It does not execute the graph. The `@tool` bodies raise
  ...
  (CSPM, ticketing, ChatOps, cloud-provider IAM).
