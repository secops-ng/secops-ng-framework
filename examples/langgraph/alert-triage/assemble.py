"""Runnable assembly for the alert-triage LangGraph worked example.
...
CACAO ``if-condition`` and ``switch-condition`` steps emit as GraphSpec
``conditional_edges`` entries: ``{src, branches: {label: dst, ...},
default}``. At runtime the preceding action writes a status into
``State['step_status'][step_id]``; the router reads that key and
returns the matching branch label so LangGraph dispatches to the
correct successor. For alert-triage:

* the ``if-condition`` after enrich reads the suppression decision
  (``success`` → suppress-and-close, ``failure`` → classify);
* the ``switch-condition`` after classification reads the priority
  bucket (``p1_severe`` / ``p2_high`` / ``p3_routine`` /
  ``p4_informational``) and routes to the matching response action.

Default fallback is the ``default`` field of the conditional edge
(``__END__`` if absent), so a missing or unrecognised status
terminates the run rather than dead-locking.
