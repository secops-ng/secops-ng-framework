...
golden under `tests/examples/langgraph/onboarding_offboarding_tracker/` at
this layer. The compiler emitter, the per-execution evidence
artifact, and the byte-parity golden land in the F-WF-11
CORE-FANOUT-LG sibling card queued serially after this SKELETON
...
| `evidence/`                | (per-execution output)   | Placeholder; representative access-evidence artifact lands in CORE-FANOUT-LG |
...
emit a langgraph workflow artefact at this layer — the canonical playbook
ships with declarative placeholder step bodies (no `core_body`
bindings), so there are no primitive bindings for the langgraph compiler
emitter to translate yet. The emitter and the worked-artefact
emission land in F-WF-11 CORE-FANOUT-LG.
...
compile-target binding lands in F-WF-11 CORE-FANOUT-LG.
