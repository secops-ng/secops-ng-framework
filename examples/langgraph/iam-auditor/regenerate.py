"""Regenerate the committed iam-auditor access-evidence artifact (LangGraph).

F-WF-08 SKELETON-FANOUT-LG — the iam-auditor capability-inventory
...
``examples/n8n/iam-auditor/regenerate.py`` and
``examples/temporal/iam-auditor/regenerate.py`` so the per-target
...
# ``examples/n8n/iam-auditor/regenerate.py`` and
# ``examples/temporal/iam-auditor/regenerate.py``; ``compile_target``
