...
```sh
PYTHONPATH=. python examples/langgraph/incident-management/evidence/bundle/regenerate.py
```

Re-runs reproduce the bundle byte-for-byte. The script writes the
manifest via the LangGraph node and re-emits the inlined incidents
