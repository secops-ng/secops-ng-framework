...
CACAO source to produce the LangGraph mirror at this directory's
``playbook.cacao.json``.

F-WF-05 CORE-WIRE-LG (SKELETON wave) seam: the canonical CACAO source
at ``content/playbooks/incident_management/playbook.cacao.json`` does
not yet carry ``x_secops_ng.core_body`` blocks. The LangGraph SKELETON
example diverges from the canonical to demonstrate the primitive
wire-in shape; bindings are cell-for-cell identical to the n8n +
...
card (n8n #259, Temporal #260, LangGraph). The seam closes in a
subsequent card that promotes the core_body blocks upward into the
canonical (a single source of truth for the three compile targets), at
which point this overlay collapses to empty and the regenerate.sh
script (and this module) remain in place so the build path is stable
across the seam closure.
...
SKELETON-stage incident_management the only overlay key is ``core_body``,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Apply the F-WF-05 CORE-WIRE-LG SKELETON-wave per-step "
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Path to write the LangGraph mirror to.",
    )
