# phishing-triage — LangGraph worked example

End-to-end demonstration of the SecOps-NG LangGraph reference compiler
on the phishing-triage CACAO playbook. It is aimed at an integrator who
...
| File | Role |
|------|------|
| `playbook.cacao.json` | Portable CACAO v2 playbook — byte-identical mirror of `content/playbooks/phishing-triage/playbook.cacao.json`, the input to the compiler. |
...
test in `tests/examples/phishing_triage/` fails the suite if the
...
- It does not execute the graph. The `@tool` bodies raise
  ...
  (user-reported / mailbox-sweep email source, email security gateway /
  URL sandbox / attachment sandbox enrichment, suppression cache,
  intent classifier, response-branch ticketing / containment actions).
