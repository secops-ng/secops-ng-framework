...
# operator's runtime.
"""Generated LangGraph state + tool bindings for playbook.executive_metrics@v1."""
...
class PlaybookExecutiveMetricsV1State(TypedDict, total=False):
    """LangGraph state for CACAO playbook playbook.executive_metrics@v1.

    Playbook id: playbook--e0a05ec0-0000-4f00-8a1b-d3e4f5a6b7c8
    ...
    # ISO 8601 window the rollup covers (e.g. `2026-05-01/2026-05-31`). Externally supplied by the scheduler so multiple cadences (monthly, quarterly) reuse the same playbook.
    rollup_window: str
...
async def llm_step(state: PlaybookExecutiveMetricsV1State) -> dict:
...
STATE_SCHEMA = PlaybookExecutiveMetricsV1State
