"""Regenerate the committed asset_management worked-example evidence record (LangGraph).

F-WF-ASSET CORE-FANOUT-LANGGRAPH — the asset_management workflow emits
...
representative reconciliation window by driving the LangGraph node
adapter at
``compilers.langgraph.evidence.emit_asset_inventory_delta_artifact_node``
exactly as a LangGraph integrator would: a state mapping carrying a
JSON-native payload is passed in, the node delegates to the
workflow-local primitive, and the artifact is written to disk under
...
``examples/n8n/asset_management/regenerate.py`` and
``examples/temporal/asset_management/regenerate.py`` so the per-target
...
byte-parity test under ``tests/examples/asset_management/`` pins
this.

Run from the repo root after any change to the asset-inventory-delta
primitive or the LangGraph node adapter::
...
# JSON-native payload — exactly what a LangGraph node input would
# carry on state after a preceding node walked the operator-configured
# inventory sources, ran the deterministic reconcile + classify
# primitives, and composed the per-execution payload. The shape
...
#
# Byte-identical to the n8n and Temporal siblings' PAYLOADs so the
# cross-target byte-parity invariant holds. artifact_id is
# target-agnostic on the wire
...
def main() -> None:
    ...
    result = emit_asset_inventory_delta_artifact_node(
        {
            "asset_inventory_delta_payload": PAYLOAD,
            "evidence_output_dir": EVIDENCE_DIR,
        }
    )
    written = Path(result["asset_inventory_delta_artifact_path"])
    ...
    print(
        f"wrote {SNAPSHOT} "
        f"(artifact_id={result['asset_inventory_delta_artifact_id']})"
    )
