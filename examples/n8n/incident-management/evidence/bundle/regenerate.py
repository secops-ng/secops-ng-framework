"""Regenerate the committed auditor-bundle worked example (n8n).

The incident-management playbook compiled to n8n exercises the
...
Streams the incident-management workflow does not exercise
...
This script drives the n8n adapter at
``compilers.n8n.evidence.emit_bundle_manifest_n8n`` exactly as an
``executeCommand`` / ``Code`` node would in an operator's n8n instance:
the payload is JSON-native (``content_root`` as a string, ISO-8601
timestamps with the ``Z`` suffix, ``regulation_refs`` as a JSON array),
and the adapter writes ``bundle.manifest.json`` atomically into the
bundle root. The inlined incidents artifact is rebuilt from the same
typed context the per-target byte-parity golden pins, so the bundle
directory's content tree is reproducible end-to-end.

Run from the repo root after any change to the bundle shared collector,
the incidents emitter, or the n8n adapter::

    PYTHONPATH=. python examples/n8n/incident-management/evidence/bundle/regenerate.py
...
# would marshal for the shared collector. The shape mirrors
...
# ``artifact_paths`` are bundle-relative (per the schema's
# ``content/evidence/<stream>/...`` regex).
PAYLOAD: dict = {
    "content_root": str(HERE),
    "generated_at": "2026-07-05T13:00:00Z",
    "bundle_window_start": "2026-05-01T00:00:00Z",
    "bundle_window_end": "2026-07-31T23:59:59Z",
    "regulation_refs": [
        "nis2:art-20",
        "nis2:art-21-2-b",
        "nis2:art-22",
    ...
    "source_url": "https://example.org/runs/incident-management-example-0001",
    "commit_sha": "d31a3a7",
...
def main() -> None:
    _stage_bundle_tree()
    result = emit_bundle_manifest_n8n(PAYLOAD, HERE)
