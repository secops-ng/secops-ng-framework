# crypto_posture_management — n8n worked example

End-to-end demonstration of the SecOps-NG n8n reference compiler on
the `crypto_posture_management` CACAO playbook. It is aimed at an
...
cross-target parity lane for the `crypto_posture_management` playbook
(NIS2 Art.21(2)(h)). The Temporal sibling already ships under
`../../temporal/crypto_posture_management/`; the LangGraph sibling
lands in a separate sibling card. Until all three are committed, this
...
3. n8n loads seven nodes wired into the topology described below. The
...
inventory, certificate-posture probe, key-rotation cadence checker,
dated-attestation evidence store, and the cryptography-owner
...
PYTHONPATH=. python -m tools.compile \
    content/playbooks/crypto_posture_management/playbook.cacao.json \
    --target n8n \
    --out examples/n8n/crypto_posture_management/workflow.n8n.json
```

The drift guard in
`tests/examples/n8n/crypto_posture_management/test_golden.py` fails
...
The crypto_posture_management playbook is a linear inventory-probe-
check-attest-notify chain with no conditional branching at the
workflow layer. Seven n8n nodes, one per CACAO step:

1. `crypto_posture_management_start` (`manualTrigger`) — entry point;
   matches the CACAO `start` step. Carries the workflow-scope
   variables (`__posture_window__`, `__crypto_scope__`, …) the
   operator's scheduler or operator-initiated trigger supplies.
2. `inventory crypto policy` (`set`) — resolve the operator's
   declared cryptography policy at the start of the posture window
   (algorithm floor, key-size floor, rotation cadence, TLS-version
   floor) against `__crypto_scope__`; emits `__policy_inventory_id__`.
3. `probe cert posture` (`set`) — probe the certificate posture of
   the TLS endpoints in scope against the policy snapshot; emits
   `__cert_posture_id__`.
4. `check key rotation` (`set`) — walk managed keys and compare
   last-rotation timestamps to the declared cadence carried in the
   policy snapshot; emits `__rotation_status__`.
5. `evidence capture` (`set`) — persist a dated cryptography-posture
   attestation record covering the policy snapshot, the cert-posture
   probe, and the key-rotation status; emits `__attestation_id__`.
6. `notify crypto owner` (`set`) — surface the attestation to the
   cryptography owner via the operator's notification channel.
7. `crypto_posture_management_end` (`noOp`) — end sentinel.
...
|----------|------------|-------------|---------------------|
| `inventory crypto policy` | `posture_window`, `crypto_scope` | `policy_inventory_id` | `control_refs`, `telemetry_refs` |
| `probe cert posture` | `crypto_scope`, `policy_inventory_id` | `cert_posture_id` | `control_refs`, `telemetry_refs` |
| `check key rotation` | `crypto_scope`, `policy_inventory_id` | `rotation_status` | `control_refs`, `telemetry_refs` |
| `evidence capture` | `policy_inventory_id`, `cert_posture_id`, `rotation_status`, `posture_window` | `attestation_id` | `control_refs`, `telemetry_refs` |
| `notify crypto owner` | `attestation_id`, `crypto_scope` | — | `telemetry_refs` |
...
- It does not execute the workflow. The Set nodes carry the CACAO I/O
  ...
  them to their own crypto-policy inventory source, certificate
  probe, key-rotation evidence store, attestation evidence store, and
  cryptography-owner notification endpoints.
...
- It does not encode the algorithm floor, key-size floor, TLS-version
  floor, per-key-class rotation cadence, or the wording of the
  cryptography-owner notification — these are intent-bearing values
...
canonical CACAO source. Detection bindings (expired-cert, weak-
cipher, missed-rotation upstream rule ids) and the cipher-suite-floor
and rotation-cadence KPI catalogue entries are owned by CORE / EXTEND
