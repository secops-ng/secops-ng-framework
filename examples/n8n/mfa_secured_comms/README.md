# mfa_secured_comms — n8n worked example

End-to-end demonstration of the SecOps-NG n8n reference compiler on
the `mfa_secured_comms` CACAO playbook. It is aimed at an integrator
who already runs n8n and wants to adopt a portable SecOps-NG playbook
...
(NIS2 Art.21(2)(j)). The Temporal sibling already ships under
`../../temporal/mfa_secured_comms/`; the LangGraph sibling lands in
a separate sibling card. Until all three are committed, this folder
...
assignments; binding those rows to real connectors (MFA-coverage
probe, continuous-authentication assessor, out-of-band emergency-
channel verifier, dated-attestation evidence store, and the
authentication-owner notification channel) is the operator's job.
...
PYTHONPATH=. python -m tools.compile \
    content/playbooks/mfa_secured_comms/playbook.cacao.json \
    --target n8n \
    --out examples/n8n/mfa_secured_comms/workflow.n8n.json
```

The drift guard in `tests/examples/n8n/mfa_secured_comms/test_golden.py`
...
verify, attest, notify. Seven n8n nodes, one per CACAO step:

1. `mfa_secured_comms_start` (`manualTrigger`) — entry point; matches
   the CACAO `start` step. Carries the workflow-scope variables
   (`__posture_window__`, `__auth_scope__`, …) the operator's
   scheduler or operator-initiated trigger supplies.
2. `probe mfa coverage` (`set`) — probe the identity-provider surface
   to confirm MFA coverage across in-scope principals; emits
   `__mfa_coverage_id__`.
3. `assess continuous auth` (`set`) — assess whether continuous-
   authentication signals are observed on long-lived sessions; emits
   `__continuous_auth_id__`.
4. `verify oob channels` (`set`) — verify the out-of-band emergency
   communications channel is reachable independently of the primary
   information-system path; emits `__oob_channel_status__`.
5. `evidence capture` (`set`) — persist a dated posture-attestation
   record covering MFA coverage, continuous-auth, and OOB
   reachability; emits `__attestation_id__`.
6. `notify authentication owner` (`set`) — surface the attestation
   to the authentication owner via the operator's notification
   channel.
7. `mfa_secured_comms_end` (`noOp`) — end sentinel.
...
|----------|------------|-------------|---------------------|
| `probe mfa coverage` | `posture_window`, `auth_scope` | `mfa_coverage_id` | `control_refs`, `telemetry_refs` |
| `assess continuous auth` | `auth_scope`, `mfa_coverage_id` | `continuous_auth_id` | `control_refs`, `telemetry_refs` |
| `verify oob channels` | `auth_scope`, `posture_window` | `oob_channel_status` | `control_refs`, `telemetry_refs` |
| `evidence capture` | `mfa_coverage_id`, `continuous_auth_id`, `oob_channel_status`, `posture_window` | `attestation_id` | `control_refs`, `telemetry_refs` |
| `notify authentication owner` | `attestation_id`, `auth_scope` | — | `telemetry_refs` |
...
- It does not execute the workflow. The Set nodes carry the CACAO I/O
  ...
  them to their own identity-provider probe, continuous-auth assessor,
  OOB-channel verifier, evidence-store, and notification endpoints.
- It does not ship operator credentials, secrets, or environment-
  specific endpoints. Secrets stay with the operator.
- It does not encode MFA-coverage thresholds, continuous-auth signal
  selection, OOB-channel reachability criteria, or the wording of the
  authentication-owner notification — these are intent-bearing values
...
canonical CACAO source. Detection bindings (missing-MFA, stale-
session, unreachable-OOB upstream rule ids) and the
session-staleness / OOB-reachability KPI catalogue entries are owned
by CORE / EXTEND siblings, per the SKELETON note on the canonical
playbook.
