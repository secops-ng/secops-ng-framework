"""Regenerate the committed asset_management worked-example evidence record (n8n).

F-WF-ASSET CORE-FANOUT-N8N — the asset_management workflow emits one
asset-inventory-delta evidence record per scheduled reconciliation
execution. This script materialises one such record for one
representative reconciliation window by driving the n8n adapter at
``compilers.n8n.evidence.emit_asset_inventory_delta_artifact_n8n``
exactly as an ``executeCommand`` / ``Code`` node would in an
operator's n8n instance: the payload is JSON-native (every field a
JSON primitive — strings, ints, lists, dicts; ``captured_at`` as the
canonical ISO-8601 ``...Z`` string) and the adapter writes the
...
``baseline-drift`` divergence — the four buckets of the closed
delta taxonomy the NIS2 Art. 21(2)(i) reviewer consumes. Asset
identifiers stay opaque operator-side strings, individual personal
names and credential-shaped strings are out of scope per AGENTS.md
§3, and ``snapshot_id`` / ``source_set_id`` are the deterministic
digests the reconcile primitive emits.

Run from the repo root after any change to the asset-inventory-delta
primitive or the n8n adapter::
...
# JSON-native payload — exactly what an n8n Code / executeCommand
# node would marshal. The shape mirrors the keyword arguments of
...
def main() -> None:
    ...
    result = emit_asset_inventory_delta_artifact_n8n(PAYLOAD, EVIDENCE_DIR)
