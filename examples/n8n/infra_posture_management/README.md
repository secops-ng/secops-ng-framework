...
under `tests/examples/n8n/infra_posture_management/` at this layer.
The compiler emitter, the per-execution evidence artifact, and the
byte-parity golden land in the F-WF-06 CORE-FANOUT-N8N sibling card
...
| `evidence/`                | (per-execution output) | Placeholder; representative posture-evidence artifact lands in CORE-FANOUT-N8N |
...
It does **not** emit an n8n workflow artefact at this layer — the
...
bindings for the n8n compiler emitter to translate yet. The emitter
and the worked-artefact emission land in F-WF-06 CORE-FANOUT-N8N.
...
- **F-WF-06 CORE-FANOUT-N8N** — bind the n8n compiler emitter against
  the canonical primitive set, regenerate `workflow.n8n.json`
  ...
  `tests/examples/n8n/infra_posture_management/`.
