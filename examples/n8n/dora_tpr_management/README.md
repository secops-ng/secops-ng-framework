...
Worked example: the `playbook.dora_tpr_management@v1` CACAO v2
...
connectors (vendor register, contract repository, criticality-scoring
inputs, exit-strategy documentation store, and the periodic-review
signal source) is the operator's job.
...
Scenario, workflow, regulatory anchors (DORA Art. 28 / 30), and
...
`content/playbooks/dora_tpr_management/playbook.cacao.json` is the
single source. The drift guard between the committed worked example
and the emitter output is pinned by the dora_tpr_management example
test suite under `tests/examples/dora_tpr_management/`.
