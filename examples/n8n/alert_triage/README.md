...
Canonical CACAO source playbook:
    ...
    ../../../content/playbooks/alert_triage.cacao.yaml
...
| `…000008` | response: p1 severe — page and escalate | `primitives.response.escalation_route(priority=__priority__, asset_criticality=…, internet_exposed=…, regulated_data=…)` | Operator binds the paging tier and the escalation handoff to their incident_management playbook; SLA enforcement lives in `kpi.mttr_critical@v1`. |
...
`content/playbooks/alert_triage/`, network policy) so it is not
