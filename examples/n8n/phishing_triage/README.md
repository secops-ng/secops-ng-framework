...
Equivalent direct invocation:
    ...
    PYTHONPATH=. python -m tools.compile \
        content/playbooks/phishing_triage/playbook.cacao.json \
        --target n8n \
        --out examples/n8n/phishing_triage/workflow.n8n.json

The canonical playbook under
`content/playbooks/phishing_triage/playbook.cacao.json` is the single
source. The drift guard between the committed worked example and the
emitter output is pinned by the phishing_triage example test suite
