"""Temporal activity wrapper for the crypto-attestation evidence emitter.

This is the SKELETON wiring for F-CP-05: one Temporal-side
``@activity.defn`` that delegates to
``compilers._shared.evidence.emit_crypto_attestation_artifact``. The
activity is intentionally a thin adapter so the shared helper stays the
source of truth — CORE-FANOUT will fan the same helper out to the n8n
and LangGraph targets without re-implementing record assembly.
...
__all__ = ["emit_crypto_attestation_artifact_activity"]


@activity.defn
async def emit_crypto_attestation_artifact_activity(
    ctx: CryptoAttestationContext,
    output_dir: str | os.PathLike[str],
) -> str:
    """Persist one crypto-attestation evidence artifact under ``output_dir``.
    ...
    Per F-CP-05's per-execution contract, one attestation is emitted per
    ...
    if not isinstance(ctx, CryptoAttestationContext):
        ctx = CryptoAttestationContext(**dict(ctx))  # type: ignore[arg-type]
    written = emit_crypto_attestation_artifact(ctx, output_dir)
