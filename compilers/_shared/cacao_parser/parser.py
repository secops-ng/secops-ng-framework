...
def _build_core_body(raw: Mapping[str, Any] | None) -> CoreBody | None:
    ...
    the primitive dotted-pattern, the ``in`` value shape, and rejects
    unknown keys via ``additionalProperties: false``. The parser simply
    freezes the ``in`` mapping so the AST stays immutable. Returns
    ``None`` when the key is absent so CACAO-only steps remain
