...
| `…000006`   | notify authentication owner    | compose the closed notification payload via `primitives.notify.compose_owner_notification` (role-shaped recipient, attestation ref, SHA-256 delivery dedup key); the delivery along the operator's pre-bound channel (ticketing, chat, email) stays operator-wired; read-only posture-readiness dispatch — no enrolment mutation, no session mutation, no channel mutation | bound (delivery operator-wired) |
